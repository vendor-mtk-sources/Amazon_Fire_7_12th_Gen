...
static void spin_bug(raw_spinlock_t *lock, const char *msg)
{
	...
	spin_dump(lock, msg);
	if (snprintf(aee_str, sizeof(aee_str), "%s: [%s]\n", current->comm, msg) < 0)
		aee_str[0] = 0;
...
#ifdef CONFIG_DEBUG_LOCK_ALLOC
		if (snprintf(lock_name, 64, "%s", lock->dep_map.name) < 0)
#else
		if (snprintf(lock_name, 64, "%ps", lock) < 0)
#endif
			lock_name[0] = 0;
		...
		*((unsigned int *)&lock->raw_lock), lock->magic,
		...
		if (print_once) {
			...
			if (!is_critical_lock_held(lock)) {
				...
				if (snprintf(aee_str, sizeof(aee_str),
					"Spinlock lockup: (%s) in %s\n",
					lock_name, current->comm) < 0)
					aee_str[0] = 0;
				aee_kernel_warning_api(__FILE__, __LINE__,
					DB_OPT_DUMMY_DUMP | DB_OPT_FTRACE,
					aee_str, "spinlock debugger\n");
