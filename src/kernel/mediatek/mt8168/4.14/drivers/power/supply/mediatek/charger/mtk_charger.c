...
#define TEMP_T0_TO_T1_DEFAULT_CHARGING_CURRENT 350000
...
void do_sw_jeita_state_machine(struct charger_manager *info)
{
	...
	if (sw_jeita->sm != TEMP_T2_TO_T3) {
		if (!gm.use_aging_zcv) {
			...
			/* use_aging_zcv */
			if (sw_jeita->sm == TEMP_ABOVE_T4)
				sw_jeita->cv = info->data.jeita_temp_above_t4_cv_voltage_aging1;
			else if (sw_jeita->sm == TEMP_T3_TO_T4)
				sw_jeita->cv = info->data.jeita_temp_t3_to_t4_cv_voltage_aging1;
			else if (sw_jeita->sm == TEMP_T2_TO_T3)
				sw_jeita->cv = 0;
			else if (sw_jeita->sm == TEMP_T1_TO_T2)
				sw_jeita->cv = info->data.jeita_temp_t1_to_t2_cv_voltage_aging1;
			else if (sw_jeita->sm == TEMP_T0_TO_T1)
				sw_jeita->cv = info->data.jeita_temp_t0_to_t1_cv_voltage_aging1;
			else if (sw_jeita->sm == TEMP_BELOW_T0)
				sw_jeita->cv = info->data.jeita_temp_below_t0_cv_voltage_aging1;
			else
...
static ssize_t show_bcm_flag(struct device *dev,
	 struct device_attribute *attr, char *buf)
{
	struct charger_manager *pinfo = dev->driver_data;
	char temp_info[200] = "";
...
static ssize_t store_bcm_flag(struct device *dev,
	 struct device_attribute *attr, const char *buf, size_t size)
{
	struct charger_manager *pinfo = dev->driver_data;
	...
	if (!strncmp(buf, "0", strlen("0"))) {
		...
		bat_metrics_bcm_flag(pinfo->bcm_flag);
		return size;
	}
...
static DEVICE_ATTR(bcm_flag, 0664, show_bcm_flag,
	 store_bcm_flag);
...
static void reset_invalid_charger_state(struct charger_manager *info)
{
	info->invalid_charger_det_done = false;
	switch_set_state(&info->invalid_charger, 0);
}

static void reset_dpm_eoc_state(struct charger_manager *info)
{
...
#define DPM_CV_BAT_VDIFF_UV     20000
#define DPM_IBAT_THRESHOLD      2000 /* 200ma */
#define DPM_STATE_COUNT_MAX     9    /* dpm mode appeared 9 times in succession */
#define DPM_RECHG_LOW_SOC_DIFF  2
#define DPM_INPUT_CUR_LIMIT_UA  300000
#define DPM_CHARGE_CUR_LIMIT_UA 300000
static void check_dpm_eoc_state(struct charger_manager *info)
{
	...
	vbat_uv = battery_get_bat_voltage() * 1000;
	...
	pr_info("%s: cv %d,bcv %d,vbat %d,ibat %d,icl %d,ccl %d,soc %d,vdpm %d,idpm %d,chg %d,done %d\n",
		...
		ibat / 10, input_current_limit / 1000, chg_current_limit / 1000,
	 ...
	 */
	if ((ibat <= 0 || ibat > DPM_IBAT_THRESHOLD)
	    || (cur_cv != battery_cv)
	    || (input_current_limit < DPM_INPUT_CUR_LIMIT_UA)
	    || (chg_current_limit < DPM_CHARGE_CUR_LIMIT_UA)
	    || !en_chg
	    || chg_done
	    || (vbat_uv <= (battery_cv - DPM_CV_BAT_VDIFF_UV))
	    || (!(vdpm || idpm))) {
	...
	bool in)
{
	if (in) {
		...
		if ((info->enable_bat_eoc_protect && info->bat_eoc_protect)
			...
			if (info->disconnect_duration >= info->bat_eoc_protect_reset_time) {
...
static void charger_top_off_mode(struct charger_manager *info)
{
	int ui_soc;
	int top_off_cv;
...
static bool get_bcm_flag(struct charger_manager *info)
{
	if (pinfo->bcm_flag == BCM_FLAG_NONE)
		return false;
...
stop_charging:
	...
	chr_err("%s: tmp:%d (jeita:%d sm:%d cv:%d en:%d) (sm:%d) en:%d c:%d s:%d ov:%d e:%d d:%d i:%d %d %d\n",
		__func__,
		temperature, info->enable_sw_jeita, info->sw_jeita.sm,
		info->sw_jeita.cv, info->sw_jeita.charging, thermal->sm,
		charging, info->cmd_discharging, info->safety_timeout,
		info->vbusov_stat, info->bat_eoc_protect,
		info->dpm_disable_charging, switch_get_state(&info->invalid_charger),
...
static void battery_protect_algo(struct charger_manager *info)
{
	...
	if (!info->bat_eoc_protect) {
		...
		if (info->custom_charging_cv > 0 || sw_jeita_cv < battery_cv) {
			chr_err("%s: Reset EOC protection state\n",
				__func__);
			reset_bat_eoc_protect_state(info);
		}
	}
...
static void battery_full_track(struct charger_manager *info)
{
	...
	if (chg_done || info->bat_eoc_protect || info->dpm_disable_charging) {
...
static int __ap15_charger_detection_read_dt(struct charger_manager *info,
			struct device *dev)
{
	struct device_node *np = dev->of_node;
	struct sw_aicl_data *data = &info->sw_aicl;
	...
	data->aicl_enable = true;

	__parse_node(np, "aicl_charging_current_max",
		&data->aicl_charging_current_max);
	__parse_node(np, "aicl_step_current", &data->aicl_step_current);

	__parse_node(np, "aicl_step_interval", &data->aicl_step_interval);
	__parse_node(np, "aicl_vbus_valid", &data->aicl_vbus_valid);
	__parse_node(np, "aicl_vbus_state_phase", &data->aicl_vbus_state_phase);

	__parse_node(np, "ap15_charger_input_current_max",
		&data->aicl_input_current_max);
	__parse_node(np, "ap15_charger_input_current_min",
		&data->aicl_input_current_min);
	...
	pr_info("%s: %d [%d %d] [%d %d %d %d]\n", __func__,
				...
				data->aicl_input_current_max,
				data->aicl_input_current_min,
				data->aicl_step_current,
				data->aicl_step_interval,
				data->aicl_vbus_valid,
				data->aicl_vbus_state_phase);

	if (!data->aicl_charging_current_max
			|| !data->aicl_input_current_max
			|| !data->aicl_input_current_min
			|| !data->aicl_step_current
			|| !data->aicl_step_interval
			|| !data->aicl_vbus_valid
			|| !data->aicl_vbus_state_phase) {
		pr_info("9w %s: parameters is missing, disable this feature\n",
			__func__);
		data->aicl_enable = false;
	}
	return 0;
}
#endif
				...
				struct device *dev)
{
...
#ifdef CONFIG_MTK_USE_AGING_ZCV
	if (of_property_read_u32(np, "jeita_temp_above_t4_cv_voltage_aging1", &val) >= 0) {
		info->data.jeita_temp_above_t4_cv_voltage_aging1 = val;
	} else {
		chr_err(
			"use default JEITA_TEMP_ABOVE_T4_CV:%d\n", JEITA_TEMP_ABOVE_T4_CV);
		info->data.jeita_temp_above_t4_cv_voltage_aging1 = JEITA_TEMP_ABOVE_T4_CV;
	}

	if (of_property_read_u32(np, "jeita_temp_t3_to_t4_cv_voltage_aging1", &val) >= 0) {
		info->data.jeita_temp_t3_to_t4_cv_voltage_aging1 = val;
	} else {
		chr_err(
			"use default JEITA_TEMP_T3_TO_T4_CV:%d\n", JEITA_TEMP_T3_TO_T4_CV);
		info->data.jeita_temp_t3_to_t4_cv_voltage_aging1 = JEITA_TEMP_T3_TO_T4_CV;
	}

	if (of_property_read_u32(np, "jeita_temp_t2_to_t3_cv_voltage_aging1", &val) >= 0) {
		info->data.jeita_temp_t2_to_t3_cv_voltage_aging1 = val;
	} else {
		chr_err(
			"use default JEITA_TEMP_T2_TO_T3_CV:%d\n", JEITA_TEMP_T2_TO_T3_CV);
		info->data.jeita_temp_t2_to_t3_cv_voltage_aging1 = JEITA_TEMP_T2_TO_T3_CV;
	}

	if (of_property_read_u32(np, "jeita_temp_t1_to_t2_cv_voltage_aging1", &val) >= 0) {
		info->data.jeita_temp_t1_to_t2_cv_voltage_aging1 = val;
	} else {
		chr_err(
			"use default JEITA_TEMP_T1_TO_T2_CV:%d\n", JEITA_TEMP_T1_TO_T2_CV);
		info->data.jeita_temp_t1_to_t2_cv_voltage_aging1 = JEITA_TEMP_T1_TO_T2_CV;
	}

	if (of_property_read_u32(np, "jeita_temp_t0_to_t1_cv_voltage_aging1", &val) >= 0) {
		info->data.jeita_temp_t0_to_t1_cv_voltage_aging1 = val;
	} else {
		chr_err(
			"use default JEITA_TEMP_T0_TO_T1_CV:%d\n", JEITA_TEMP_T0_TO_T1_CV);
		info->data.jeita_temp_t0_to_t1_cv_voltage_aging1 = JEITA_TEMP_T0_TO_T1_CV;
	}

	if (of_property_read_u32(np, "jeita_temp_below_t0_cv_voltage_aging1", &val) >= 0) {
		info->data.jeita_temp_below_t0_cv_voltage_aging1 = val;
	} else {
		chr_err(
			"use default JEITA_TEMP_BELOW_T0_CV:%d\n", JEITA_TEMP_BELOW_T0_CV);
		info->data.jeita_temp_below_t0_cv_voltage_aging1 = JEITA_TEMP_BELOW_T0_CV;
	}
	...
	if (of_property_read_u32(np, "jeita_temp_above_t4_cv_voltage_aging2", &val) >= 0) {
		...
		chr_err(
			"use default JEITA_TEMP_ABOVE_T4_CV:%d\n", JEITA_TEMP_ABOVE_T4_CV);
		info->data.jeita_temp_above_t4_cv_voltage_aging2 = JEITA_TEMP_ABOVE_T4_CV;
	}
	...
	if (of_property_read_u32(np, "jeita_temp_t3_to_t4_cv_voltage_aging2", &val) >= 0) {
		...
		chr_err(
			"use default JEITA_TEMP_T3_TO_T4_CV:%d\n", JEITA_TEMP_T3_TO_T4_CV);
		info->data.jeita_temp_t3_to_t4_cv_voltage_aging2 = JEITA_TEMP_T3_TO_T4_CV;
	}
	...
	if (of_property_read_u32(np, "jeita_temp_t2_to_t3_cv_voltage_aging2", &val) >= 0) {
		...
		chr_err(
			"use default JEITA_TEMP_T2_TO_T3_CV:%d\n", JEITA_TEMP_T2_TO_T3_CV);
		info->data.jeita_temp_t2_to_t3_cv_voltage_aging2 = JEITA_TEMP_T2_TO_T3_CV;
	}
	...
	if (of_property_read_u32(np, "jeita_temp_t1_to_t2_cv_voltage_aging2", &val) >= 0) {
		...
		chr_err(
			"use default JEITA_TEMP_T1_TO_T2_CV:%d\n", JEITA_TEMP_T1_TO_T2_CV);
		info->data.jeita_temp_t1_to_t2_cv_voltage_aging2 = JEITA_TEMP_T1_TO_T2_CV;
	}
	...
	if (of_property_read_u32(np, "jeita_temp_t0_to_t1_cv_voltage_aging2", &val) >= 0) {
		...
		chr_err(
			"use default JEITA_TEMP_T0_TO_T1_CV:%d\n", JEITA_TEMP_T0_TO_T1_CV);
		info->data.jeita_temp_t0_to_t1_cv_voltage_aging2 = JEITA_TEMP_T0_TO_T1_CV;
	}
	...
	if (of_property_read_u32(np, "jeita_temp_below_t0_cv_voltage_aging2", &val) >= 0) {
		...
		chr_err(
			"use default JEITA_TEMP_BELOW_T0_CV:%d\n", JEITA_TEMP_BELOW_T0_CV);
		info->data.jeita_temp_below_t0_cv_voltage_aging2 = JEITA_TEMP_BELOW_T0_CV;
	}
#endif
	...
	if (of_property_read_u32(np, "temp_t0_to_t1_charger_current", &val) >= 0)
	...
	if (of_property_read_u32(np, "temp_t3_to_t4_charger_current", &val) >= 0)
		info->data.temp_t3_to_t4_charger_current = val;
	else {
		chr_err("use default charging current\n");
		info->data.temp_t3_to_t4_charger_current = -1;
	}
...
#if defined(CONFIG_SW_AICL_SUPPORT)
	__ap15_charger_detection_read_dt(info, dev);
#endif
			...
			struct device_attribute *attr, char *buf)
{
	...
	if (true == pinfo->sw_aicl.aicl_enable)
		return scnprintf(buf, PAGE_SIZE, "%d\n",
			pinfo->sw_aicl.aicl_result);
...
static int mtk_charger_probe(struct platform_device *pdev)
{
	...
	info->disconnect_time.tv_sec = 0 - info->bat_eoc_protect_reset_time;
...
static void mtk_charger_shutdown(struct platform_device *dev)
{
	...
	if (info->chg1_dev != NULL && info->do_event != NULL)
		unregister_charger_device_notifier(info->chg1_dev, &info->chg1_nb);
	if (mtk_pe20_get_is_connect(info) || mtk_pe_get_is_connect(info)) {
		if (info->chg2_dev)
