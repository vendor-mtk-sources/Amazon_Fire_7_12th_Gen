...
int charger_dev_get_indpm_state(struct charger_device *chg_dev,
				bool *vdpm, bool *idpm)
{
	if (chg_dev != NULL && chg_dev->ops != NULL &&
	    chg_dev->ops->get_indpm_state)
		return chg_dev->ops->get_indpm_state(chg_dev, vdpm, idpm);

	return -ENOTSUPP;
}
EXPORT_SYMBOL(charger_dev_get_indpm_state);
