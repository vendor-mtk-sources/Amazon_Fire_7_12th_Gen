...
static int fusb301_read_device_id(struct fusb301_chip *chip)
{
	...
	dev_info(cdev, "fusb301a usb device id: 0x%x\n", FUSB301_REG_DEVICEID);
	if (IS_ERR_VALUE_FUSB301(rc)) {
...
int fusb301_get_mode(struct fusb301_chip *chip)
{
	struct device *cdev = &chip->client->dev;
	int rc;
...
static int fusb_set_mode(struct tcpc_device *tcpc, int mode)
{
	...
	if (mode == TYPEC_ROLE_SNK) {
		rc = fusb301_set_mode(chip, FUSB301_SNK);
		if (IS_ERR_VALUE_FUSB301(rc)) {
...
static int fusb301a_pm_suspend(struct device *dev)
{
	...
	struct fusb301_chip *chip = i2c_get_clientdata(client);
	struct device *cdev = &chip->client->dev;
	unsigned int status, rc;

	if (device_may_wakeup(&client->dev))
		enable_irq_wake(chip->irq_gpio);
	disable_irq(chip->irq_gpio);

	status = fusb301_get_mode(chip);
	pr_info("%s: status=%d\n", __func__, status);

	if (FUSB301_SNK != status) {
		rc = fusb301_set_mode(chip, FUSB301_SNK);
		if (IS_ERR_VALUE_FUSB301(rc)) {
			dev_err(cdev, "%s: failed to set mode in suspend\n",
				__func__);
		}
	}
	return 0;
...
static int fusb301a_pm_resume(struct device *dev)
{
	...
	status = fusb301_get_mode(chip);
	pr_info("%s: status=%d, chip->cc_mode =%d\n",
		__func__, status, chip->cc_mode);
	if (status == FUSB301_SNK && chip->cc_mode >= TYPEC_ROLE_DRP) {
		rc = fusb301_set_mode(chip, FUSB301_DRP_ACC);
		if (IS_ERR_VALUE_FUSB301(rc)) {
			dev_err(cdev, "%s: failed to set mode in resume\n",
				__func__);
		}
	}
	return 0;
			...
			const struct i2c_device_id *id)
{
	...
	if (IS_ENABLED(CONFIG_DUAL_ROLE_USB_INTF)) {
		...
		chip->desc = desc;
		dev_info(cdev, "fusb301_probe desc ok\n");
	}
	g_fusb301_chip = chip;
	dev_info(cdev, "fusb301_probe OK!!\n");
	enable_irq_wake(chip->irq_gpio);
