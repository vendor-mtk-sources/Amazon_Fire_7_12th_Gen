...
static void max20342_sbu_is_wet(struct max20342 *max20342, int cur_state)
{
	if (cur_state == TYPE_DRY) {
		...
		tcpm_typec_set_mode(max20342->tcpc, TYPEC_ROLE_SNK);
...
static void max20342_sbu_is_dry(struct max20342 *max20342, int cur_state)
{
	if (cur_state != TYPE_DRY) {
		...
		tcpm_typec_set_mode(max20342->tcpc, TYPEC_ROLE_TRY_SNK);
...
static int max20342_init(struct max20342 *max20342)
{
	...
	/* LPDRP mode */
	max20342_config_interface(max20342, COMM_CTRL1, LPDRP_MASK,
		 LPDRP_SHIFT, LPDRP_VAL);
...
void max20342_vbus_detect(struct work_struct *work)
{
	int state;
...
static bool vsim1_init(struct max20342 *max20342)
{
	int ret;
	max20342->vsim1_ldo = devm_regulator_get(max20342->dev, "reg-ldmax");
	if (IS_ERR(max20342->vsim1_ldo)) {
		ret = PTR_ERR(max20342->vsim1_ldo);
		pr_err("%s failed to get reg-ldmax LDO, %d\n", __func__, ret);
		return false;
	}

	regulator_set_voltage(max20342->vsim1_ldo, 3100000, 3100000);
	ret = regulator_enable(max20342->vsim1_ldo);
	if (ret < 0) {
...
static int max20342_get_gpio_id(struct max20342 *max20342)
{
	int ret = 0;
	struct device_node *np = max20342->dev->of_node;

	max20342->gpio_max20342_id = of_get_named_gpio(np, "max20342-id", 0);
	if (!gpio_is_valid(max20342->gpio_max20342_id)) {
		pr_err("%s: no valid gpio\n", __func__);
		return -1;
	}

	ret = gpio_request(max20342->gpio_max20342_id, "max20342id");
	if (ret) {
		pr_err("[%s] request liquid gpio failed ret = %d\n",
			 __func__, ret);
		return -1;
	}
	gpio_direction_input(max20342->gpio_max20342_id);
	ret = gpio_get_value(max20342->gpio_max20342_id);

	gpio_free(max20342->gpio_max20342_id);

	if (ret) {
		pr_err("[%s] max20342 is not present, exit: id=%d\n", __func__, ret);
		return -1;
	}
...
static bool max20342_pinctrl_init(struct max20342 *max20342)
{
	 ...
	 const struct i2c_device_id *id)
{
	...
	ret = max20342_get_gpio_id(max20342);
	if (ret) {
		ret = -ENODEV;
		goto err_gpio_id;
	}

	if (!vsim1_init(max20342)) {
		ret = -ENODEV;
		goto err_vsim1;
	}
	...
	max20342->mos_is_valid = max20342_pinctrl_init(max20342);
...
err_tcpc:
err_id:
	regulator_disable(max20342->vsim1_ldo);
err_vsim1:
err_gpio_id:
