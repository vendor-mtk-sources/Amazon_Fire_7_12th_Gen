...
static void fusb251_routine_work(struct work_struct *work)
{
	...
	if (state == TYPE_DRY) {
		if (fusb251->status_ovp_cc || fusb251->status_ovp_sbu) {
			...
			tcpm_typec_set_mode(fusb251->tcpc, TYPEC_ROLE_SNK);
		...
		if (!(fusb251->mos_status_sbu1 || fusb251->mos_status_sbu2)) {
			...
			tcpm_typec_set_mode(fusb251->tcpc, TYPEC_ROLE_TRY_SNK);
...
void fusb251_vbus_detect(struct work_struct *work)
{
	int state;
...
static void fusb251_vbus_changed(void)
{
	pr_info("%s enter\n", __func__);
	if (!g_fusb251)
		return;

	cancel_delayed_work(&g_fusb251->delayed_work);
	queue_delayed_work(system_freezable_wq, &g_fusb251->delayed_work,
			...
			const struct i2c_device_id *id)
{
	...
	fusb251->chg1_dev = get_charger_by_name("primary_chg");
	if (fusb251->chg1_dev)
	...
	fusb251->chg1_nb.notifier_call = liquid_dev_event;
	register_charger_device_notifier(fusb251->chg1_dev, &fusb251->chg1_nb);
	...
	INIT_DELAYED_WORK(&g_fusb251->delayed_work, fusb251_vbus_detect);
	queue_delayed_work(system_freezable_wq, &fusb251->routine_work, 0);
...
err_switch:
err_init:
	unregister_charger_device_notifier(fusb251->chg1_dev, &fusb251->chg1_nb);
err_getcharge:
err_tcpc:
err_id:
	i2c_set_clientdata(client, NULL);
	mutex_destroy(&fusb251->fusb251_io_mutex);
	kfree(fusb251);
	fusb251 = NULL;
...
static void fusb251_i2c_shutdown(struct i2c_client *client)
{
	...
	unregister_charger_device_notifier(fusb251->chg1_dev, &fusb251->chg1_nb);
	cancel_delayed_work_sync(&g_fusb251->routine_work);
	cancel_delayed_work_sync(&g_fusb251->delayed_work);
