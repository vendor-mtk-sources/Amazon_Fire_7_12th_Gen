...
#define MTK_WDT_MODE_DDR_RESERVE	(1U << 7)
...
int mtk_rgu_dram_reserved(int enable)
{
	...
	if (enable == 1) {
		...
		reg |= (MTK_WDT_MODE_DDR_RESERVE | WDT_MODE_KEY);
		iowrite32(reg, wdt_base + WDT_MODE);
	} else if (enable == 0) {
		/* disable ddr reserved mode, set reset mode, */
		reg = readl(wdt_base + WDT_MODE);
		reg &= (~MTK_WDT_MODE_DDR_RESERVE);
		reg |= WDT_MODE_KEY;
		iowrite32(reg, wdt_base + WDT_MODE);
	}
	pr_info("%s: MTK_WDT_MODE(0x%x)\n",
		__func__, readl(wdt_base + WDT_MODE));

	return 0;
}

/*
 * strict Canonical Boot Reason format as AOSP document since Android 9,
 * uses the following syntax: <reason>,<subreason>,<detail>…
 * refer to kBootReasonMap in system/core/bootstat/bootstat.cpp
 */
 ...
 */
	...
	if (cmd && (strcmp(cmd, "rpmbp-eng") == 0)) {
		...
		writel(__raw_readl(wdt_base + WDT_MODE) |
		WDT_MODE_AUTO_START | WDT_MODE_KEY, wdt_base + WDT_MODE);
	} else if (cmd && (strncmp(cmd, "recovery", 8) == 0)) {
...
#ifdef CONFIG_MTK_DDR_RESERVE_MODE
	if (cmd && (strstr(cmd, "ddr_reserve"))) {
