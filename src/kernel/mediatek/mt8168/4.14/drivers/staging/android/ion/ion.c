...
void user_ion_free_nolock(struct ion_client *client,
			  struct ion_handle *handle)
{
	bool valid_handle;
...
#ifdef CONFIG_MTK_IOMMU_V2
	dev = ion_iommu_heap_type(buffer) ?
		g_iommu_device : g_ion_device->dev.this_device;
#else
	dev = g_ion_device->dev.this_device;
#endif
		...
		if (heapid == id ||
		    ...
		    (id == ION_HEAP_TYPE_MULTIMEDIA &&
		     (type == ION_HEAP_TYPE_SYSTEM ||
		      type == (int)ION_HEAP_TYPE_MULTIMEDIA ||
		      type == (int)ION_HEAP_TYPE_MULTIMEDIA_SEC))) {
			client->dbg_hnd_cnt++;
				      ...
				      unsigned long arg))
{
	...
	ret = ion_init_sysfs();
	if (ret) {
		pr_info("ion: failed to add sysfs attributes.\n");
		goto err_sysfs;
	}
				      ...
				      int from_kernel)
{
	...
	return handle;
err:
	IONMSG("%s handle invalid, kernel:%d, handle=0x%lx, handle_id=%d\n",
	       __func__, from_kernel, (unsigned long)handle, user_handle);
	mutex_unlock(&client->lock);
	return ERR_PTR(-EINVAL);
