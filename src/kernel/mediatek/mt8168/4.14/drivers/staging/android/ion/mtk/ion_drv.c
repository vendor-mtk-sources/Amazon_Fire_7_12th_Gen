			       ...
			       int from_kernel)
{
	...
	ret = __cache_sync_by_range(client, sync_type, sync_va, sync_size,
				    from_kernel | ion_use_mapped_kva);
	if (ret < 0)
		goto err;
...
err:
	IONMSG(
	       "%s sync err:%d|k%d|hdl:%d-%p|addr:0x%p|iova:0x%llx|sz:%d|clt:%s\n",
			  ...
			  unsigned long arg, int from_kernel)
{
	...
	case ION_SYS_SET_CLIENT_NAME:
		...
		break;
	case ION_SYS_DMA_OP:
		IONMSG("ION_SYS_DMA_OP err!, please use ION_SYS_CACHE_SYNC\n");
		break;
	default:
		IONMSG(
