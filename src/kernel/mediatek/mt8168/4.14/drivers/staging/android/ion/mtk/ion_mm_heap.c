			    ...
			    ion_phys_addr_t *addr, size_t *len)
{
	...
	(CONFIG_MTK_IOMMU_PGTABLE_EXT > 32)
		...
		*addr = port_info.mva;
		...
		IONDBG(
		       "%d, iova mapping done, buffer:0x%p, port:%d, mva:0x%lx, fix:0x%lx, return:0x%x, cnt=%u, domain%d\n",
	...
	if (port_info.flags > 0) {
		...
		IONDBG("(region 0x%lx--0x%lx) (VA 0x%lx)\n",
			      ...
			      unsigned int fd)
{
	...
	if (buffer->heap->type != (unsigned int)ION_HEAP_TYPE_MULTIMEDIA &&
		...
		ION_DUMP(s, "skip heap %d in mm dump.\n",
			 (unsigned int)buffer->heap->type);
		return 0;
	}
		...
		for (n = rb_first(&dev->buffers); n; n = rb_next(n)) {
			...
			*buffer = rb_entry(n, struct ion_buffer, node);
			...
			if (((1 << heapid) & ION_HEAP_MULTIMEDIA_MASK) ||
				...
				bug_info = (struct ion_mm_buffer_info *)
					buffer->priv_virt;
				pdbg = &bug_info->dbg_info;
				if ((1 << heapid) & ION_HEAP_MULTIMEDIA_MASK) {
					mm_size += buffer->size;
