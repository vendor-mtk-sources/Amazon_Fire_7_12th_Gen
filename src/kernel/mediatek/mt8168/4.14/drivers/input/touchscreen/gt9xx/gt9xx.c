...
static unsigned char *modu_info = "";
struct i2c_client *i2c_connect_client;
static struct proc_dir_entry *gtp_firmware_proc;
			   ...
			   u16 addr, u8 *rxbuf, int len)
{
	...
	while (retry++ < 3) {
		...
		buf[0] = (u8)(addr >> 8);
		buf[1] = (u8)(addr & 0xFF);
		if (gtp_i2c_read(client, buf, len + 2) < 0)
			;

		memset(confirm_buf, 0xAB, 16);
		confirm_buf[0] = (u8)(addr >> 8);
		confirm_buf[1] = (u8)(addr & 0xFF);
		if (gtp_i2c_read(client, confirm_buf, len + 2) < 0)
			;
...
s32 gtp_send_cfg(struct i2c_client *client)
{
	...
	if (cfg->length < sizeof(cfg->data) && (cfg->length >= 0))
...
static int gtp_gesture_handler(struct goodix_ts_data *ts)
{
	...
	dev_info(&ts->client->dev, "0x814B = 0x%02X", doze_buf[2]);
	if ((doze_buf[2] == 'a') || (doze_buf[2] == 'b') ||
	    ...
	    (doze_buf[2] == 0xCC)) {
		if (ts->single_click_gesture_flag == 1) {
			dev_info(&ts->client->dev, "Gesture : First Effective Single Click.\n");
			...
			ts->single_click_gesture_flag = 0;
		} else {
			dev_info(&ts->client->dev, "Gesture : Multiple invalid click.\n");
		}
	} else {
		/*  clear 0x814B */
		doze_buf[2] = 0x00;
			 ...
			 u8 *key_value)
{
	...
	if ((8 * touch_num + 3) < sizeof(point_data) && (touch_num >= 0))
			      ...
			      struct goodix_point_t *points)
{
	...
	for (i = 0; i < ts->pdata->max_touch_id; i++) {
		if (touch_num && i == points->id) {
			...
			points->x = ts->pdata->abs_size_x - points->x;
			points->y = ts->pdata->abs_size_y - points->y;
...
static void gtp_work_func(struct goodix_ts_data *ts)
{
	...
	if ((ts->pdata->slide_wakeup && test_bit(DOZE_MODE, &ts->flags)) &&
	    (atomic_read(&ts->pdata->gesture_switch) == 1)) {
		ret =  gtp_gesture_handler(ts);
		dev_info(&ts->client->dev, "gtp_work_func:gesture mode handler\n");
		if (ret)
			dev_err(&ts->client->dev,
...
static int gtp_enter_doze(struct goodix_ts_data *ts)
{
	...
	dev_info(&ts->client->dev, "Entering gesture mode.");
	while (retry++ < 5) {
		...
		struct device_attribute *attr, char *buf)
{
	...
	return scnprintf(buf, PAGE_SIZE, "gt9xx:%x\n", fw_info->version);
}
...
static ssize_t gtp_cfg_version_read(char *buf)
{
	...
	if (ret < 0) {
		dev_err(&ts->client->dev, "Failed read config registers. Is it poweroff?\n");
		return 0;
	}
	*buf = temp_data[2];
	return 1;
...
static ssize_t gtp_fw_read_proc(struct file *file, char __user *page,
				    size_t size, loff_t *ppos)
{
	...
	ret = gtp_get_fw_info(data->client, &data->fw_info);
	if (ret < 0)
		dev_err(&data->client->dev, "Error read fw version\n");

	ptr = kzalloc(PROP_NAME_SIZE, GFP_KERNEL);
	if (!ptr) {
		dev_err(&data->client->dev, "Failed alloc memory for config\n");
		return -ENOMEM;
	}

	ret = gtp_cfg_version_read(&config_version);
	if (ret == 0) {
		dev_err(&data->client->dev, "Failed read touch config version.\n");
	}
	data_len += snprintf(ptr + data_len, PROP_NAME_SIZE - data_len,
			"goodix sid:0x%02X fw_ver:0x%02X cfg_ver:0x%02X vendor:%s\n",
			fw_info->sensor_id, fw_info->version,
			config_version, modu_info);
...
static const struct file_operations firmware_proc_ops = {
	.owner = THIS_MODULE,
	.read = gtp_fw_read_proc,
};
...
static ssize_t gtp_gesturemode_store(struct device *dev,
				 struct device_attribute *attr,
				 const char *buf, size_t count)
{
	...
	case 0:
		...
		atomic_set(&ts->pdata->gesture_ex_switch, 0);
		break;
	case 1:
		/* Enable gesture mode */
		atomic_set(&ts->pdata->gesture_ex_switch, 1);
		break;
	default:
		dev_err(dev, "Invalid value\n");
		return -EINVAL;
	}
	dev_info(dev, "gtp_gesturemode_store: gesture_ex_switch = %d\n",
			atomic_read(&ts->pdata->gesture_ex_switch));
	return count;
}
static ssize_t gtp_gesturemode_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int ret = 0;
	struct goodix_ts_data *ts = dev_get_drvdata(dev);

	dev_info(dev, "gtp_gesturemode_show: gesture_ex_switch = %d, gesture_switch = %d\n",
			atomic_read(&ts->pdata->gesture_ex_switch), atomic_read(&ts->pdata->gesture_switch));
	ret = scnprintf(buf, PAGE_SIZE, "GT9XX Gesture set Mode: %s, Now Mode: %s\n",
			atomic_read(&ts->pdata->gesture_ex_switch) ? "ON" : "OFF",
			atomic_read(&ts->pdata->gesture_switch) ? "ON" : "OFF");

	return ret;
}
static DEVICE_ATTR(gesture_wakeup, 0664, gtp_gesturemode_show, gtp_gesturemode_store);
...
static int gtp_create_file(struct goodix_ts_data *ts)
{
	...
	gtp_config_proc = proc_create(GT91XX_CONFIG_PROC_FILE, 0660,
	...
	gtp_firmware_proc = NULL;
	gtp_firmware_proc = proc_create(GT91XX_FIRMWARE_PROC_FILE, 0444,
				      NULL, &firmware_proc_ops);
	if (!gtp_firmware_proc)
		dev_err(&client->dev, "create_proc_entry %s failed\n",
			GT91XX_FIRMWARE_PROC_FILE);
	else
		dev_info(&client->dev, "create proc entry %s success\n",
			 GT91XX_FIRMWARE_PROC_FILE);
	...
	if (ts->touchscreen_link != NULL) {
		ret = sysfs_create_link(ts->touchscreen_link,
				...
				"link_input_dev");
		if (ret < 0)
			dev_err(&client->dev, "Failed to create sysfs link for input_dev %d\n", ret);
	}
...
exit_free_config_proc:
	...
	remove_proc_entry(GT91XX_FIRMWARE_PROC_FILE, gtp_firmware_proc);
	return -ENODEV;
}
...
s32 gtp_get_fw_info(struct i2c_client *client, struct goodix_fw_info *fw_info)
{
	...
	switch (fw_info->sensor_id) {
	case Wally_DOE:
		modu_info = "Wally_DOE\0";
	break;
	case Wally:
		modu_info = "Wally\0";
	break;
	...
	dev_info(&client->dev,
		"Sensor_id:(0x%02X) module info: %s\n",
		fw_info->sensor_id, modu_info);
	return ret;
}
...
static int gtp_pinctrl_init(struct goodix_ts_data *ts)
{
	...
	if (IS_ERR_OR_NULL(pinctrl->pinctrl)) {
		dev_err(&ts->client->dev, "No pinctrl found\n");
...
static int gtp_request_irq(struct goodix_ts_data *ts)
{
	...
	if (gpio_is_valid(ts->pdata->irq_gpio) || ts->client->irq > 0) {
		...
		dev_dbg(&ts->client->dev, "INT num %d, trigger type:%d\n",
...
static s8 gtp_request_input_dev(struct goodix_ts_data *ts)
{
	...
	if (!ts->pdata->type_a_report) {
		...
		dev_dbg(&ts->client->dev, "Use slot report protocol\n");
	} else {
		__set_bit(INPUT_PROP_DIRECT, ts->input_dev->propbit);
		__set_bit(BTN_TOUCH, ts->input_dev->keybit);
		dev_dbg(&ts->client->dev, "Use type A report protocol\n");
	}
...
int gtp_parse_dt_cfg(struct device *dev, u8 *cfg, int *cfg_len, u8 sid)
{
	...
	if (snprintf(cfg_name, sizeof(cfg_name), "goodix,cfg-group%d", sid) < 0)
		;
...
static int gtp_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (ts->pdata->slide_wakeup) {
		...
		atomic_set(&ts->pdata->gesture_ex_switch, 0);
		atomic_set(&ts->pdata->gesture_switch, 0);
...
exit_free_delay_work:
#ifdef CONFIG_TOUCHSCREEN_GT9XX_UPDATE
	if (ts->pdata->auto_update)
		cancel_delayed_work_sync(&ts->delayed_work);
#endif
exit_free_io_port:
	if (gpio_is_valid(ts->pdata->rst_gpio))
...
static void gtp_suspend(struct goodix_ts_data *ts)
{
	...
	gtp_esd_off(ts);
	ts->single_click_gesture_flag = 1;
	gtp_work_control_enable(ts, false);
	atomic_set(&ts->pdata->gesture_switch, atomic_read(&ts->pdata->gesture_ex_switch));
	if (ts->pdata->slide_wakeup && (atomic_read(&ts->pdata->gesture_switch) == 1)) {
		dev_info(&ts->client->dev, "gtp_suspend:enter suspend,enable gesture mode\n");
...
static int gtp_gesture_wakeup(struct goodix_ts_data *ts)
{
	...
	if (!retry)
		ret = -EIO;
	dev_dbg(&ts->client->dev, "gtp_gesture_wakeup:gesture mode,ret = %d\n", ret);
	clear_bit(DOZE_MODE, &ts->flags);
	return ret;
}
...
static void gtp_resume(struct goodix_ts_data *ts)
{
	...
	if ((ts->pdata->slide_wakeup && test_bit(DOZE_MODE, &ts->flags)) &&
	    (atomic_read(&ts->pdata->gesture_switch) == 1)) {
		dev_info(&ts->client->dev, "gtp_resume:enter resume,enable gesture mode\n");
