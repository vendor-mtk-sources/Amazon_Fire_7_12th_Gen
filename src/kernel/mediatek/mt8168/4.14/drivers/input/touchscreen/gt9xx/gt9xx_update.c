...
#define GOODIX9110_FIRMWARE_FILE_NAME	"goodix9110_firmware.bin"
		...
		u8 **path_p)
{
	...
	if (!(*path_p)) {
		if (sensor_id == Wally_DOE) {
			*path_p = GOODIX9110_FIRMWARE_FILE_NAME;
			dev_info(&client->dev, "sensor id = (0x%02X), gup_check_fw_name: %s",
				sensor_id, *path_p);
		} else {
...
void gtp_fw_check_func(struct work_struct *work)
{
...
{
	INIT_DELAYED_WORK(&ts->delayed_work, gtp_fw_check_func);
	schedule_delayed_work(&ts->delayed_work, msecs_to_jiffies(5000));
