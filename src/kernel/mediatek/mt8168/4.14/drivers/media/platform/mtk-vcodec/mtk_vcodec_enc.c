...
static void mtk_venc_queue_stop_enc_event(struct mtk_vcodec_ctx *ctx)
{
				 ...
				 struct v4l2_format *f)
{
	...
	if (ctx->state == MTK_STATE_FREE) {
		...
		if (ret) {
			mtk_v4l2_err("venc_if_init failed=%d, MTK_STATE_ABORT codec type=%x",
...
static int vb2ops_venc_start_streaming(struct vb2_queue *q, unsigned int count)
{
	...
	if (ret) {
		mtk_v4l2_err("venc_if_set_param failed=%d MTK_STATE_ABORT", ret);
		ctx->state = MTK_STATE_ABORT;
		mtk_venc_queue_error_event(ctx);
		goto err_set_param;
	}
...
static int mtk_venc_encode_header(void *priv)
{
	...
	if (enc_result.bs_va == 0) {
		...
		v4l2_m2m_buf_done(to_vb2_v4l2_buffer(dst_buf),
				  VB2_BUF_STATE_ERROR);
		mtk_v4l2_err("%s venc_if_encode failed=%d MTK_STATE_ABORT",
			__func__, ret);
		return -EINVAL;
	}
	...
	dst_vb2_v4l2->flags |= V4L2_BUF_FLAG_CSD;
	mtk_v4l2_debug(1, "dst_vb2_v4l2 flags=%x", dst_vb2_v4l2->flags);
...
static int mtk_venc_param_change(struct mtk_vcodec_ctx *ctx)
{
	...
	if (ret) {
		ctx->state = MTK_STATE_ABORT;
		mtk_venc_queue_error_event(ctx);
		mtk_v4l2_err("venc_if_set_param %d failed=%d MTK_STATE_ABORT",
...
static void m2mops_venc_job_abort(void *priv)
{
	...
	ctx->state = MTK_STATE_ABORT;
	mtk_v4l2_debug(0, "[%d] state=%d", ctx->id, ctx->state);
}
