...
long long div_64(long long a, long long b)
{
	...
	a = ((a < 0) ^ (b < 0)) ? (0LL - (long long)dividend)
				: (long long)dividend;
...
#if SHOW_ALGO_INFO
	pr_info("%s first_idx %d(%lld) prev_idx %d(%lld)", __func__, first_idx,
		hist->submit[first_idx], prev_idx, hist->submit[prev_idx]);
#endif
...
#if SHOW_ALGO_INFO
		pr_info("%s %lld -> MAX SUBMIT(%d)", __func__, next_submit,
			MAX_SUBMIT);
#endif
