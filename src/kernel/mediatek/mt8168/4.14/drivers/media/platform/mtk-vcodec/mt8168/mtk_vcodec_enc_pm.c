...
int mtk_vcodec_init_enc_pm(struct mtk_vcodec_dev *mtkdev)
{
	...
	pm->clk_MT_SCP_SYS_VEN =
		devm_clk_get(&pdev->dev, "MT_CG_VENC_JPGENC");
	if (IS_ERR(pm->clk_MT_SCP_SYS_VEN)) {
		mtk_v4l2_err("[VCODEC][ERROR] Unable to devm_clk_get MT_CG_VENC_JPGENC\n");
		ret = PTR_ERR(pm->clk_MT_SCP_SYS_VEN);
	}
...
void mtk_vcodec_enc_clock_on(struct mtk_vcodec_ctx *ctx, int core_id)
{
	...
	if (ret)
		mtk_v4l2_err("%s larbvenc fail %d", __func__, ret);

	ret = clk_prepare_enable(pm->clk_MT_CG_VENC);
	if (ret)
		mtk_v4l2_err("clk_prepare_enable CG_VENC fail %d", ret);

	ret = clk_prepare_enable(pm->clk_MT_SCP_SYS_VEN);
	if (ret)
		mtk_v4l2_err("clk_prepare_enable CG_VENC_JPGENC fail %d", ret);
#endif
}
...
void mtk_vcodec_enc_clock_off(struct mtk_vcodec_ctx *ctx, int core_id)
{
	...
	clk_disable_unprepare(pm->clk_MT_SCP_SYS_VEN);
