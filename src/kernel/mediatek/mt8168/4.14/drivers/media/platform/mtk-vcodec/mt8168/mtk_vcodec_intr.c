...
irqreturn_t mtk_vcodec_enc_irq_handler(int irq, void *priv)
{
	...
	/* mtee don't need kernel deal with irq */
	//if (ctx->enc_params.svp_mode != INHOUSE_TEE) {
		//mtk_v4l2_debug(1, "read irq,not mtee\n");
	...
	struct mtk_vcodec_dev *dev)
{
	...
	ret = devm_request_irq(&pdev->dev, dev->dec_irq[0],
		mtk_vcodec_dec_irq_handler, IRQF_SHARED, pdev->name, dev);
	if (ret) {
	...
	struct mtk_vcodec_dev *dev)
{
	...
	ret = devm_request_irq(&pdev->dev, dev->enc_irq,
		mtk_vcodec_enc_irq_handler, IRQF_SHARED, pdev->name, dev);
	if (ret) {
