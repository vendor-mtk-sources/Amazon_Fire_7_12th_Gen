		...
		struct page **pages, size_t size, dma_addr_t *handle)
{
	...
	if (!domain || !iovad || !handle) {
		dev_err(dev, "%s %d.domain %pK, iod %pK hnd %pK.\n",
			__func__, __LINE__, domain, iovad, handle);
		return;
	}
		...
		void (*flush_page)(struct device *, const void *, phys_addr_t))
{
	...
	if (!domain) {
		dev_info(dev, "%s: get domain fail\n", __func__);
		return NULL;
	}
				       ...
				       int prot, size_t size)
{
	...
	if (!domain || !iovad || !sg) {
		dev_err(dev, "%s %d.domain %pK, iod %pK sg %pK.\n",
			__func__, __LINE__, domain, iovad, sg);
		return;
	}
