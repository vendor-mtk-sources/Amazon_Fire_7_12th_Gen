...
static struct usb_extcon_info *extcon_info;
...
void chrdet_int_handler(void)
{
	...
	#ifdef CONFIG_EXTCON_USB_CHG
	if (upmu_get_rgs_chrdet())
		extcon_set_state_sync(extcon_info->edev, EXTCON_USB, true);
	else
		extcon_set_state_sync(extcon_info->edev, EXTCON_USB, false);
	#endif
}
...
static void init_extcon_work(struct work_struct *work)
{
	...
	extcon_info->edev = extcon_get_edev_by_phandle(extcon_info->dev, 0);
	if (IS_ERR(extcon_info->edev)) {
		schedule_delayed_work(&extcon_work,
			msecs_to_jiffies(50));
		return;
	}

	INIT_DELAYED_WORK(&extcon_info->wq_detcable, usb_extcon_detect_cable);
...
void PMIC_EINT_SETTING(struct platform_device *pdev)
{
	...
	if (of_property_read_bool(node, "extcon")) {
		extcon_info = devm_kzalloc(&pdev->dev,
				sizeof(*extcon_info), GFP_KERNEL);
		INIT_DELAYED_WORK(&extcon_work, init_extcon_work);
		extcon_info->dev = &pdev->dev;
		extcon_info->edev = extcon_get_edev_by_phandle(&pdev->dev, 0);
		if (IS_ERR(extcon_info->edev))
			schedule_delayed_work(&extcon_work,
				msecs_to_jiffies(50));
		else {
			INIT_DELAYED_WORK(&extcon_info->wq_detcable,
				usb_extcon_detect_cable);
			schedule_delayed_work(&extcon_info->wq_detcable,
