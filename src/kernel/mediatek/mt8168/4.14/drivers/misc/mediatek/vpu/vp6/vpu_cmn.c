...
inline struct vpu_core *vpu_get_vpu_core(unsigned int core)
{
...
void MET_Events_BusyRate_Trace(struct vpu_device *vpu_device,
			       unsigned int core)
{
...
int vpu_set_power(struct vpu_user *user, struct vpu_power *power)
{
	...
	if (core >= vpu_device->core_num || core == 0xFF) {
