...
static struct thermal_cooling_device *cl_dev_sysrst_mtkts_bts[MAX_MTK_COOLER_SYSRST];
...
static int sysrst_mtkts_bts_get_max_state(
struct thermal_cooling_device *cdev, unsigned long *state)
{
	*state = 1;
	return 0;
...
static int sysrst_mtkts_bts_get_cur_state(
struct thermal_cooling_device *cdev, unsigned long *state)
{
	*state = cl_dev_sysrst_state_mtkts_bts;
	return 0;
}

static int sysrst_mtkts_bts_set_cur_state(
struct thermal_cooling_device *cdev, unsigned long state)
{
	struct thermal_instance *instance;
	int trip_temp;

	cl_dev_sysrst_state_mtkts_bts = state;

	if (cl_dev_sysrst_state_mtkts_bts == 1) {

		list_for_each_entry(instance, &(cdev->thermal_instances), cdev_node) {
			if (instance->tz && instance->tz->ops && instance->tz->ops->get_trip_temp) {
				instance->tz->ops->get_trip_temp(instance->tz, instance->trip, &trip_temp);
				tscpu_printk("sysrst_%s_set_cur_state = 1\n", instance->tz->type);
				tscpu_printk("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n");
				tscpu_printk("*****************************************\n");
				tscpu_printk("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n");
				pr_err("[%s][%s]type:[%s] Thermal reset BTS, current temp=%d, trip=%d, trip_temp=%d\n",
...
static struct thermal_cooling_device_ops mtkts_bts_cooling_sysrst_ops = {
	.get_max_state = sysrst_mtkts_bts_get_max_state,
	.get_cur_state = sysrst_mtkts_bts_get_cur_state,
	.set_cur_state = sysrst_mtkts_bts_set_cur_state,
};
...
static int __init mtk_cooler_sysrst_init(void)
{
	...
	for (i = 0; i < MAX_MTK_COOLER_SYSRST; i++) {
		char bts_rst_name[20] = {0};
		sprintf(bts_rst_name, "mtkts_bts%d-sysrst", i);
		cl_dev_sysrst_mtkts_bts[i] = mtk_thermal_cooling_device_register(
				bts_rst_name, NULL,
				&mtkts_bts_cooling_sysrst_ops);
	}
...
static void __exit mtk_cooler_sysrst_exit(void)
{
	...
	for (i = 0; i < MAX_MTK_COOLER_SYSRST; i++) {
		if (cl_dev_sysrst_mtkts_bts[i]) {
			mtk_thermal_cooling_device_unregister(cl_dev_sysrst_mtkts_bts[i]);
			cl_dev_sysrst_mtkts_bts[i] = NULL;
		}
	}
