...
#define MIPITX_Write60384(slave_addr, write_addr, write_data)                  \
	{                                                                      \
		DDPDUMP("MIPITX_Write60384:0x%x,0x%x,0x%x\n", slave_addr,     \
		...
		mt_reg_sync_writel(0xFF, MIPITX_BASE + 0xC);                   \
									       \
		DDPDUMP("MIPI write data = 0x%x, read data = 0x%x\n",         \
			 write_data, INREG32(MIPITX_BASE));                    \
		if (INREG32(MIPITX_BASE) == write_data)                        \
			DDPDUMP("MIPI write success\n");                      \
		else                                                           \
			DDPDUMP("MIPI write fail\n");                         \
	}
	...
	struct cmdqRecStruct *cmdq, bool enable, unsigned int value)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		if (i < 0 || i >= DSI_INTERFACE_NUM) {
			DISPERR("[DSI]index out of range\n");
			return -1;
		}
		...
		if (enable) {
			DSI_OUTREG32(cmdq, &DSI_REG[i]->DSI_BIST_PATTERN,
				0x00FFFFFF);
			DSI_OUTREGBIT(cmdq, struct DSI_BIST_CON_REG,
				DSI_REG[i]->DSI_BIST_CON, rsv_07, 1);

			DSI_OUTREGBIT(cmdq, struct DSI_BIST_CON_REG,
				DSI_REG[i]->DSI_BIST_CON, BIST_LANE_NUM, value);
...
void DSI_PHY_hstx_voltage_setting(enum DISP_MODULE_ENUM module, char voltage_sel)
{
	unsigned int mipitx_voltage = 0;
	int i = DSI_MODULE_to_ID(module);

	/* RG_DSI_HSTX_LDO_REF_SEL	mipitx_voltage	Bit[9:6]
	 * 0.4				0x4444222A	8
	 * 0.42				0x4444226A	9
	 * 0.44				0x444422AA	A
	 * 0.46				0x444422EA	B
	...
	MIPITX_OUTREGBIT(DSI_PHY_REG[i]+MIPITX_VOLTAGE_SEL, FLD_RG_DSI_HSTX_LDO_REF_SEL, voltage_sel);
	mipitx_voltage = MIPITX_INREGBIT(DSI_PHY_REG[i]+MIPITX_VOLTAGE_SEL, FLD_RG_DSI_HSTX_LDO_REF_SEL);
	if (mipitx_voltage != voltage_sel)
		pr_err("[LCM] set MIPITX_VOLTAGE_SEL at 0x%x fail\n", voltage_sel);
}
	...
	struct cmdqRecStruct *cmdq, struct LCM_DSI_PARAMS *dsi_params)
{
	...
	if (dsi_params->hstx_voltage)
		DSI_PHY_hstx_voltage_setting(module, dsi_params->hstx_voltage);
	...
	unsigned char force_update)
{
	...
	unsigned long goto_addr = 0, mask_para = 0, set_para = 0;
	/* UINT32 fbPhysAddr, fbVirAddr; */
	struct DSI_T0_INS t0;
	/* DSI_T1_INS t1; */
	struct DSI_T2_INS t2;
	UINT32 index = 0;
	unsigned char data_id = 0, cmd = 0, count = 0;
	unsigned char *para_list = NULL;
	UINT32 d = 0;
	...
	struct LCM_DRIVER *lcm_drv)
{
	...
	if (module == DISP_MODULE_DSI0) {
		...
		utils->dsi_get_cmdq_V4 =
			primary_display_read_lcm_cmdq;
	...
	unsigned char force_update)
{
	...
	if (count > 1) {
		...
		goto_addr =
			(unsigned long)(&DSI_CMDQ_REG[dsi_i]->data[1].byte0);
		mask_para = (0xFFu << ((goto_addr & 0x3u) * 8));
		set_para = (cmd << ((goto_addr & 0x3u) * 8));
		...
		for (i = 0; i < count; i++) {
			goto_addr =
				(unsigned long)(&DSI_CMDQ_REG[dsi_i]->data[1].byte1)
				+ i;
			mask_para = (0xFFu << ((goto_addr & 0x3u) * 8));
...
int ddp_dsi_read_lcm_register_cmdq(enum DISP_MODULE_ENUM module,
