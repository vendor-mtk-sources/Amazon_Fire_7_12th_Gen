...
int cm_debug_log;
int cm_max_opp = CM_MGR_EMI_OPP;
int cm_min_opp;

#define cm_bw_printk(fmt, args...)   \
	do {                                    \
		if (cm_debug_log & 0x4) {                \
...
#ifdef CM_EMI_API
static unsigned int total_emi_count;
static unsigned int total_emi_avg_bw;
static ktime_t cm_emi_start, cm_emi_end;
#endif
/* 0: < 50us */
/* 1: 50~100us */
/* 2: 100~200us */
...
static void update_v2f(int update, int debug)
{
	...
	for (j = 0; j < CM_MGR_CPU_CLUSTER; j++) {
		for (i = 0; i < 16; i++) {
			...
			_f = mt_cpufreq_get_freq_by_idx(j,
				CM_CPUFREQ_MAX_OPP - i) / 1000;
			_v = mt_cpufreq_get_volt_by_idx(j,
				CM_CPUFREQ_MAX_OPP - i) / 1000;
...
static int cm_mgr_check_up_status(int level, int *cpu_ratio_idx)
{
	...
	for (i = 0; i < CM_MGR_CPU_CLUSTER; i++) {
		...
		cm_printk("%s cpu_power_up_array[%d] = %d\n", __func__,
			i, cpu_power_up_array[i]);
		cm_printk("%s v2f[%d] = %d\n", __func__, i, v2f[i]);
		cm_printk("%s cpu_power_total = %d\n", __func__,
				cpu_power_total);
	}
#else
	for (i = 0; i < CM_MGR_CPU_CLUSTER; i++) {
...
#ifdef DEBUG_CM_MGR
			cm_printk("#@# %s(%d) vcore_dram_opp %d->%d\n",
					__func__, __LINE__,
...
#ifdef DEBUG_CM_MGR
	cm_printk("#@# vcore_power_up %d < cpu_power_total %d\n",
...
#ifdef DEBUG_CM_MGR
			cm_printk("#@# %s(%d) vcore_dram_opp up %d->%d\n",
					__func__, __LINE__,
					vcore_dram_opp_cur, vcore_dram_opp);
#endif /* DEBUG_CM_MGR */
			cm_mgr_set_dram_level(
...
static int cm_mgr_check_down_status(int level, int *cpu_ratio_idx)
{
	...
	for (i = 0; i < CM_MGR_CPU_CLUSTER; i++) {
		...
		cm_printk("%s cpu_power_down_array[%d] = %d\n", __func__,
			i, cpu_power_down_array[i]);
		cm_printk("%s v2f[%d] = %d\n", __func__, i, v2f[i]);
		cm_printk("%s cpu_power_total = %d\n", __func__,
				cpu_power_total);
	}
...
#ifdef DEBUG_CM_MGR
	cm_printk("#@# vcore_power_down %d > cpu_power_total %d\n",
	...
	if ((vcore_power_down * vcore_power_ratio_down[idx]) >
		...
		if (++debounce_times_down >= debounce_times_down_adb[idx]) {
			...
			vcore_dram_opp = vcore_dram_opp_cur + 1;
#ifdef DEBUG_CM_MGR
			cm_printk("#@# %s(%d) vcore_dram_opp down %d->%d\n",
					__func__, __LINE__,
					vcore_dram_opp_cur, vcore_dram_opp);
#endif /* DEBUG_CM_MGR */
			cm_mgr_set_dram_level(
...
#ifdef CM_EMI_API
static void cm_emi_counter_pause(void)
{
	unsigned int value = readl(EMI_BMEN);

	/* BW monitor */
	writel(value | BUS_MON_PAUSE, EMI_BMEN);
}

static void cm_emi_counter_continue(void)
{
	unsigned int value = readl(EMI_BMEN);
...
static void cm_emi_counter_enable(const unsigned int enable)
{
	unsigned int value = readl(EMI_BMEN);
...
static void cm_emi_counter_reset(void)
{
	writel(EMI_BMEN_DEFAULT_VALUE, EMI_BMEN);
}

void cm_emi_mon_init(void)
{
	cm_emi_counter_enable(0);
	cm_emi_counter_reset();
	cm_emi_counter_enable(1);
}

void cm_emi_mon_restart(void)
{
	cm_emi_counter_enable(0);
	cm_emi_counter_continue();
	cm_emi_counter_enable(1);
}

static int cm_get_total_emi_bw(void)
{
	unsigned long long result;

	/* stop to counter for access amount for EMI */
	cm_emi_counter_pause();

	/* measure emi count of end time */
	cm_emi_end = ktime_get();

	/* delta time between start to end */
	result = ktime_to_us(ktime_sub(cm_emi_end, cm_emi_start));

	/* EMI count */
	total_emi_count = readl(EMI_WSCT);

	/* Count average bw */
	total_emi_avg_bw = (total_emi_count * 8) / (unsigned int) result;

	cm_bw_printk("%s total_emi_count = %d\n", __func__, total_emi_count);
	cm_bw_printk("%s total_emi_avg_bw = %d, diff time = %lld us\n",
		__func__, total_emi_avg_bw, result);

	/* restart emi monitor */
	cm_emi_mon_restart();

	/* measure emi count of start time */
	cm_emi_start = ktime_get();

	return total_emi_avg_bw;
}
#endif
	...
	if (spin_trylock_irqsave(&cm_mgr_lock, flags)) {
		...
		for_each_online_cpu(cpu) {
			...
			if (cpu >= CM_MGR_CPU_COUNT)
				break;
#if defined(CONFIG_MACH_MT8168)
			tmp = mt_cpufreq_get_cur_freq(
				cpu / CM_MGR_CPU_LIMIT) / 100000;
#else
			...
			cm_dprintk("%s cpufreq_cur_phy_freq = %d, cpu = %d\n",
					__func__, tmp, cpu);
			cm_dprintk("%s rel_load = %d, abs_load = %d\n",
					__func__, rel_load, abs_load);
			cm_dprintk("%s cm_rel_load = %d, cm_abs_load = %d\n",
					__func__,
					cm_mgr_rel_load,
					cm_mgr_abs_load);
...
#ifdef CM_EMI_API
		total_bw = cm_get_total_emi_bw() / 512;
#else
		...
		cm_bw_printk("%s total_bw unit = %d\n", __func__, total_bw);
		...
		for (i = 0; i < CM_MGR_CPU_CLUSTER; i++) {
			...
			cm_dprintk("%s cpu_opp_cur[%d] = %d\n",
					__func__, i, cpu_opp_cur[i]);
...
#ifdef DEBUG_CM_MGR
		if (cm_debug_log & 0x8)
			print_hex_dump(KERN_INFO, "cpu_opp_cur: ",
					DUMP_PREFIX_NONE, 16,
					1, &cpu_opp_cur[0],
					sizeof(cpu_opp_cur), 0);
#endif /* DEBUG_CM_MGR */
...
#ifdef DEBUG_CM_MGR
		if (cm_debug_log & 0x8) {
			print_hex_dump(KERN_INFO, "ratio: ",
					DUMP_PREFIX_NONE, 16,
...
void check_cm_mgr_status(unsigned int cluster, unsigned int freq)
{
...
#if defined(CONFIG_MACH_MT8168)
	prev_freq_idx[cluster] = CM_CPUFREQ_MAX_OPP - freq_idx;
#else
	...
	cm_dprintk("%s prev_freq_idx =%d, prev_freq = %d\n", __func__,
		prev_freq_idx[cluster], prev_freq[cluster]);
...
#ifdef CM_EMI_API
	seq_printf(m, "total_emi_count = %d\n", total_emi_count);
	seq_printf(m, "total_emi_avg_bw = %d\n", total_emi_avg_bw);
#endif
...
static int dbg_cm_mgr_proc_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "cm_debug_log %d\n", cm_debug_log);
	seq_printf(m, "cm_max_opp %d\n", cm_max_opp);
	seq_printf(m, "cm_min_opp %d\n", cm_min_opp);
		...
		const char __user *buffer, size_t count, loff_t *pos)
{
	...
	if (!strcmp(cmd, "cm_mgr_opp_enable")) {
		...
		cm_mgr_perf_set_force_status(cm_mgr_perf_force_enable);
	} else if (!strcmp(cmd, "cm_mgr_debug")) {
#if defined(CONFIG_MACH_MT8168)
		cm_debug_log = val_1;
#endif
	} else if (!strcmp(cmd, "cm_max_opp")) {
#if defined(CONFIG_MACH_MT8168)
		cm_max_opp = val_1;
#endif
	} else if (!strcmp(cmd, "cm_min_opp")) {
#if defined(CONFIG_MACH_MT8168)
		cm_min_opp = val_1;
#endif
	}
