...
#ifdef FREE_FB_BUFFER
static ssize_t mtkfb_fb_write(struct fb_info *info, const char __user *buf,
...
static ssize_t mtkfb_fb_read(struct fb_info *info, char __user *buf,
				size_t count, loff_t *ppos)
{
	unsigned long p = *ppos;
	u8 *buffer, *dst;
...
#ifdef FREE_FB_BUFFER
	.fb_read	= mtkfb_fb_read,
	.fb_write	= mtkfb_fb_write,
#endif
};
				...
				char *buf)
{
	...
	if (cnt < 0) {
		DISPERR("%s snprintf err, cabc_mode=%d\n", __func__, cabc_mode);
		return 0;
	}
...
static int mtkfb_probe(struct platform_device *pdev)
{
	...
	if (ddp_get_module_va(DISP_MODULE_MUTEX) == 0) {
		pr_info("dispsys not start probe\n");
		return -EPROBE_DEFER;
	}
