	...
	enum MTK_SMI_BWC_SCEN smi_scenario, int mmdvfs_step)
{
	...
	g_mmdvfs_adaptor->enable_vcore = !is_mmdvfs_scen_disabled();
...
{
	if (is_mmdvfs_scen_disabled() || g_mmdvfs_mgr->is_mmdvfs_start == 0) {
	...
	enum MTK_SMI_BWC_SCEN smi_scenario, int mmdvfs_step)
{
	...
	/* Update HW runtime option */
	g_mmdvfs_non_force_adaptor->enable_vcore = !is_mmdvfs_scen_disabled();
...
#if IS_ENABLED(CONFIG_MTK_SMI_EXT)
static struct clk *mmdvfs_clk_get_by_name(struct device *dev, char *clk_name)
{
	struct clk *clk_ptr = NULL;

	clk_ptr = clk_get(dev, clk_name);
	if (IS_ERR(clk_ptr)) {
...
static int mmdvfs_clks_init(struct device *dev)
{
	int i;
	...
	for (i = 0; i < g_mmdvfs_adaptor->mmdvfs_clk_hw_maps_num; i++) {
		...
		if (hw_map_ptr->config_method != MMDVFS_CLK_NONE) {
			...
			hw_map_ptr->clk_mux.ccf_handle = mmdvfs_clk_get_by_name(
				dev, hw_map_ptr->clk_mux.ccf_name);
		}
	}
...
void mmdvfs_init(struct device *dev, struct MTK_SMI_BWC_MM_INFO *info)
{
	...
	if (mmdvfs_get_mmdvfs_profile() == MMDVFS_PROFILE_LAF)
		g_mmdvfs_mgr->is_mmdvfs_start = 1;
	if (mmdvfs_get_mmdvfs_profile() == MMDVFS_PROFILE_AQU)
		g_mmdvfs_mgr->is_mmdvfs_start = 1;
#if IS_ENABLED(CONFIG_MTK_SMI_EXT)
	mmdvfs_clks_init(dev);
#endif
}
...
#elif defined(SMI_AQU)
	mmdvfs_profile_id = MMDVFS_PROFILE_AQU;
#endif
