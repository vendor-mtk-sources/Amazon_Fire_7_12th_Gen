...
static int edmc_set_isr(bool toApmcu);
...
unsigned int apu_conn_reg_read(u32 offset)
{
	return ioread32(apu_conn_base + offset);
}

static void apu_conn_reg_write(u32 value, u32 offset)
{
	iowrite32(value, apu_conn_base + offset);
}
...
static void edmc_start_power_off(struct work_struct *work)
{
	...
	if (edmc_is_fifo_empty() && edmc_is_queue_empty()) {
		...
		vpu_pdev = vpu_get_plat_device();
		if (vpu_pdev == NULL) {
			LOG_ERR("vpu_pdev is NULL please check!");
			mutex_unlock(&cmd_list_mutex);
			return;
		}

		vpu_set_internal_ram(vpu_pdev, false);
	}
	mutex_unlock(&cmd_list_mutex);
...
static int edmc_set_isr(bool toApmcu)
{
	...
	if (toApmcu) {
		/* For EDMA ISR to CPU */
		LOG_DBG("before: edma irq2vpu sel value(0x%x)\n",
			apu_conn_reg_read(0x24));

		/* APU_CONN_EDMA_IRQ2VPU_SEL */
		/* 0->cpu, 1->core (default) */
		apu_conn_reg_write(0, 0x24);
		LOG_DBG("after: edma irq2vpu sel value(0x%x)\n",
			apu_conn_reg_read(0x24));
	} else {
		/* For EDMA ISR to VPU */
		LOG_DBG("before: edma irq2vpu sel value(0x%x)\n",
		      ...
		      u32 write_ptr)
{
	...
	// Set ISR to APMCU
	vpu_pdev = vpu_get_plat_device();
	/* When power on apu, edmc enable setting will be clear.
	 * Must set edmc enable again
	 */
	if (vpu_pdev == NULL) {
		LOG_ERR("vpu_pdev is NULL please check!");
		goto exit;
	}
	vpu_set_internal_ram(vpu_pdev, true);
	edmc_set_isr(true);
...
static int edmc_enable_ion(bool enable)
{
	...
	if (enable) {
		for (i = 0; i < ARRAY_SIZE(regs); i++) {
			...
			value = value | (0x8<<4) | (0x4<<9); /*enable ion*/
...
static int edmc_probe(struct platform_device *pdev)
{
	...
	apu_conn_base = devm_ioremap_resource(&pdev->dev, mem);
	if (IS_ERR((void *)apu_conn_base)) {
		LOG_ERR("cannot get ioremap 0\n");
		return -ENOENT;
	}
	...
	lp = (struct mtk_edmc_drvdata *)
				kmalloc(sizeof(struct mtk_edmc_drvdata),
					GFP_KERNEL);
	if (!lp) {
		LOG_ERR("Could not allocate mtk_edmc device\n");
		return -ENOMEM;
	}
	...
	if (!request_mem_region(lp->mem_start, lp->mem_end - lp->mem_start + 1,
				DRIVER_NAME)) {
		LOG_ERR("Couldn't lock memory region at %p\n",
				(void *) lp->mem_start);
		rc = -EBUSY;
		return 0;
	}
	...
	if (!lp->base_addr) {
		LOG_ERR("mtk_edmc: Could not allocate iomem\n");
		rc = -EIO;
		return 0;
	}
	...
	dev_info(dev, "platform_get_resource irq: 0x%lx\n",
	...
	if (rc) {
		LOG_ERR("mtk_edmc: Could not allocate interrupt %d.\n",
			lp->irq);
		return 0;
	}
#endif
...
static int edmcctl_init(void)
{
	...
	if (majorNumber < 0) {
		LOG_WRN("EDMC failed to register a major number\n");
		return majorNumber;
	}
	...
	if (IS_ERR(edmcctlClass)) {
		...
		LOG_WRN("ailed to register device class\n");
	...
	if (IS_ERR(edmcctlDevice)) {
		...
		unregister_chrdev(majorNumber, DEVICE_NAME);
		LOG_WRN("ailed to create the device\n");
