...
/*
 * It is used for direct mapping dma region because those dma_unmap_xxxx() in
 * dma-mapping.h are required the dma_map_xxxx() to be called firstly. For the
 * direct mapping dma region, dma_unmap_xxxx() will not be able to pass the
 * dma debug check if CONFIG_DMA_API_DEBUG is opened.
 */
void m4u_dma_unmap_region_nocheck(struct device *dev, dma_addr_t addr,
				  size_t size, enum dma_data_direction dir,
				  unsigned long attrs)
{
	...
	dev_dbg(dev, "%s: iova 0x%lx, size 0x%x\n", __func__,
		(unsigned long)addr, (unsigned int)size);
}
EXPORT_SYMBOL(m4u_dma_unmap_region_nocheck);
