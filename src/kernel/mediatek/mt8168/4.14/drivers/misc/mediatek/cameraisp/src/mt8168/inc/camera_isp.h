...
struct ISP_IRQ_TIME_STRUCT {
	unsigned int
		tLastSig_sec; /* time stamp of the latest occurring signal */
	unsigned int
		tLastSig_usec; /* time stamp of the latest occurring signal */
	...
	unsigned int
		tMark2WaitSig_usec; /* time period from marking a signal to user
				     ...
				     */
	unsigned int tLastSig2GetSig_sec; /* time period from latest occurring
					   * signal
					   */
	/*to user try to wait and get the signal */
	unsigned int tLastSig2GetSig_usec; /* time period from latest occurring
					    * signal
					    */
	/*to user try to wait and get the signal */
...
struct ISP_EIS_META_STRUCT {
	...
	unsigned int tLastSOF2P1done_usec;
					/* time stamp of the last closest
					 * occurring sof signal for pass1
					 * done
					 */
};
...
struct ISP_RT_RING_BUF_INFO_STRUCT {
	...
	unsigned int total_count; /* total buffer number.Include
				   * Filled and empty
				   */
...
enum ISP_ED_BUFQUE_CTRL_ENUM {
	...
	ISP_ED_BUFQUE_CTRL_WAKE_WAITFRAME, /* 5,wake all slept users to check
...
/* this must be equal to ISP_DRV_ED_BUFQUE_STRUCT in isp_drv.h */
...
enum ISP_CMD_ENUM {
	...
	ISP_CMD_GET_DMA_ERR,
	ISP_CMD_GET_INT_ERR,
	ISP_CMD_GET_DROP_FRAME, /* dump current frame informaiton, 1 for drop
				 * frmae, 2 for last working frame
				 */
#ifdef T_STAMP_2_0
	ISP_CMD_SET_FPS,
#endif
...
#define COMPAT_ISP_UPDATE_MAGNUM                                               \
	_IOW(ISP_MAGIC, ISP_CMD_UPDATE_MAGNUM, compat_uint_t)

#define COMPAT_ISP_WAIT_IRQ                                                    \
	_IOW(ISP_MAGIC, ISP_CMD_WAIT_IRQ, struct compat_ISP_WAIT_IRQ_STRUCT)
