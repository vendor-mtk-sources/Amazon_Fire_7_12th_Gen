...
static struct stCAM_CAL_CMD_INFO_STRUCT
		g_camCalDrvInfo[CAM_CAL_I2C_MAX_SENSOR];
...
static int EEPROM_set_i2c_bus(unsigned int deviceID)
{
	...
	PK_DBG("end! deviceID=%d g_curBusIdx=%d g_Current_Client=%p\n",
...
static int EEPROM_get_cmd_info(
	unsigned int sensorID, struct stCAM_CAL_CMD_INFO_STRUCT *cmdInfo)
{
	...
	if (sensorID == GC2375HMIPI_CXT_REAR_SENSOR_ID ||
	    ...
	    sensorID == GC02M1_QUARTZ_CXT_REAR_SENSOR_ID ||
	    sensorID == GC02M1_QUARTZ_CXT_FRONT_SENSOR_ID ||
	    sensorID == GC02M1_QUARTZ_LCE_REAR_SENSOR_ID ||
	    sensorID == GC02M1_QUARTZ_LCE_FRONT_SENSOR_ID) {
		use_idme = true;
	}
	...
	if (i == CAM_CAL_I2C_MAX_SENSOR) {
		for (i = 0; i < CAM_CAL_I2C_MAX_SENSOR; i++) {
			...
			if (g_camCalDrvInfo[i].sensorID == 0) {
				PK_DBG(
				    "g_camCalDrvInfo[%d].sensorID == 0, start get_cmd_info!\n",
				    i);
...
static const struct i2c_device_id EEPROM_HW_i2c_id[] = {
			{CAM_CAL_DRV_NAME, 0},
			{} };
static const struct i2c_device_id EEPROM_HW_i2c_id2[] = {
			{CAM_CAL_I2C_DEV2_NAME, 0},
			{} };
static const struct i2c_device_id EEPROM_HW_i2c_id3[] = {
			{CAM_CAL_I2C_DEV3_NAME, 0},
			{} };
	...
	struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case COMPAT_CAM_CALIOC_G_READ: {
		...
		if (err != 0)
			PK_ERR(
			  "compat_put_acdk_sensor_getinfo_struct failed\n");
...
#endif
{
	...
	if (_IOC_DIR(a_u4Command) != _IOC_NONE) {
		...
		if (copy_from_user((u8 *)pu1Params,
			(u8 *)ptempbuf->pu1Params, ptempbuf->u4Length)) {
			kfree(pBuff);
			kfree(pu1Params);
			PK_DBG("ioctl copy from user failed\n");
			return -EFAULT;
	...
	if (ptempbuf->sensorID == GC2375HMIPI_CXT_REAR_SENSOR_ID ||
	    ...
	    ptempbuf->sensorID == GC02M1_QUARTZ_CXT_REAR_SENSOR_ID ||
	    ptempbuf->sensorID == GC02M1_QUARTZ_CXT_FRONT_SENSOR_ID ||
	    ptempbuf->sensorID == GC02M1_QUARTZ_LCE_REAR_SENSOR_ID ||
	    ptempbuf->sensorID == GC02M1_QUARTZ_LCE_FRONT_SENSOR_ID) {
		use_idme = true;
	}
	...
	case CAM_CALIOC_G_READ:
		...
		if (pcmdInf != NULL) {
			if (pcmdInf->readCMDFunc != NULL)
				i4RetValue = pcmdInf->readCMDFunc(
					pcmdInf->client,
					ptempbuf->u4Offset,
					pu1Params,
					ptempbuf->u4Length);
	...
	if (_IOC_READ & _IOC_DIR(a_u4Command)) {
		...
		if (copy_to_user((u8 __user *) ptempbuf->pu1Params,
			(u8 *)pu1Params, ptempbuf->u4Length)) {
			kfree(pBuff);
			kfree(pu1Params);
...
static inline int EEPROM_chrdev_register(void)
{
	...
	device = device_create(
		g_drvClass, NULL, g_devNum, NULL, CAM_CAL_DRV_NAME);
	PK_DBG("End\n");
