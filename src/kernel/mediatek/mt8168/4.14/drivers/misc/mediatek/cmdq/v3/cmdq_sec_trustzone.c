...
static void *g_iwc_wsm_buf_ex;
static void *g_iwc_wsm_buf_ex2;
...
void cmdq_sec_alloc_iwc_buffer(void)
{
	...
	if (g_iwc_sec_buffer == NULL)
		CMDQ_ERR("allocate iwcCmdqMessage_t fail size:%zu\n",
			sizeof(struct iwcCmdqMessage_t));

	g_iwc_wsm_buf_ex = kmalloc(sizeof(struct iwcCmdqMessageEx_t),
			GFP_KERNEL);
	if (g_iwc_wsm_buf_ex == NULL)
		CMDQ_ERR("allocate iwcCmdqMessageEx_t fail size:%zu\n",
			sizeof(struct iwcCmdqMessageEx_t));

	g_iwc_wsm_buf_ex2 = kmalloc(sizeof(struct iwcCmdqMessageEx2_t),
			GFP_KERNEL);
	if (g_iwc_wsm_buf_ex2 == NULL)
		CMDQ_ERR("allocate iwcCmdqMessageEx2_t fail size:%zu\n",
			sizeof(struct iwcCmdqMessageEx2_t));


	CMDQ_ERR("allocate iwc buffer success:%p\n", g_iwc_sec_buffer);
	CMDQ_ERR("allocate iwc buffer success:%p\n", g_iwc_wsm_buf_ex);
	CMDQ_ERR("allocate iwc buffer success:%p\n", g_iwc_wsm_buf_ex2);
}
...
void cmdq_sec_setup_tee_context(struct cmdq_sec_tee_context *tee)
{
	...
	if (snprintf(tee->uuid, sizeof(tee->uuid), "%s",
			"5c071864-505d-11e4-9e35-164230d1df67") <= 0)
		CMDQ_ERR("assign cmdq uuid fail\n");
}
	...
	void **wsm_buf_ex2, u32 size_ex2)
{
	if (!wsm_buffer || !wsm_buf_ex || !wsm_buf_ex2 ||
		(size != sizeof(struct iwcCmdqMessage_t)) ||
		(size_ex != sizeof(struct iwcCmdqMessageEx_t)) ||
		(size_ex2 != sizeof(struct iwcCmdqMessageEx2_t))) {
		CMDQ_ERR("%s invalid param[%p %p %p %d %d %d]\n",
			__func__,
			wsm_buffer,
			wsm_buf_ex,
			wsm_buf_ex2,
			...
			size_ex,
			size_ex2);
		CMDQ_ERR("required size[%zu %zu %zu]\n",
			sizeof(struct iwcCmdqMessage_t),
			sizeof(struct iwcCmdqMessageEx_t),
			sizeof(struct iwcCmdqMessageEx2_t));
		return -EINVAL;
	...
	if (!tee->share_memory) {
		CMDQ_ERR("share_memory kmalloc failed, size[%d]\n",
			size);
		return -ENOMEM;
	}
	...
	if (g_iwc_wsm_buf_ex == NULL)
		...
		tee->wsm_buf_ex = g_iwc_wsm_buf_ex;
	if (!tee->wsm_buf_ex) {
		CMDQ_ERR("wsm_buf_ex kmalloc failed, size[%d]\n",
			size_ex);
		return -ENOMEM;
	}
	...
	if (g_iwc_wsm_buf_ex2 == NULL)
		...
		tee->wsm_buf_ex2 = g_iwc_wsm_buf_ex2;
	if (!tee->wsm_buf_ex2) {
		CMDQ_ERR("wsm_buf_ex2 kmalloc failed, size[%d]\n",
			size_ex2);
		return -ENOMEM;
	}
