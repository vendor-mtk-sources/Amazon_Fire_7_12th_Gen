...
struct disp_iommu_device *disp_get_iommu_dev(void)
{
	...
	np = of_find_compatible_node(NULL, NULL, "mediatek,mtk-pseudo-m4u");
	if (np == NULL) {
		DDPERR("DT,mediatek,mtk-pseudo-m4u is not found\n");
	} else {
...
static int disp_probe_1(void)
{
	...
	mipi_tx_reg = (unsigned long)ioremap(0x1401A000, 0x1000);
...
struct platform_device *drv_dev;
static int disp_probe(struct platform_device *pdev)
{
	...
	if (!is_SMI_exist()) {
		pr_notice("%s SMI not start probe\n", __func__);
		return -EPROBE_DEFER;
	}
