...
static inline void svs_init01_isr_handler(struct mtk_svs *svs)
{
	...
	/*
	 * always dump the Vproc DC offset here,
	 * that is the gap data we want to track & statistic
	 */
...
static int svs_mt8168_efuse_parsing(struct mtk_svs *svs)
{
	const struct svs_platform *svsp = svs->platform;
	struct picachu_data pi_data;
	struct svs_bank *svsb;
	u32 idx, i;
	int ret;
	...
	if (svsp->fake_efuse) {
		...
		svs->efuse[1] = 0x20f751c6;
		svs->efuse[2] = 0x00160000;
	} else {
		ret = get_picachu_calibrate_data(PICACHU_CLUSTER_L_VMIN_OPP0,
						 &pi_data);
		if (!ret && pi_data.valid) {
			svs->efuse[1] = (svs->efuse[1] & ~0xff) | pi_data.BDES;
			svs->efuse[1] = (svs->efuse[1] & ~0xff00) |
					(pi_data.MDES << 8);
			svs->efuse[2] = (svs->efuse[2] & ~0xff0000) |
					(pi_data.MTDES << 16);
		}
	}
	...
	for (idx = 0; idx < svsp->bank_num; idx++) {
		...
		switch (svsb->sw_id) {
		case SVS_CPU_LITTLE:
			svsb->bdes = svs->efuse[1] & 0xff;
			svsb->mdes = (svs->efuse[1] >> 8) & 0xff;
			svsb->dcbdet = (svs->efuse[1] >> 16) & 0xff;
			svsb->dcmdet = (svs->efuse[1] >> 24) & 0xff;
			svsb->mtdes  = (svs->efuse[2] >> 16) & 0xff;
			break;
		default:
...
static int svs_create_svs_procfs(struct mtk_svs *svs)
{
	...
	for (i = 0; i < ARRAY_SIZE(svs_entries); i++) {
		if (!proc_create_data(svs_entries[i].name, 0660,
	...
	for (idx = 0; idx < svsp->bank_num; idx++) {
		...
		for (i = 0; i < ARRAY_SIZE(bank_entries); i++) {
			if (!proc_create_data(bank_entries[i].name, 0660,
...
static struct svs_bank_ops svs_mt8168_banks_ops = {
	.set_freqs_pct	= svs_set_freqs_pct_v2,
	.get_vops	= svs_get_vops_v2,
};

static struct svs_bank svs_mt8168_banks[1] = {
	{
		.of_compatible		= "mediatek,mt8168-svs-cpu-little",
		.sw_id			= SVS_CPU_LITTLE,
		.hw_id			= 0,
		.ops			= &svs_mt8168_banks_ops,
		.zone_name		= "tzts1",
...
static const struct svs_platform svs_mt8168_platform = {
	.name		= "mt8168-svs",
	.banks		= svs_mt8168_banks,
	.efuse_parsing	= svs_mt8168_efuse_parsing,
	.regs		= svs_regs_v2,
	.fake_efuse	= false,
	.bank_num	= 1,
	.efuse_num	= 8,
