...
static unsigned char No_DummyRead;
...
#define dramc_rsv_aee_pr_err(string, args...) do {\
...
void dump_tx_log(unsigned int res)
{
	...
	case TX_TIMEOUT_MRR_ENABLE:
		pr_debug("[DRAMC] TX MRR enable timeout\n");
		break;
	case TX_TIMEOUT_MRR_DISABLE:
		pr_debug("[DRAMC] TX MRR disable timeout\n");
		break;
	case TX_TIMEOUT_DQSOSC:
		pr_debug("[DRAMC] TX DQS OSC timeout\n");
		break;
	case TX_TIMEOUT_DDRPHY:
		pr_debug("[DRAMC] TX DDRPHY update timeout\n");
		break;
	case TX_FAIL_DATA_RATE:
		pr_debug("[DRAMC] TX read data rate fail\n");
		break;
	case TX_FAIL_VARIATION:
		pr_debug("[DRAMC] TX variation is too large\n");
		break;
	default:
		pr_debug("[DRAMC] TX unknown error\n");
		break;
	}
}
#endif
...
unsigned int get_dram_data_rate(void)
{
	...
	if ((u4DataRate > 1100) && (u4DataRate <= 1200))
		return 1200;

	if ((u4DataRate > 1200) && (u4DataRate <= 1333))
		return 1333;

	if ((u4DataRate > 1333) && (u4DataRate <= 1600))
		return 1600;

	if ((u4DataRate > 1600) && (u4DataRate <= 2400))
		return 2400;

	if ((u4DataRate > 2400) && (u4DataRate <= 2666))
		return 2666;

	if ((u4DataRate > 2666) && (u4DataRate <= 3200))
		return 3200;

	/* pr_info("[DRAMC] PCW=0x%X, u4PREDIV=%d, */
...
static int Binning_DRAM_complex_mem_test(void)
{
...
static ssize_t dram_cur_freq_show(struct device_driver *driver, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%u\n",
	get_dram_data_rate()*1000000);
}
...
static DRIVER_ATTR_RO(dram_cur_freq);
...
#endif /* #if defined(SW_ZQCS) || defined(SW_TX_TRACKING) */
...
static int dram_probe(struct platform_device *pdev)
{
	...
	SYS_TIMER_BASE_ADDR = base_temp[8];
	/* SLEEP_BASE_ADDR = base_temp[8]; */
	...
	DRAM_TYPE = (readl(PDEF_DRAMC0_CHA_REG_010) >> 10) & 0x7;
	pr_info("[DRAMC] dram type =%d\n", DRAM_TYPE);

	if (DRAM_TYPE > TYPE_LPDDR4X) {
		pr_err("[DRAMC] dram type error !!\n");
		return -1;
	}

#ifdef EMI_READY
	CH_NUM = get_ch_num();
	pr_info("[DRAMC] Channel num =%d\n", CH_NUM);

	if (!CH_NUM) {
		pr_err("[DRAMC] channel number error !!\n");
		return -1;
	}
#else
	CH_NUM = 2;
	...
	if ((DRAM_TYPE == TYPE_LPDDR4) || (DRAM_TYPE == TYPE_LPDDR4X)) {
		...
		if (mod_timer(&zqcs_timer, jiffies + msecs_to_jiffies(280)))
			pr_info("[DRAMC] Error in ZQCS mod_timer\n");
	}
#endif
	...
	ret = driver_create_file(pdev->dev.driver,
	&driver_attr_dram_cur_freq);
	if (ret) {
		pr_warn("fail to create dram_cur_freq sysfs files\n");
		return ret;
	}
...
MODULE_DESCRIPTION("MediaTek DRAMC v0.1");
