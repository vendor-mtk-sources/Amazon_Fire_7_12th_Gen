...
void do_lp_write(unsigned char cmd, unsigned char *para,
	unsigned int count)
{
...
unsigned char do_lp_read(unsigned char cmd, unsigned int count)
{
	unsigned char reg_value = 0x00;
...
void do_vdo_dsi_write(unsigned char cmd, unsigned char count, unsigned char *para_list)
{
	struct ddp_lcm_write_cmd_table write_table = {0x00};
	write_table.cmd = cmd;
	write_table.count = count;
	memcpy(write_table.para_list, para_list, count);
...
static void process_dbg_opt(const char *opt)
{
	...
	if (strncmp(opt, "regr:", 5) == 0) {
		...
		DISPINFO("enter vdo_dsi_read!\n");
		reg_value = do_vdo_dsi_read(cmd);

		snprintf(buf, 50, "read reg:0x%x,value: 0x%x\n", cmd, reg_value);
		return;
	} else if (strncmp(opt, "vdo_dsi_write:", 14) == 0) {
		int cmd;
		int para_cnt, i;
		char para[15] = {0};
		static char fmt[256] = {0};
		static const char temp[] = "vdo_dsi_write:0x%x";

		DISPINFO("enter vdo_dsi_write!\n");
		...
		do_vdo_dsi_write(cmd, para_cnt, para);
		return;
		...
		char para[15] = {0};
		static char fmt[256] = {0};
		static const char temp[] = "set_lp_cmd:0x%x";

		DISPMSG("Enter set_lp_cmd!\n");
		...
		DISPMSG("cmd=0x%x, para_cnt=%d\n", cmd, para_cnt);
		for (i = 0; i < para_cnt; i++)
			DISPMSG("para[%d] = 0x%x\n", i, para[i]);

		do_lp_write(cmd, para, para_cnt);
...
#define SPRINTF_DEV_ATTR(fmt, arg...) \
		...
		if (count > 10) {
			DISPERR("Count should no more than 10!\n");
			return;
		}

		DISPMSG("cmd=0x%x, count=%d\n", cmd, count);

		ret = do_lp_read(cmd, count);
