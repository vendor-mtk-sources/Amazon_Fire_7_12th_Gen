...
static int freqhopping_slt_test_proc_read(struct seq_file *m, void *v)
{
	 ...
	 */
	pr_info("%s()\n", __func__);
	pr_info("fhctl slt result info\n");
	pr_info("return 0 => [SLT]FHCTL Test Pass!\n");
	pr_info("return -1 => [SLT]FHCTL Test Fail!\n");
	pr_info("return (result != 0 && result != -1) ");
	pr_info("=> [SLT]FHCTL Test Untested!\n");
...
{
	if (!g_p_fh_hal_drv) {
		pr_info("%s: g_p_fh_hal_drv is uninitialized."
			, __func__);
		return 1;
	}
...
static int freqhopping_debug_proc_init(void)
{
	...
	if (prdumpregentry == NULL) {
		pr_info("%s: failed to create "PROC_FH_(dumpregs)
			, __func__);
		return -EINVAL;
	}
#ifdef FH_FULL_PROC_INTERFACE_SUPPORT
	...
	if (prStatusentry == NULL) {
		pr_info("%s: failed to create "PROC_FH_(status)
			, __func__);
		return -EINVAL;
	}
	...
	if (prDebugentry == NULL) {
		pr_info("%s: failed to create "PROC_FH_(freqhopping_debug)
			, __func__);
		return -EINVAL;
	}
#endif
	...
	if (prslttestentry == NULL) {
		pr_info("%s: failed to create /proc/freqhopping/slt_test"
			, __func__);
		return -EINVAL;
	}
#endif /* SUPPORT_SLT_TEST */
	...
	, unsigned long vco_freq, unsigned int enable)
{
	...
	if ((g_p_fh_hal_drv->mt_fh_get_init()) == 0) {
		pr_info("Not init yet, init first.");
		return 1;
	}
