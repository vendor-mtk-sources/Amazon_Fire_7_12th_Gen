...
#include <mrdump.h>
#ifdef CONFIG_CHECK_MTK8168_SRAM_STATUS
#include <linux/io.h>
#endif
...
#ifdef CONFIG_CHECK_MTK8168_SRAM_STATUS
static unsigned char __iomem *g_sram_status_vreg1;
static unsigned char __iomem *g_sram_status_vreg2;
#endif
...
#ifdef CONFIG_CHECK_MTK8168_SRAM_STATUS
static void sram_status_log(void)
{
	if (!g_sram_status_vreg1 || !g_sram_status_vreg2) {
		pr_err("%s: remap io failed!\n", __func__);
		return;
	}
	pr_err("CPU SRAM status:0x1020007c=0x%08x, 0x10200080=0x%08x\n",
			readl(g_sram_status_vreg1), readl(g_sram_status_vreg2));
}
#endif
...
static int __init mrdump_panic_init(void)
{
	...
	mrdump_wdt_init();
#ifdef CONFIG_CHECK_MTK8168_SRAM_STATUS
	/* There is might-sleep in ioremap, should avoid using in atomic context.
	 * 0x1020007c & 0x10200080 are mt8168 private registers, used for dump
	 * bitflip status in L2 cache.
	 */
	g_sram_status_vreg1 = ioremap(0x1020007c, 4);
	g_sram_status_vreg2 = ioremap(0x10200080, 4);
