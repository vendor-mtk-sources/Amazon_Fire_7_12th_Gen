...
static int mtk_devfreq_gov_func(struct devfreq *df,
				    unsigned long *freq)
{
	/*
...
static int mtk_devfreq_gov_handler(struct devfreq *devfreq,
...
static struct devfreq_governor mtk_devfreq_gov = {
	.name = "ahlahuagua",
	.get_target_freq = mtk_devfreq_gov_func,
	.event_handler = mtk_devfreq_gov_handler,
};
...
static void mtk_devfreq_set_cur_freq(unsigned long cur_freq)
{
	struct kbase_device *kbdev;

	kbdev = (struct kbase_device *)mtk_get_mali_dev();
	if (!kbdev)
		return;
...
int mtk_devfreq_target(struct device *dev, unsigned long *target_freq, u32 flags)
{
	...
	mt_gpufreq_devfreq_target(*target_freq / 1000);

	dev_info(dev, "target_freq (%d -> %d)\n",
		*target_freq, kbdev->current_nominal_freq);
...
int kbase_devfreq_init(struct kbase_device *kbdev)
{
	...
	err = devfreq_add_governor(&mtk_devfreq_gov);
	if (err) {
		dev_info(kbdev->dev, "add mtk governor failed\n");
		return err;
	}
	...
	mtk_devfreq_set_cur_freq_fp = mtk_devfreq_set_cur_freq;
...
void kbase_devfreq_term(struct kbase_device *kbdev)
{
	...
	devfreq_remove_governor(&mtk_devfreq_gov);
}
