...
#define IDME_CAP_ID	"/idme/clk_buf_capid"
...
static void clk_buf_ctrl_internal(enum clk_buf_id id, bool onoff)
{
	...
	case CLK_BUF_UFS:
		...
		break;
	default:
		pr_notice("%s: id=%d isn't supported\n", __func__, id);
		break;
	}
...
int clk_buf_mode_set(enum clk_buf_id id, int mode)
{
	int ret = 0;
	int reg = PMIC_DCXO_CW00_SET;
	int reg_enbb = PMIC_DCXO_CW00_CLR;

	if (mode) {
		reg = PMIC_DCXO_CW00_CLR;
		reg_enbb = PMIC_DCXO_CW00_SET;
	}

	switch (id) {
	case CLK_BUF_BB_MD:
		pmic_config_interface(reg, 1, PMIC_XO_EXTBUF1_MODE_MASK,
				      PMIC_XO_EXTBUF1_MODE_SHIFT);
		pmic_config_interface(reg_enbb, 1,
					      PMIC_XO_ENBB_MAN_MASK,
					      PMIC_XO_ENBB_MAN_SHIFT);
		break;
	case CLK_BUF_CONN:
		pmic_config_interface(reg, 1, PMIC_XO_EXTBUF2_MODE_MASK,
				      PMIC_XO_EXTBUF2_MODE_SHIFT);
		break;
	case CLK_BUF_NFC:
		pmic_config_interface(reg, 1, PMIC_XO_EXTBUF3_MODE_MASK,
				      PMIC_XO_EXTBUF3_MODE_SHIFT);
		break;
	case CLK_BUF_RF:
		pmic_config_interface(reg, 1, PMIC_XO_EXTBUF4_MODE_MASK,
				      PMIC_XO_EXTBUF4_MODE_SHIFT);
		break;
	default:
		ret = -5;
...
EXPORT_SYMBOL(clk_buf_mode_set);
...
bool clk_buf_ctrl(enum clk_buf_id id, bool onoff)
{
	...
	case CLK_BUF_CONN:
		...
		if (!(pwrap_dcxo_en_flag & DCXO_CONN_ENABLE)) {
			ret = -1;
			pr_notice("%s: id=%d skip due to non co-clock for CONN\n",
				__func__, id);
			pmic_clk_buf_ctrl_wcn(0);
			pmic_clk_buf_swctrl[XO_WCN] = 0;
			break;
...
static void dcxo_capid_write(uint32_t cap_code)
{
...
static uint32_t dcxo_capid_read(void)
{
	uint32_t cap_code = 0;
	...
	struct kobj_attribute *attr, char *buf)
{
	...
	len += snprintf(buf + len, PAGE_SIZE - len, "clkid=%d\n",
		dcxo_capid_read());
...
static int get_capid_idme(u32 *cap)
{
	struct device_node *ap;
	int len = 0;
	int ret = 0;

	ap = of_find_node_by_path(IDME_CAP_ID);
	if (ap) {
		const char *buf = of_get_property(ap, "value", &len);

		if (len > 0) {
			ret = kstrtouint(buf, 0, cap);
			if (ret || *cap > XO_CDAC_TOTAL_MASK) {
				pr_notice("%s wrong format!\n", __func__);
				return EINVAL;
			}
...
int clk_buf_dts_map(void)
{
	...
	node = of_find_compatible_node(NULL, NULL, "mediatek,mt6357-clkbuf");
	if (node) {
		...
		if (is_clk_buf_xowcn_coclock(node)) {
			ret = get_capid_idme(&cap);
			if (!ret)
				dcxo_capid_write(cap);
			else
				dcxo_capid_write(0xD7);
		}
