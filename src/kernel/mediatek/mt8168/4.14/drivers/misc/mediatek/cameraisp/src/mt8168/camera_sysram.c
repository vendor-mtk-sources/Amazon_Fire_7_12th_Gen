...
/* ---------------------------------------------------------------------------
 */
static void SYSRAM_GetTime(unsigned long long *pUS64, unsigned int *pSec,
			   unsigned int *pUSec)
{
	ktime_t Time;
...
{
	/* LOG_MSG("AllocatedTbl(0x%08X), EnableClk(%d)", Sysram.AllocatedTbl,
	 * Sysram.EnableClk);
	 */
...
static void SYSRAM_DumpResMgr(void)
{
	...
	LOG_MSG("TotalUserCount(%d), AllocatedTbl(0x%X)", Sysram.TotalUserCount,
		Sysram.AllocatedTbl);
	...
	for (u4Idx = 0; u4Idx < SYSRAM_USER_AMOUNT; u4Idx++) {
		if (Sysram.AllocatedSize[u4Idx] > 0) {
			...
			pr_debug("[id:%u][%s][size:0x%X][pid:%d][tgid:%d][%s][%5u.%06u]",
...
{
	if (User < 0 || SYSRAM_UserIsLocked(User))
		return;
	...
	if ((1 << User) & SysramLogUserMask) {
		struct SYSRAM_USER_STRUCT *const pUserInfo =
			&Sysram.UserInfo[User];
		LOG_MSG("[%s][%u bytes]OK, Time(%u.%06u)", SysramUserName[User],
			Sysram.AllocatedSize[User], pUserInfo->TimeS,
			pUserInfo->TimeUS);
	}
}
/* ---------------------------------------------------------------------------
...
{
	if (User < 0 || SYSRAM_UserIsUnlocked(User))
		return;
	/* Debug Log. */
	if ((1 << User) & SysramLogUserMask) {
		struct SYSRAM_USER_STRUCT *const pUserInfo =
...
static void SYSRAM_DumpLayout(void)
{
	...
	for (Index = 0; Index < SYSRAM_MEM_BANK_AMOUNT; Index++) {
		...
		pr_debug("[Locked Time][Owner Offset Size Index pCurrent pPrevious pNext][pid tgid][Proc Name / Owner Name]\n");
		pCurrNode = &SysramMemPoolInfo[Index].pMemNode[0];
		while (pCurrNode != NULL) {
			enum SYSRAM_USER_ENUM const User = pCurrNode->User;

			if (SYSRAM_IsBadOwner(User)) {
				pr_debug("------------ --------  %2d\t0x%05X 0x%05X  %d    %p %p\t%p\n",
				...
				pr_debug("%5u.%06u  %2d\t0x%05X 0x%05X  %d    %p %p\t%p   %-4d %-4d \"%s\" / \"%s\"\n",
					pUserInfo->TimeS, pUserInfo->TimeUS,
...
SYSRAM_AllocNode(struct SYSRAM_MEM_POOL_STRUCT *const pMemPoolInfo)
{
	...
	/* Shouldn't happen. */
	if (!pNode)
		LOG_MSG("returns NULL - pMemPoolInfo->IndexTbl(%X)",
...
static unsigned int
SYSRAM_AllocUserPhy(enum SYSRAM_USER_ENUM const User, unsigned int const Size,
			       ...
			       enum SYSRAM_MEM_BANK_ENUM const MemBankNo)
{
	...
	if (!pMemPoolInfo) {
		LOG_MSG("pMemPoolInfo==NULL, User(%d), MemBankNo(%d)", User,
			MemBankNo);
		return MFALSE;
	}
...
/* ---------------------------------------------------------------------------
 */
static unsigned int SYSRAM_AllocUser(enum SYSRAM_USER_ENUM const User,
				     unsigned int Size,
				     unsigned int const Alignment)
{
	unsigned int Addr = 0;
					...
					unsigned int const Alignment)
{
	...
	if (SYSRAM_UserIsLocked(User)) {
		SYSRAM_SpinUnlock();
		LOG_MSG("[User:%s]has been already allocated!",
			SysramUserName[User]);
		return 0;
	}
...
EXIT:
	if (Addr == 0) { /* Failure */
		pr_debug("[User:%s]fails to allocate.Size(%u), Alignment(%u), TimeoutMS(%u)",
			SysramUserName[User], Size, Alignment, TimeoutMS);
		SYSRAM_DumpLayout();
	} else { /* Success */
		if ((1 << User) & SysramLogUserMask) {
			LOG_MSG("[User:%s]%u bytes OK", SysramUserName[User],
				Size);
		}
	}
...
{
	if (User < 0 || SYSRAM_IsBadOwner(User)) {
		LOG_MSG("User(%d) out of range(%d)", User, SYSRAM_USER_AMOUNT);
		return;
	}
...
static int SYSRAM_Open(struct inode *pInode, struct file *pFile)
{
	...
	LOG_MSG("Cur:Name(%s), pid(%d), tgid(%d), Time(%d.%06d)", current->comm,
...
static int SYSRAM_Release(struct inode *pInode, struct file *pFile)
{
	...
	if (pFile->private_data != NULL) {
		...
		if (pProc->Pid != 0 || pProc->Tgid != 0 || pProc->Table != 0) {
			/* */
			pr_debug("Proc:Name(%s), pid(%d), tgid(%d), Table(0x%08X), Time(%d.%06d)",
			...
			if (pProc->Table) {
				...
				/*LOG_MSG("Proc:Name(%s), pid(%d), tgid(%d),*/
				/* Table(0x%08lX), Time(%ld.%06ld)",*/
				/*		pProc->ProcName,*/
				/*		pProc->Pid,*/
				/*		pProc->Tgid,*/
				/*		pProc->Table,*/
...
static int SYSRAM_Flush(struct file *pFile, fl_owner_t Id)
{
	...
	LOG_MSG("Proc:Name(%s),pid(%d),tgid(%d),Table(0x%08X),Time(%d.%06d)",
		pProc->ProcName, pProc->Pid, pProc->Tgid, pProc->Table,
		pProc->TimeS, pProc->TimeUS);

	if (pProc->Tgid == 0 && pProc->Table != 0) {
		LOG_MSG("No Tgid info");

		/*LOG_MSG("Cur:Name(%s), pid(%d), tgid(%d),*/
		/* Time(%ld.%06ld)",*/
		/*		current->comm,*/
		/*		current->pid,*/
		/*		current->tgid,*/
...
static int SYSRAM_mmap(struct file *pFile, struct vm_area_struct *pVma)
{
	...
	pfn = pVma->vm_pgoff << PAGE_SHIFT;/* page from number, physical
					    * address of kernel memory
					    */
	pr_debug("pVma->vm_pgoff(0x%lx), phy(0x%lx), pVmapVma->vm_start(0x%lx), pVma->vm_end(0x%lx), length(%lx)",
	...
	if ((length > ISP_VALID_REG_RANGE) || (pfn < IMGSYS_BASE_ADDR) ||
		...
		pr_debug("mmap range error : vm_start(0x%lx), vm_end(0x%lx), length(%lx), pfn(%u)!",
	...
	case SYSRAM_FREE: {
		if (copy_from_user(&User, (void *)Param,
			...
			if ((pProc->Table) & (1 << User)) {
				...
				SYSRAM_SpinUnlock();
				LOG_MSG("Freeing unallocated buffer user(%d)",
					User);
				Ret = -EFAULT;
			}
		} else {
...
static struct platform_driver SysramPlatformDriver = {
	...
	.driver = {
		.name = SYSRAM_DEV_NAME, .owner = THIS_MODULE,
	} };
/* ---------------------------------------------------------------------------
 */
...
static ssize_t SYSRAM_DumpLayoutToProc(struct file *pFile, char *pStart,
				       size_t Off, loff_t *Count)
{
	LOG_MSG("%s: Not implement", __func__);
	return 0;
}
/* ---------------------------------------------------------------------------
 */
...
static ssize_t SYSRAM_ReadFlag(struct file *pFile, char *pStart, size_t Off,
			       loff_t *Count)
{
	LOG_MSG("%s: Not implement", __func__);
	return 0;
...
static ssize_t SYSRAM_WriteFlag(struct file *pFile, const char *pBuffer,
...
#if 1
	proc_create("sysram", 0440, NULL, &fsysram_proc_fops);
	proc_create("sysram_flag", 0440, NULL, &fsysram_flag_proc_fops);
#else
