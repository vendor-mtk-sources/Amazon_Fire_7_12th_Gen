...
static void fh_switch2fhctl(enum FH_PLL_ID pll_id, int i_control)
{
	...
	if ((pll_id >= FH_PLL_NUM || pll_id < 0) ||
	    (g_reg_pll_con0[pll_id] == REG_PLL_NOT_SUPPORT)) {
		pr_err("Invalid pll id!\n");
		return;
	}
...
static int __freqhopping_ctrl(struct freqhopping_ioctl *fh_ctl, bool enable)
{
	...
	if ((fh_ctl->pll_id >= FH_PLL_NUM || fh_ctl->pll_id < 0) ||
	    (g_reg_pll_con0[fh_ctl->pll_id] == REG_PLL_NOT_SUPPORT)) {
		pr_err("Invalid pll id!\n");
		retVal = 0;
		goto Exit;
	}
				     ...
				     unsigned int target_dds)
{
	...
	FH_MSG_DEBUG("%s, [Pll_ID]:%d [current dds]:0x%x, [target dds]:%d",
	       __func__, pll_id,
