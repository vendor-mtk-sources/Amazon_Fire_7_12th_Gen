...
static atomic_t g_pwm_max_backlight[PWM_TOTAL_MODULE_NUM] = {
	ATOMIC_INIT(1023) };
static atomic_t g_pwm_is_power_on[PWM_TOTAL_MODULE_NUM] = { ATOMIC_INIT(1) };
static atomic_t g_pwm_value_before_power_off[PWM_TOTAL_MODULE_NUM] = {
	ATOMIC_INIT(0) };
static atomic_t g_pwm_is_change_state[PWM_TOTAL_MODULE_NUM] = {
	ATOMIC_INIT(0) };
...
#ifdef CONFIG_SKIP_BRIGHTNESS_AFTER_PWM_CLOCKOFF
	if (is_power_on) {
#else
	if (is_power_on == true && high_width > 0) {
#endif
		PWM_MSG("backlight is on (%d), power:(%d), pwm id: (%d)",
		...
		PWM_MSG("backlight is off, power:(%d), pwm id: (%d)",
			is_power_on, index);
...
#ifdef CONFIG_SKIP_BRIGHTNESS_AFTER_PWM_CLOCKOFF
	if (atomic_read(&g_pwm_is_power_on[index]) == 0) {
		PWM_ERR("[ERROR] not allow setup disp_pwm[%d] registers after clock off", index);
		return -EPERM;
	}
#endif
...
static int ddp_pwm_power_on(enum DISP_MODULE_ENUM module, void *handle)
{
...
#ifdef CONFIG_SYNC_DISP_PWM_OFF
	int index = index_of_pwm(id);
	const unsigned long reg_base = pwm_get_reg_base(id);
#endif
	...
	defined(CONFIG_MACH_MT6761) || defined(CONFIG_MACH_MT8168)
	/* pwm ccf api */
...
#ifdef CONFIG_SYNC_DISP_PWM_OFF
	if (atomic_read(&g_pwm_is_power_on[index]) == 0) {
		DISP_REG_MASK(NULL, reg_base + DISP_PWM_CON_1_OFF,
			atomic_read(&g_pwm_value_before_power_off[index]) << 16, 0x1fff << 16);
