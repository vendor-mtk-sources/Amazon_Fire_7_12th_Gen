...
static void cat_register(char *buf)
{
...
#ifdef CONFIG_ACCDET_EINT_IRQ
#ifdef CONFIG_ACCDET_SUPPORT_EINT0
	nRet = sprintf(buf_temp, "[Accdet EINT0 support][MODE_%d]regs:\n",
		    accdet_dts.mic_mode);
	if (nRet < 0) {
...
#elif defined CONFIG_ACCDET_SUPPORT_EINT1
	nRet = sprintf(buf_temp, "[ccdet EINT1 support][MODE_%d]regs:\n",
		accdet_dts.mic_mode);
	if (nRet < 0) {
		pr_info("sprintf failed");
		return;
	...
	strncat(buf, buf_temp, strlen(buf_temp));
#elif defined CONFIG_ACCDET_SUPPORT_BI_EINT
	nRet = sprintf(buf_temp, "[Accdet BIEINT support][MODE_%d] regs:\n",
		accdet_dts.mic_mode);
	if (nRet < 0) {
		pr_info("sprintf failed");
		return;
...
#endif
#elif defined CONFIG_ACCDET_EINT
	nRet = sprintf(buf_temp, "[Accdet AP EINT][MODE_%d] regs:\n",
		accdet_dts.mic_mode);
	if (nRet < 0) {
		pr_info("sprintf failed");
		return;
	...
	for (i = ACCDET_RSV; i <= ACCDET_EINT1_CUR_DEB; i += 2) {
		nRet = sprintf(buf_temp, "ADDR[0x%x]=0x%x\n", i, pmic_read(i));
		if (nRet < 0) {
			pr_info("sprintf failed");
			return;
		}
	...
	nRet = sprintf(buf_temp, "[0x%x]=0x%x\n",
		TOP_CKPDN_CON0, pmic_read(TOP_CKPDN_CON0));
	if (nRet < 0) {
		pr_info("sprintf failed");
		return;
	...
	nRet = sprintf(buf_temp, "[0x%x]=0x%x, [0x%x]=0x%x, [0x%x]=0x%x\n",
	...
	nRet = sprintf(buf_temp, "[0x%x]=0x%x, [0x%x]=0x%x\n",
		AUXADC_RQST0, pmic_read(AUXADC_RQST0),
		AUXADC_ACCDET, pmic_read(AUXADC_ACCDET));
	if (nRet < 0) {
		pr_info("sprintf failed");
...
static ssize_t state_show(struct device_driver *ddri, char *buf)
{
	...
	ret_snprintf = snprintf(buf, 3, "%d\n", temp_type);
	if (ret_snprintf < 0) {
		pr_notice("snprintf alloc error!, ret = %d", ret_snprintf);
		return -EINVAL;
	}
