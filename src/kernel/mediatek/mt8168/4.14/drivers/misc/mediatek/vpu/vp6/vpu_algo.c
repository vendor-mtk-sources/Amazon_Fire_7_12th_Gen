...
int vpu_init_algo(struct vpu_device *vpu_device)
{
	...
	for (i = 0; i < vpu_device->core_num; i++)
		INIT_LIST_HEAD(&(vpu_device->vpu_core[i]->vpu_algo_pool));
	...
	vpu_device->prop_info_data_length = offset;
...
{
	LOG_DBG("[vpu] %s +\n", __func__);
	list_add_tail(vlist_link(algo, struct vpu_algo),
			&vpu_core->vpu_algo_pool);
	return 0;
}
...
int vpu_free_algo_from_user_pool(unsigned int core, vpu_id_t id,
...
int vpu_find_algo_by_id_from_user(unsigned int core, vpu_id_t id,
...
int vpu_find_algo_by_name_from_user(unsigned int core, char *name,
			struct vpu_algo **ralgo, struct vpu_user *user)
{
	struct vpu_algo *algo;
	struct list_head *head;
	...
	struct vpu_algo **ralgo, struct vpu_user *user)
{
	...
	struct list_head *head;
	char *name;
	unsigned int core = vpu_core->core;

	if (id < 1)
		goto err;


	list_for_each(head, &vpu_core->vpu_algo_pool)
	{
		algo = vlist_node_of(head, struct vpu_algo);
		if (algo->id[core] == id) {
			*ralgo = algo;
	...
	if (vpu_get_name_of_algo(vpu_core, id, &name))
		goto err;

	if (vpu_create_algo(vpu_core, name, &algo, false) == 0) {
		vpu_add_algo_to_pool(vpu_core, algo);
		*ralgo = algo;
		return 0;
	}
	...
	struct vpu_algo **ralgo, bool needload, struct vpu_user *user)
{
	...
	if (vpu_create_algo(vpu_core, name, &algo,
				needload) == 0) {
		vpu_add_algo_to_pool(vpu_core, algo);
		*ralgo = algo;
		return 0;
				...
				struct vpu_user *user)
{
	...
	ret = vpu_wait_load_image(vpu_core->vpu_device);
	if (ret) {
		LOG_ERR("[%s][%d] wait load image failed!\n",
			__func__, __LINE__);
		goto out;
	}

	LOG_DBG("%s core:%d\n", __func__, core);
	ret = vpu_find_algo_by_name(vpu_core, name, &algo, false, user);
	LOG_DBG("ret:%d\n", ret);
	if (ret) {
...
int vpu_create_algo(struct vpu_core *vpu_core, char *name,
	struct vpu_algo **ralgo, bool needload)
{
	...
	core = vpu_core->core;
	ret = vpu_get_entry_of_algo(vpu_core, name, &id, &mva, &length);
	if (ret) {
		LOG_ERR("algo(%s) is not existed in image files!\n", name);
		goto out;
	}

	ret = vpu_alloc_algo(vpu_device, &algo);
	if (ret) {
	...
	if (needload) {
		ret = vpu_hw_load_algo(vpu_core, algo);
		if (ret) {
		...
		ret = vpu_hw_get_algo_info(vpu_core, algo);
		if (ret) {
				...
				struct vpu_create_algo *create_algo)
{
	...
	ret = vpu_wait_load_image(vpu_device);
	if (ret) {
		LOG_ERR("[%s][%d] wait load image failed!\n",
			__func__, __LINE__);
		goto out;
	...
	vpu_get_default_algo_num(vpu_device->vpu_core[core], &algo_num);
...
int vpu_alloc_algo(struct vpu_device *vpu_device, struct vpu_algo **ralgo)
{
	...
	algo = kzalloc(sizeof(vlist_type(struct vpu_algo)) +
				vpu_device->prop_info_data_length, GFP_KERNEL);
	if (algo == NULL) {
		LOG_ERR("%s(), algo=0x%p\n", __func__, algo);
		return -ENOMEM;
	}
	algo->info_ptr = (uintptr_t) algo + sizeof(vlist_type(struct vpu_algo));
	algo->info_length = vpu_device->prop_info_data_length;
...
int vpu_dump_algo(struct seq_file *s, struct vpu_device *gvpu_device)
{
	...
	int debug_core = 0;
	const char *line_bar1 = "  +-----+---------------+-------+-------+\n";
	const char *line_bar2 =
"  +-----+---------------+-------+-------+------------------------------+\n";
	const char *line_bar3 =
		"  +-----+---------------+-------+-------+-------+\n";
	int ret = 0;
	...
	vpu_core = vpu_device->vpu_core[debug_core];

	ret = vpu_check_load_image_state(vpu_device);
	if (ret)
		return -EINVAL;

	if (vpu_get_name_of_algo(vpu_core, vpu_device->debug_algo_id,
						&debug_algo_name)) {
		vpu_print_seq(s, "vpu_get_name_of_algo fail, id=%d\n",
					vpu_device->debug_algo_id);
		goto err;
	}

	if (vpu_create_algo(vpu_core, debug_algo_name,
					&debug_algo, true) == 0) {
...
out:
		...
		vpu_debug_func_core_state(vpu_device->vpu_core[debug_core],
						VCT_IDLE);
		vpu_shut_down(vpu_device->vpu_core[debug_core]);
	} else {
				...
				int argc, int *args)
{
	...
	case VPU_DEBUG_ALGO_PARAM_DUMP_ALGO:
		...
		vpu_device->debug_algo_id = args[0];
