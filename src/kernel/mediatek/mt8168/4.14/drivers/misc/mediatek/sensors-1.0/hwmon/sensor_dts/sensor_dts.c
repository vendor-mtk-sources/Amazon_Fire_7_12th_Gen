...
int check_caculate_by_dts(const struct device_node *node, int *change_caculate)
{
	int ret = 0;
	u32 is_change_caculate = 0;

	if (node) {
		ret = of_property_read_u32(node, "is_change_caculate",
		 &is_change_caculate);
		if (ret == 0)
			*change_caculate = is_change_caculate;
	} else {
		pr_info("Not find dts node, ST gsensor apply default algorithm.\n");
		ret = -1;
	}
