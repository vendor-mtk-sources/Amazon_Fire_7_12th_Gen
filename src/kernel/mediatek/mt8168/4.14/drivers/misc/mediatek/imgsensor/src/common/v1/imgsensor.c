 ...
 ******************************************************************************/
#define IMGSENSOR_FUNCTION_ENTRY()  /*cam_pr_info("[%s]:E\n",__FUNCTION__) */
#define IMGSENSOR_FUNCTION_EXIT()   /*cam_pr_info("[%s]:X\n",__FUNCTION__) */
...
#if defined(CONFIG_MTK_CAM_SECURITY_SUPPORT)
	cam_pr_info("%s secure state %d", __func__,
		...
		MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
	...
	if (psensor_func &&
		...
		if (ret != ERROR_NONE)
			...
			cam_pr_err("[%s] SensorGetInfo failed\n", __func__);
...
static void imgsensor_init_sensor_list(void)
{
	...
	if (ret < 0) {
		cam_pr_debug("Property cust-sensor not defined: %s\n",
			penable_sensor);
		...
		cam_pr_debug("Customizedsensors: %s\n", penable_sensor);
		while (psensor_list->id && i < MAX_NUM_OF_SUPPORT_SENSOR) {
...
static inline int imgsensor_check_is_alive(struct IMGSENSOR_SENSOR *psensor)
{
	...
	if (sensorID == 0 || sensorID == 0xFFFFFFFF) {
		...
		cam_pr_debug("Sensor found ID = 0x%x\n", sensorID);
		err = ERROR_NONE;
	}
	if (err != ERROR_NONE)
		cam_pr_err("ERROR:%s, No imgsensor alive\n", __func__);
...
int imgsensor_set_driver(struct IMGSENSOR_SENSOR *psensor)
{
	...
	struct IMGSENSOR *pimgsensor = &gimgsensor;
	struct IMGSENSOR_SENSOR_INST *psensor_inst = &psensor->inst;
	struct IMGSENSOR_SENSOR_LIST *pSensorList = gimgsensor_sensor_list;
	...
	if (psensor_inst->sensor_idx >= IMGSENSOR_SENSOR_IDX_MIN_NUM &&
		 psensor_inst->sensor_idx < IMGSENSOR_SENSOR_IDX_MAX_NUM)
	...
	if (get_search_list) {
		...
		if (psensor_list_config) {
			char *tmp_psensor_list_config = NULL;
			...
			*(psensor_list_config+strlen(sensor_configs)-2) = '\0';
			tmp_psensor_list_config = psensor_list_config;
			cam_pr_info("sensor_list %s\n", psensor_list_config);
			driver_name = strsep(&tmp_psensor_list_config, " \0");
			cam_pr_err("drv_name: [%s]\n", driver_name);
			...
			while (driver_name != NULL) {
				...
				driver_name =
					strsep(&tmp_psensor_list_config, " \0");
			}
	...
	for (i = 0; i < MAX_NUM_OF_SUPPORT_SENSOR; i++) {
		...
		if (pSensorList[drv_idx].init) {
			...
			if (psensor->pfunc) {
				/* get sensor list and sensor name */
				psensor_inst->psensor_list
					= pimgsensor->psensor_list[drv_idx];
			...
			cam_pr_err("ERROR:NULL sensor list[%d]\n", drv_idx);
		}
	}
...
static inline int adopt_CAMERA_HW_GetInfo(void *pBuf)
{
	...
	if (pSensorGetInfo == NULL ||
		...
		cam_pr_err("[CAMERA_HW] NULL arg.\n");
		return -EFAULT;
	}

	psensor = imgsensor_sensor_get_inst(pSensorGetInfo->SensorId);
	if (psensor == NULL) {
		cam_pr_err("[CAMERA_HW] NULL psensor.\n");
		return -EFAULT;
	}
	...
	if (pInfo == NULL || pConfig == NULL) {
		kfree(pInfo);
		kfree(pConfig);
		cam_pr_err(" ioctl allocate mem failed\n");
		return -ENOMEM;
	}
	...
	if (copy_to_user((void __user *)(pSensorGetInfo->pInfo),
		...
		cam_pr_err("[CAMERA_HW][info] ioctl copy to user failed\n");
		if (pInfo != NULL)
			kfree(pInfo);
		if (pConfig != NULL)
			kfree(pConfig);
	...
	if (copy_to_user((void __user *) (pSensorGetInfo->pConfig),
		...
		cam_pr_err("[CAMERA_HW][config] ioctl copy to user failed\n");
	...
	MUINT8 *valid, MUINT32 *temp)
{
	...
	*temp  = 0;
	...
	/* Sensor is not in close state, where in close state */
	...
	if (psensor_inst->state != IMGSENSOR_STATE_CLOSE) {
		...
		cam_pr_debug("senDevId(%d), temperature(%d)\n",
			senDevId,
			*temp);
...
static void cam_temperature_report_wq_routine(struct work_struct *data)
{
	...
	cam_pr_debug("Temperature Meter Report.\n");

	/* Main cam */
	ret = Get_Camera_Temperature(
		DUAL_CAMERA_MAIN_SENSOR, &valid[0], &temp[0]);
	cam_pr_info("senDevId(%d), valid(%d), temperature(%d)\n",
		DUAL_CAMERA_MAIN_SENSOR, valid[0], temp[0]);

	if (ret != ERROR_NONE)
		cam_pr_err("Get Main cam temperature error(%d)!\n", ret);
	...
	if (ret != ERROR_NONE)
		cam_pr_err("Get Sub cam temperature error(%d)!\n", ret);
	...
	if (ret != ERROR_NONE)
		cam_pr_err("Get Main2 cam temperature error(%d)!\n", ret);
	...
	if (ret != ERROR_NONE)
		cam_pr_err("Get Sub2 cam temperature error(%d)!\n", ret);
...
static inline int adopt_CAMERA_HW_GetInfo2(void *pBuf)
{
	...
	if (pSensorGetInfo == NULL ||
		...
		cam_pr_err("[%s] NULL arg.\n", __func__);
		return -EFAULT;
	}

	psensor = imgsensor_sensor_get_inst(pSensorGetInfo->SensorId);
	if (psensor == NULL) {
		cam_pr_err("[%s] NULL psensor.\n", __func__);
		return -EFAULT;
	}

	cam_pr_debug("[%s]Entry%d\n", __func__, pSensorGetInfo->SensorId);
	...
	pSensorInfo->SensorClockPolarity =
		pInfo->SensorClockPolarity;
	pSensorInfo->SensorClockFallingPolarity =
		pInfo->SensorClockFallingPolarity;
	pSensorInfo->SensorClockRisingCount =
		pInfo->SensorClockRisingCount;
	pSensorInfo->SensorClockFallingCount =
		pInfo->SensorClockFallingCount;
	pSensorInfo->SensorClockDividCount =
		pInfo->SensorClockDividCount;
	pSensorInfo->SensorPixelClockCount =
		pInfo->SensorPixelClockCount;
	pSensorInfo->SensorDataLatchCount =
		pInfo->SensorDataLatchCount;
	pSensorInfo->SensorHsyncPolarity =
		pInfo->SensorHsyncPolarity;
	pSensorInfo->SensorVsyncPolarity =
		pInfo->SensorVsyncPolarity;
	pSensorInfo->SensorInterruptDelayLines =
		pInfo->SensorInterruptDelayLines;
	pSensorInfo->SensorResetActiveHigh =
		pInfo->SensorResetActiveHigh;
	pSensorInfo->SensorResetDelayCount =
		pInfo->SensorResetDelayCount;
	pSensorInfo->SensroInterfaceType =
		pInfo->SensroInterfaceType;
	pSensorInfo->SensorOutputDataFormat =
		pInfo->SensorOutputDataFormat;
	pSensorInfo->SensorMIPILaneNumber =
		pInfo->SensorMIPILaneNumber;
	pSensorInfo->CaptureDelayFrame =
		pInfo->CaptureDelayFrame;
	pSensorInfo->PreviewDelayFrame =
		pInfo->PreviewDelayFrame;
	pSensorInfo->VideoDelayFrame =
		pInfo->VideoDelayFrame;
	pSensorInfo->HighSpeedVideoDelayFrame =
		pInfo->HighSpeedVideoDelayFrame;
	pSensorInfo->SlimVideoDelayFrame =
		pInfo->SlimVideoDelayFrame;
	pSensorInfo->Custom1DelayFrame =
		pInfo->Custom1DelayFrame;
	pSensorInfo->Custom2DelayFrame =
		pInfo->Custom2DelayFrame;
	pSensorInfo->Custom3DelayFrame =
		pInfo->Custom3DelayFrame;
	pSensorInfo->Custom4DelayFrame =
		pInfo->Custom4DelayFrame;
	pSensorInfo->Custom5DelayFrame =
		pInfo->Custom5DelayFrame;
	pSensorInfo->YUVAwbDelayFrame =
		pInfo->YUVAwbDelayFrame;
	pSensorInfo->YUVEffectDelayFrame =
		pInfo->YUVEffectDelayFrame;
	pSensorInfo->SensorGrabStartX_PRV =
		pInfo->SensorGrabStartX;
	pSensorInfo->SensorGrabStartY_PRV =
		pInfo->SensorGrabStartY;
	pSensorInfo->SensorGrabStartX_CAP =
		pInfo1->SensorGrabStartX;
	pSensorInfo->SensorGrabStartY_CAP =
		pInfo1->SensorGrabStartY;
	pSensorInfo->SensorGrabStartX_VD =
		pInfo2->SensorGrabStartX;
	pSensorInfo->SensorGrabStartY_VD =
		pInfo2->SensorGrabStartY;
	pSensorInfo->SensorGrabStartX_VD1 =
		pInfo3->SensorGrabStartX;
	pSensorInfo->SensorGrabStartY_VD1 =
		pInfo3->SensorGrabStartY;
	pSensorInfo->SensorGrabStartX_VD2 =
		pInfo4->SensorGrabStartX;
	pSensorInfo->SensorGrabStartY_VD2 =
		pInfo4->SensorGrabStartY;
	pSensorInfo->SensorDrivingCurrent =
		pInfo->SensorDrivingCurrent;
	pSensorInfo->SensorMasterClockSwitch =
		pInfo->SensorMasterClockSwitch;
	pSensorInfo->AEShutDelayFrame =
		pInfo->AEShutDelayFrame;
	pSensorInfo->AESensorGainDelayFrame =
		pInfo->AESensorGainDelayFrame;
	pSensorInfo->AEISPGainDelayFrame =
		pInfo->AEISPGainDelayFrame;
	pSensorInfo->FrameTimeDelayFrame =
		pInfo->FrameTimeDelayFrame;
	...
	pSensorInfo->SensorWidthSampling =
		pInfo->SensorWidthSampling;
	pSensorInfo->SensorHightSampling =
		pInfo->SensorHightSampling;
	pSensorInfo->SensorPacketECCOrder =
		pInfo->SensorPacketECCOrder;
	...
	pSensorInfo->SensorGrabStartX_CST1 =
		pInfo->SensorGrabStartX;
	pSensorInfo->SensorGrabStartY_CST1 =
		pInfo->SensorGrabStartY;
	pSensorInfo->SensorGrabStartX_CST2 =
		pInfo1->SensorGrabStartX;
	pSensorInfo->SensorGrabStartY_CST2 =
		pInfo1->SensorGrabStartY;
	pSensorInfo->SensorGrabStartX_CST3 =
		pInfo2->SensorGrabStartX;
	pSensorInfo->SensorGrabStartY_CST3 =
		pInfo2->SensorGrabStartY;
	pSensorInfo->SensorGrabStartX_CST4 =
		pInfo3->SensorGrabStartX;
	pSensorInfo->SensorGrabStartY_CST4 =
		pInfo3->SensorGrabStartY;
	pSensorInfo->SensorGrabStartX_CST5 =
		pInfo4->SensorGrabStartX;
	pSensorInfo->SensorGrabStartY_CST5 =
		pInfo4->SensorGrabStartY;
	...
	if (copy_to_user(
		...
		cam_pr_err("[CAMERA_HW][Resolution] ioctl copy to user failed\n");
		ret = -EFAULT;
		goto IMGSENSOR_GET_INFO_RETURN;
	}
...
static inline int adopt_CAMERA_HW_FeatureControl(void *pBuf)
{
	...
	if (pFeatureCtrl->FeatureId == SENSOR_FEATURE_SINGLE_FOCUS_MODE ||
		...
		if (copy_from_user((void *)&FeatureParaLen,
			...
			cam_pr_err(" ioctl copy from user failed\n");
			return -EFAULT;
		}
	...
	case SENSOR_FEATURE_OPEN_SECURE_SESSION:
		cam_pr_info("SECURE_SENSOR_ID = %x\n",
		...
		c_params, &ret) != 0) || (ret != 0)) {
			cam_pr_err("Error!! set secure sensor_pfunc failed!");
			return ret;
		}
		break;
	...
	case SENSOR_FEATURE_SET_AS_SECURE_DRIVER:
		...
		cam_pr_info("debug: secure set as %d",
			(int)((&gimgsensor)->imgsensor_sec_flag));
		break;
#endif
	...
	case SENSOR_FEATURE_SET_DRIVER:
	{
		...
		cam_pr_debug("sensor drv idx: [%d]\n", drv_idx);
		memcpy(pFeaturePara, &drv_idx, sizeof(drv_idx));
		break;
	}
	...
	case SENSOR_FEATURE_SET_SENSOR_SYNC_MODE:
		if (copy_from_user((void *)pFeaturePara,
			...
			FeatureParaLen)) {
			kfree(pFeaturePara);
			cam_pr_err("[CAMERA_HW] ioctl copy from user failed\n");
			return -EFAULT;
		}
		break;
	case SENSOR_FEATURE_SET_SENSOR_SYNC:
	case SENSOR_FEATURE_SET_ESHUTTER_GAIN:
		cam_pr_debug("[kd_sensorlist]enter kdSetExpGain\n");
		if (copy_from_user((void *)pFeaturePara,
			(void *) pFeatureCtrl->pFeaturePara,
			FeatureParaLen)) {
			kfree(pFeaturePara);
	...
	case SENSOR_FEATURE_GET_EV_AWB_REF:
	{
		...
		*(pFeaturePara_64) = (uintptr_t) pAeAwbRef;
		...
		if (copy_to_user((void __user *)usr_ptr,
			(void *)pAeAwbRef,
			sizeof(struct SENSOR_AE_AWB_REF_STRUCT)))
			cam_pr_err("[CAMERA_HW]ERROR: copy_to_user fail\n");
		kfree(pAeAwbRef);
	...
	case SENSOR_FEATURE_SET_PDAF_REG_SETTING:
	{
		...
		if ((sizeof(kal_uint8) * u4RegLen) >
			...
			kfree(pFeaturePara);
			cam_pr_err(" buffer size (%u) is too large\n",
				u4RegLen);
			return -EINVAL;
		}
	...
	case SENSOR_FEATURE_SET_AE_WINDOW:
	{
		...
		if (FeatureParaLen < 1 * sizeof(unsigned long long)) {
			cam_pr_err("FeatureParaLen is too small %d\n",
				FeatureParaLen);
			kfree(pFeaturePara);
			return -EINVAL;
		}
	...
	case SENSOR_FEATURE_SET_LSC_TBL:
	{
		...
		pReg = kmalloc_array(u4RegLen, sizeof(kal_uint8),
				GFP_KERNEL);
		if (pReg == NULL) {
			kfree(pFeaturePara);
			cam_pr_err(" ioctl allocate mem failed\n");
	...
	case SENSOR_FEATURE_GET_SENSOR_SYNC_MODE:
		if (copy_to_user((void __user *) pFeatureCtrl->pFeaturePara,
			...
			kfree(pFeaturePara);
			cam_pr_err("[CAMERA_HW] ioctl copy to user failed\n");
			return -EFAULT;
		}
		break;
	...
	unsigned int cmd, unsigned long arg)
{
	...
	case COMPAT_KDIMGSENSORIOC_X_GETINFO:
	{
		...
		cam_pr_debug(
		"[CAMERA SENSOR] CAOMPAT_KDIMGSENSORIOC_X_GETINFO E\n");
		...
		if (err != 0)
			cam_pr_err("[CAMERA SENSOR] compat_put_acdk_sensor_getinfo_struct failed\n");
		return ret;
	}
	case COMPAT_KDIMGSENSORIOC_X_FEATURECONCTROL:
	{
		...
		if (err != 0)
			cam_pr_err("[CAMERA SENSOR] compat_put_acdk_sensor_featurecontrol_struct failed\n");
		return ret;
	}
	case COMPAT_KDIMGSENSORIOC_X_CONTROL:
	{
		...
		if (err != 0)
			cam_pr_err("[CAMERA SENSOR] compat_put_acdk_sensor_control_struct failed\n");
		return ret;
	}
	...
	if (_IOC_DIR(a_u4Command) != _IOC_NONE) {
		...
		if (_IOC_WRITE & _IOC_DIR(a_u4Command)) {
			if (copy_from_user(pBuff, (void *)a_u4Param,
				...
				kfree(pBuff);
				pBuff = NULL;
				cam_pr_err("[CAMERA SENSOR] ioctl copy from user failed\n");
				i4RetValue = -EFAULT;
				goto CAMERA_HW_Ioctl_EXIT;
			}
		} else
...
#define ISP_CLK_HIGH   490
#ifdef CONFIG_MTK_SMI_EXT
		cam_pr_debug("KDIMGSENSORIOC_X_GET_ISP_CLK current_mmsys_clk=%d\n",
			current_mmsys_clk);
	...
	if ((_IOC_READ & _IOC_DIR(a_u4Command)) &&
		...
		kfree(pBuff);
		pBuff = NULL;
		cam_pr_err("[CAMERA SENSOR] ioctl copy to user failed\n");
		i4RetValue = -EFAULT;
		goto CAMERA_HW_Ioctl_EXIT;
	}
...
static int imgsensor_open(struct inode *a_pstInode, struct file *a_pstFile)
{
	...
	if (atomic_read(&pimgsensor->imgsensor_open_cnt) == 0)
		imgsensor_clk_enable_all(&pimgsensor->clk);
...
static void imgsensor_release_secure_flag(void)
{
	...
	cam_pr_debug("release secure flag! %d",
		(int)pimgsensor->imgsensor_sec_flag);
}
#endif
...
static int imgsensor_release(struct inode *a_pstInode, struct file *a_pstFile)
{
	...
	if (atomic_read(&pimgsensor->imgsensor_open_cnt) == 0) {
		imgsensor_clk_disable_all(&pimgsensor->clk);
...
static int imgsensor_probe(struct platform_device *pplatform_device)
{
	...
	if (alloc_chrdev_region(&pimgsensor->dev_no, 0, 1,
			IMGSENSOR_DEV_NAME)) {
		cam_pr_err("[CAMERA SENSOR] Allocate device no failed\n");
		return -EAGAIN;
	}
	...
	if (pimgsensor->pcdev == NULL) {
		...
		cam_pr_err("[CAMERA SENSOR] Allocate mem for kobject failed\n");
		return -ENOMEM;
	}
	...
	if (cdev_add(pimgsensor->pcdev, pimgsensor->dev_no, 1)) {
		cam_pr_err("Attatch file operation failed\n");
		unregister_chrdev_region(pimgsensor->dev_no, 1);
		return -EAGAIN;
	}
	...
	if (IS_ERR(pimgsensor->pclass)) {
		...
		cam_pr_err("Unable to create class, err = %d\n", ret);
		return ret;
	}

	gimgsensor_device = device_create(pimgsensor->pclass, NULL,
		pimgsensor->dev_no, NULL, IMGSENSOR_DEV_NAME);

	gimgsensor_device->of_node =
		of_find_compatible_node(NULL, NULL, "mediatek,camera_hw");
	if (!gimgsensor_device->of_node) {
		cam_pr_err("Get cust camera node failed!\n");
		return -ENODEV;
	}

	gpimgsensor_hw_platform_device = pplatform_device;

	imgsensor_clk_init(&pimgsensor->clk);
	imgsensor_hw_init(&pimgsensor->hw);
	imgsensor_i2c_create();
	imgsensor_proc_init();
	imgsensor_init_sensor_list();

	atomic_set(&pimgsensor->imgsensor_open_cnt, 0);
#ifdef CONFIG_MTK_SMI_EXT
...
{
	cam_pr_debug("%s ori: %d, new: %d, current_mmsys_clk %d\n",
		__func__,
		ori_clk_mode,
		new_clk_mode,
		current_mmsys_clk);
...
{
	cam_pr_debug("[camerahw_probe] start\n");

	if (platform_driver_register(&gimgsensor_platform_driver)) {
		cam_pr_err("failed to register CAMERA_HW driver\n");
		return -ENODEV;
	}
