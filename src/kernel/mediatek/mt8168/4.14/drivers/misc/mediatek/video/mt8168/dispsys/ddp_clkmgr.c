...
static struct ddp_clk ddp_clks[MAX_DISP_CLK_CNT] = {
	...
	{NULL, "MMSYS_DISP_RDMA1", 0, (1|1<<1), DISP_MODULE_RDMA1},
	...
	{NULL, "MMSYS_DPI0_DPI0", 0, (0), DISP_MODULE_UNKNOWN},
	{NULL, "MMSYS_DPI0", 0, (0), DISP_MODULE_UNKNOWN},
	{NULL, "MMSYS_LVDSTX_PXL", 0, (0), DISP_MODULE_UNKNOWN},
	{NULL, "MMSYS_LVDSTX_CTS", 0, (0), DISP_MODULE_UNKNOWN},
	{NULL, "TOP_LVDSTX_CLKDIG_EN", 0, (0), DISP_MODULE_UNKNOWN},
	{NULL, "TOP_VPLL_DPIX_EN", 0, (0), DISP_MODULE_UNKNOWN},
	...
	{NULL, "APMIXED_LVDSPLL", 0, (0), DISP_MODULE_UNKNOWN},
	{NULL, "DPI0_SEL", 0, (0), DISP_MODULE_UNKNOWN},
	{NULL, "TOP_LVDSPLL_D2", 0, (0), DISP_MODULE_UNKNOWN},
	{NULL, "TOP_LVDSPLL_D4", 0, (0), DISP_MODULE_UNKNOWN},
	{NULL, "TOP_LVDSPLL_D8", 0, (0), DISP_MODULE_UNKNOWN},
	{NULL, "TOP_LVDSPLL_D16", 0, (0), DISP_MODULE_UNKNOWN},
...
int ddp_clk_set_rate(enum DDP_CLK_ID id, unsigned long rate)
{
	int ret = 0;
	...
	if (id >= MAX_DISP_CLK_CNT || id < 0) {
		DDPERR(
			"DISPSYS CLK id=%d is more than MAX_DISP_CLK_CNT or negative\n",
			id);
		return -1;
	}
	...
	return clk_set_rate(ddp_clks[id].pclk, rate);
}


unsigned long ddp_clk_get_rate(enum DDP_CLK_ID id)
{
	pr_info("%s, clkid = %d\n", __func__, id);

	if (disp_helper_get_stage() != DISP_HELPER_STAGE_NORMAL) {
		DDPERR("disp_helper_get_stage() != DISP_HELPER_STAGE_NORMAL\n");
		return 0;
	}
	...
	return clk_get_rate(ddp_clks[id].pclk);
}
...
int ddp_main_modules_clk_on(void)
{
	...
	/*pm_runtime_enable(&drv_dev->dev);*/
	pm_runtime_get_sync(&drv_dev->dev);
...
int ddp_main_modules_clk_off(void)
{
	...
	pm_runtime_put_sync(&drv_dev->dev);
	/*pm_runtime_disable(&drv_dev->dev);*/
