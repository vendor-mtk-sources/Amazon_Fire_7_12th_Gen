...
bool is_fb_freed;
struct rw_semaphore fb_free_lock;
#endif
...
void _primary_path_unlock(const char *caller)
{
	...
	if (mutex_time_period > 300000000) {
		DISPWARN("mutex_release_timeout1 <%lld ns>\n",
			mutex_time_period);
	}
	...
	if ((mutex_time_period < 300000000 && mutex_time_period1 > 300000000) ||
	   (mutex_time_period < 300000000 && mutex_time_period1 < 0)) {
		DISPWARN("mutex_release_timeout2 <%lld ns>,<%lld ns>\n",
	...
	unsigned int userdata)
{
	...
	if (_should_flush_cmdq_config_handle()) {
		if (g_to_nonsec == true) {
			pr_info("[SVP]cmdq sync flush in trigger\n");
			_cmdq_flush_config_handle(1, callback, userdata);
			if (primary_display_is_video_mode())
...
#ifdef FREE_FB_BUFFER
	if (down_write_trylock(&fb_free_lock)) {
		if (!is_fb_freed) {
			cmdqBackupReadSlot(pgc->ovl_config_fb_cnt, 0, &is_config_fb);
			if (!is_config_fb) {
				DISPWARN("free the frame buffer after kernel init");
				primary_display_free_fb_buf();
				is_fb_freed = true;
			}
		}
		up_write(&fb_free_lock);
	}
#endif
...
#ifdef FREE_FB_BUFFER
	init_cmdq_slots(&(pgc->ovl_config_fb_cnt), 1, 1);
	init_rwsem(&fb_free_lock);
#endif
...
extern void m4u_dma_unmap_region_nocheck(struct device *dev, dma_addr_t addr,
...
int primary_display_free_fb_buf(void)
{
	...
	if (!is_fb_freed) {
		...
		free_reserved_area((void *)va_start, (void *)va_end, 0xFF,
				   "fbmem");
		m4u_dma_unmap_region_nocheck(&drv_dev->dev,
				pgc->framebuffer_pa, vramsize, 0, 0);
		primary_display_set_frame_buffer_address(0, 0, 0);
		fb_base  = 0x0;
		vramsize = 0x0;
		DISPCHECK("reserve_framebuffer_free framebuffer_mva = 0x%lx, size 0x%x\n",
...
int primary_display_suspend(void)
{
	...
	if (dpmgr_path_is_busy(pgc->dpmgr_handle)) {
		...
		if (event_ret == 0) {
			...
			DISPERR("wait frame done was interrupted, event_ret=%d\n", event_ret);
			ret = -1;
		}
	}
...
{
	if (pgc->plcm == NULL) {
		DISPERR("%s lcm handle is null\n", __func__);
		return;
	}

	DISPDBG("primary_backlight_power_off enter\n");
	disp_lcm_set_backlight_power_off(pgc->plcm);
}
			     ...
			     struct cmdqRecStruct *cmdq_handle)
{
...
#ifdef FREE_FB_BUFFER
		down_read(&fb_free_lock);
		if (!is_fb_freed && !ovl_is_config_fb) {
			...
			DISPMSG("[%d/%d] ovl_is_config_fb=%d, id=%d/%d, 0x%p/%d, fb=0x%p/%d\n", i, cfg->input_layer_num,
					ovl_is_config_fb, ovl_cfg->layer, ovl_cfg->layer_en, (void *)ovl_cfg->addr, layer_size,
					(void *)pgc->framebuffer_mva, (int)vramsize);
		}
		up_read(&fb_free_lock);
#endif
	}
#ifdef FREE_FB_BUFFER
	down_read(&fb_free_lock);
	if (!is_fb_freed && !ovl_is_config_fb) {
		DISPWARN("%s trigger to release framebuffer 0x%p.\n", __func__, (void *)pgc->framebuffer_mva);
		cmdqRecBackupUpdateSlot(cmdq_handle, pgc->ovl_config_fb_cnt, 0, ovl_is_config_fb);
#if 0
		{
			struct OVL_BASIC_STRUCT ovlInfo[4+2/*DISP_HW_MAX_LAYER*/];
			memset(ovlInfo, 0, sizeof(ovlInfo));
			ovl_get_info(DISP_MODULE_OVL0, &ovlInfo);
			ovl_get_info(DISP_MODULE_OVL0_2L, &ovlInfo[4]);
			for (i = 0; i < 6; i++)
				DISPMSG("Overlay layer[%d] en=%d, addr=0x%p, %dX%d\n",
						ovlInfo[i].layer, ovlInfo[i].layer_en, (void *)ovlInfo[i].addr,
						ovlInfo[i].src_w, ovlInfo[i].src_h);
		}
#endif
	}
	up_read(&fb_free_lock);
	...
	for (i = 0; i < cfg->input_layer_num; i++) {
		unsigned int last_fence, cur_fence, last_sub;
		struct disp_input_config *input_cfg = &cfg->input_cfg[i];

		layer = input_cfg->layer_id;

		cmdqBackupReadSlot(pgc->cur_config_fence, layer, &last_fence);
		cmdqBackupReadSlot(pgc->subtractor_when_free, layer, &last_sub);
		cur_fence = input_cfg->next_buff_idx;

		if (cur_fence != -1 && ((cur_fence - last_fence) > (last_sub & 0xFFFF)))
			data_config->sbch_enable = 0;
	}
...
#ifdef CONFIG_MTK_ONLY_SINGLE_DISPLAY
		...
		unsigned int size, unsigned char force_update)
{
	...
	ret = cmdqRecCreate(CMDQ_SCENARIO_DISP_ESD_CHECK,
		&cmdq_handle_backlight);
	if (ret) {
		DISPERR("fail to create primary cmdq handle\n");
		return -1;
	}
	...
	if (primary_display_is_video_mode()) {
		...
		/* 2.stop dsi vdo mode */
		dpmgr_path_build_cmdq(pgc->dpmgr_handle, cmdq_handle_backlight,
				      CMDQ_STOP_VDO_MODE, 0);

		/* set cabc cmd*/
		do {
			ddp_dsi_write_lcm_register_cmdq(DISP_MODULE_DSI0,
				cmdq_handle_backlight, para_tbl,
				index, force_update);
		} while (++index < size);
		...
		dpmgr_path_trigger(pgc->dpmgr_handle,
			cmdq_handle_backlight, CMDQ_ENABLE);

		/* mutex sof wait*/
		ddp_mutex_set_sof_wait(dpmgr_path_get_mutex(pgc->dpmgr_handle),
			cmdq_handle_backlight, 0);
...
int read_lcm_lp_by_cmdq(struct LCM_setting_table_V4 *para_tbl,
		unsigned int size,
		unsigned int count,
		unsigned char force_update)
{
	...
	struct rx_data rx_data2 = {0x00};
	struct rx_data rx_data3 = {0x00};
	...
	ret = cmdqRecCreate(CMDQ_SCENARIO_DISP_ESD_CHECK,
		&cmdq_handle_backlight);
	if (ret) {
		DISPERR("fail to create cmdq handle for backlight\n");
		return -1;
	}
	cmdqBackupAllocateSlot(&read_Slot, 4);
	for (i = 0; i < 4; i++)
	...
	if (primary_display_is_video_mode()) {
		...
		/* 2.stop dsi vdo mode */
		dpmgr_path_build_cmdq(phandle, cmdq_handle_backlight,
			CMDQ_STOP_VDO_MODE, 0);

		/* 3. set cabc command */
		for (index = 0; index < size; index++) {
			flag = para_tbl[index].flag;
			if (flag) { /* Read */
				ddp_dsi_read_lcm_register_cmdq(DISP_MODULE_DSI0,
					&read_Slot,
					cmdq_handle_backlight,
					para_tbl,
					index,
				...
				ddp_dsi_write_lcm_register_cmdq(
					DISP_MODULE_DSI0,
					cmdq_handle_backlight,
					para_tbl,
					index,
		...
		/* 5.trigger path */
		dpmgr_path_trigger(phandle, cmdq_handle_backlight, CMDQ_ENABLE);

		/* mutex sof wait*/
		ddp_mutex_set_sof_wait(dpmgr_path_get_mutex(phandle),
			cmdq_handle_backlight, 0);

		/* 6.sync flush instruction */
		cmdqRecFlush(cmdq_handle_backlight);
	} else {
		DISPERR("Not support cmd mode\n");
	}

	for (i = 0; i < 1; i++) {
		cmdqBackupReadSlot(read_Slot, 2*i, (uint32_t *)&rx_data0);
		cmdqBackupReadSlot(read_Slot, 2*i+1, (uint32_t *)&rx_data1);
		cmdqBackupReadSlot(read_Slot, 2*i+2, (uint32_t *)&rx_data2);
		cmdqBackupReadSlot(read_Slot, 2*i+3, (uint32_t *)&rx_data3);
		packet_type = rx_data0.byte0;

		DISPCHECK("packet_type = 0x%x\n", packet_type);
		DISPCHECK("rx_data0: b0=0x%x, b1=0x%x, b2=0x%x, b3=0x%x\n",
			rx_data0.byte0, rx_data0.byte1, rx_data0.byte2,
			rx_data0.byte3);
		DISPCHECK("rx_data1: b0=0x%x, b1=0x%x, b2=0x%x, b3=0x%x\n",
			rx_data1.byte0, rx_data1.byte1, rx_data1.byte2,
			rx_data1.byte3);
		DISPCHECK("rx_data2: b0=0x%x, b1=0x%x, b2=0x%x, b3=0x%x\n",
			rx_data2.byte0, rx_data2.byte1, rx_data2.byte2,
			rx_data2.byte3);
		DISPCHECK("rx_data3: b0=0x%x, b1=0x%x, b2=0x%x, b3=0x%x\n",
			rx_data3.byte0, rx_data3.byte1, rx_data3.byte2,
			rx_data3.byte3);
		...
		if (packet_type == 0x1A || packet_type == 0x1C) {
			...
			DISPERR("packet_type = 0x%x\n", packet_type);
		}

		if (packet_type == 0x1A || packet_type == 0x1C) {
			recv_data_cnt = rx_data0.byte1 + rx_data0.byte2 * 16;
			if (recv_data_cnt > 10) {
				DISPCHECK(
			...
			if (recv_data_cnt <= 4) {
				memcpy((void *)reg_value_buf, (void *)&rx_data1, recv_data_cnt);
			} else if (recv_data_cnt <= 8) {
				memcpy((void *)reg_value_buf, (void *)&rx_data1, 4);
				memcpy((void *)reg_value_buf + 4, (void *)&rx_data2,
					recv_data_cnt - 4);
			} else {
				memcpy((void *)reg_value_buf, (void *)&rx_data1, 4);
				memcpy((void *)reg_value_buf + 4, (void *)&rx_data2, 4);
				memcpy((void *)reg_value_buf + 8, (void *)&rx_data3,
					recv_data_cnt - 8);
			}
		} else if (packet_type == 0x11 || packet_type == 0x12 ||
			packet_type == 0x21 || packet_type == 0x22) {
			...
			if (recv_data_cnt > buffer_size) {
				DISPCHECK(
					...
					buffer_size);
				recv_data_cnt = buffer_size;
				memcpy((void *)reg_value_buf, (void *)&rx_data0.byte1,
					recv_data_cnt);
			} else {
				memcpy((void *)reg_value_buf, (void *)&rx_data0.byte1,
					recv_data_cnt);
			...
			DISPERR("Read return type is non-recognite: 0x%x\n",
				  packet_type);
		}
	}

	/* 7.destroy read_Slot and cmdq_handle_backlight thread */
	cmdqBackupFreeSlot(read_Slot);
	cmdqRecDestroy(cmdq_handle_backlight);

	return rx_data0.byte1;
}
...
int primary_display_read_lcm_cmdq(
		struct LCM_setting_table_V4 *para_tbl,
		unsigned int size,
		unsigned int count,
		unsigned char force_update)
{
	...
	if (pgc->state == DISP_SLEPT) {
		...
		primary_display_idlemgr_kick(__func__, 0);
		if (primary_display_cmdq_enabled())
			ret = read_lcm_lp_by_cmdq(para_tbl, size,
				count, force_update);
	}
	return ret;
}
	...
	UINT32 config_value)
{
	...
	if (!strcmp(name, "PM_CLK")) {
		...
		if ((config_value >= 0) && (config_value <= 8))
