...
static unsigned long clipped_freq = -1;
...
{
/* TBD, need MN confirm */
#define CA35_4CORE_REF_POWER	931 /* mW  */
#define CA35_REF_FREQ	2001000	/* KHz */
...
static void init_mt_cpu_dvfs(struct mt_cpu_dvfs *p)
{
...
static int setup_power_table(struct mt_cpu_dvfs *p)
{
	...
	for (i = 0; i < p->nr_opp_tbl; i++) {
		for (j = 0; j < possible_cpu; j++) {
			/* only create table for 4-core */
			if (pwr_eff_tbl[i][j] == 0 && j == 3)
			...
			for (i = 0; i < p->nr_opp_tbl * possible_cpu; i++) {
				...
				if (p->power_tbl[i].cpufreq_power
					...
					if (p->power_tbl[i].cpufreq_power
						...
						/*invalid power level*/
						limited_max_ncpu =
						p->power_tbl[i-1].cpufreq_ncpu;
						limited_max_freq =
						p->power_tbl[i-1].cpufreq_khz;
					}
					break;
				}
			}
