...
TRACE_EVENT(VPU__D2D_enter,
	TP_PROTO(int core, int algo_id, int apu_freq),
	TP_ARGS(core, algo_id, apu_freq),
	TP_STRUCT__entry(
		__field(int, core)
		__field(int, algo_id)
		__field(int, apu_freq)
		),
	TP_fast_assign(
		__entry->core = core;
		__entry->algo_id = algo_id;
		__entry->apu_freq = apu_freq;
	),
	TP_printk("_id=c%da%d, dsp%d_freq=%d", __entry->core,
	__entry->algo_id, __entry->core, __entry->apu_freq)
);
...
TRACE_EVENT(VPU__DVFS,
	TP_PROTO(int vcore_opp, int apu_freq, int apu_if_freq),
	TP_ARGS(vcore_opp, apu_freq, apu_if_freq),
	TP_STRUCT__entry(
		__field(int, vcore_opp)
		__field(int, apu_freq)
		__field(int, apu_if_freq)
		),
	TP_fast_assign(
		__entry->vcore_opp = vcore_opp;
		__entry->apu_freq = apu_freq;
		__entry->apu_if_freq = apu_if_freq;
	),
	TP_printk(
		"vcore_opp=%d, apu_freq=%d,apu_if_freq=%d",
		__entry->vcore_opp,
		__entry->apu_freq,
		__entry->apu_if_freq)
);
