...
#endif
{
	...
	do {
		if (_pwrap_timeout_ns(start_time_ns, timeout_ns)) {
			pr_debug("state_idle timeout\n");
			pwrap_dump_ap_register();
			return E_PWR_WAIT_IDLE_TIMEOUT;
		}
		reg_rdata = WRAP_RD32(wacs_register);
		if (GET_WACS2_INIT_DONE2(reg_rdata) != WACS_INIT_DONE) {
			pr_debug("init isn't finished\n");
				       ...
				       unsigned int wdata, unsigned int *rdata)
{
	...
	if (write == 0) {
		...
		if (return_value != 0) {
			pr_debug("fsm_vldclr fail, ret=%d\n", return_value);
			return_value += 1;
			goto FAIL;
		}
...
static void pwrap_logging_at_isr(void)
{
	...
	if (__ratelimit(&ratelimit)) {
		...
		WRAP_WR32(PMIC_WRAP_MONITOR_CTRL_0, 0x5); /* reenable */
#else
		/* Matching mode and Continue
		 * recording after interrupt trigger
		 */
		WRAP_WR32(PMIC_WRAP_MONITOR_CTRL_0, 0x1); /* reenable */
#endif
...
{
	unsigned int int0_flg = 0, int1_flg = 0;
	int ret = 0;
	...
	if ((int0_flg & 0x01) == 0x01) {
		...
		ret = snprintf(str, 50, "[PWRAP] CRC = 0x%x",
			WRAP_RD32(PMIC_WRAP_SIG_ERRVAL));
		if (ret < 0)
			pr_notice("snprintf fail, ret=%d\n", ret);
...
static int __init pwrap_hal_init(void)
{
	...
	if (!pwrap_node) {
		pr_debug("PWRAP get node failed\n");
		return -ENODEV;
	}
...
void mt_pmic_wrap_eint_clr(int offset)
{
	if ((offset < 0) || (offset > 3))
		pr_debug("clear EINT flag error, only 0-3 bit\n");
	else
