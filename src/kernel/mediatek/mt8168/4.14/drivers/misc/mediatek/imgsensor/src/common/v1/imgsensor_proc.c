	...
	loff_t *data)
{
	...
	if (sscanf(
		...
		ret = snprintf(
		    ...
		    sensorReg.RegData);
		if (ret < 0) {
			pr_debug("Reg_Debug2 error, ret %d\n", ret);
			return ret;
		}
		...
		if (ret < 0) {
			pr_debug("Reg_Debug2 2 error, ret %d\n", ret);
			return ret;
		}
	}
...
static int imgsensor_proc_status_info_read(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "%s\n", status_info);
	if (ret < 0) {
		pr_debug("status_info_read error, ret %d\n", ret);
		return ret;
	}
	return 0;
};
...
enum IMGSENSOR_RETURN imgsensor_proc_init(void)
{
	...
	proc_create(
		"driver/pdaf_type", 0664, NULL,
		&fcamera_proc_fops_set_pdaf_type);
	proc_create(PROC_SENSOR_STAT, 0664, NULL,
