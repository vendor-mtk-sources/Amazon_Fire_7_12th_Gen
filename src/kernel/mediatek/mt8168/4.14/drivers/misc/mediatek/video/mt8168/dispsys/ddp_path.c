...
#ifndef CONFIG_MTK_ONLY_SINGLE_DISPLAY
...
static struct sel_s sel_out_map[] = {
	...
	{DISP_MODULE_RSZ0_VIRT1, {DISP_MODULE_DSI0,
		DISP_MODULE_COLOR0, DISP_MODULE_CCORR0, DISP_MODULE_DPI}, 0, 0},

	/* DISP_RDMA1_SOUT_SEL */
	{DISP_MODULE_RDMA1, {DISP_MODULE_DSI0,
		DISP_MODULE_DPI}, 0, 0},
};
...
static struct sel_s sel_in_map[] = {
	...
	{DISP_MODULE_DSI0, {DISP_MODULE_RSZ0_VIRT1,
	 DISP_MODULE_DITHER0, DISP_MODULE_RDMA1, -1},
	 0, 0},
	...
	/* DPI_SEL_IN */
	{DISP_MODULE_DPI, {DISP_MODULE_RDMA1,
	 DISP_MODULE_DITHER0, DISP_MODULE_RSZ0_VIRT1, -1},
	 0, 0},
};
...
int ddp_path_init(void)
{
	...
	sel_in_map[6].reg =
		(unsigned long *)DISP_REG_CONFIG_DPI0_SEL_IN;
...
void ddp_connect_path(enum DDP_SCENARIO_ENUM scenario, void *handle)
{
	...
	if (scenario == DDP_SCENARIO_PRIMARY_ALL) {
		...
		ddp_connect_path_l(
			module_list_scenario[DDP_SCENARIO_SUB_RDMA1_DISP],
			handle);
		ddp_connect_path_l(
...
int ddp_path_top_clock_on(void)
{
	...
	/*use power-domain(MT8168_POWER_DOMAIN_DISP) instead of mtcmos*/
	...
	/*pm_runtime_get_sync larb0 will open smi common/com0/com1/larb0 clk*/
	for (larb_idx = 0; larb_idx < DISP_LARB_COUNT; larb_idx++)
		pm_runtime_get_sync(&iommu_dev->larb_pdev[larb_idx]->dev);
#endif
...
int ddp_path_top_clock_off(void)
{
	...
	/*pm_runtime_put_sync larb0 will close smi common/com0/com1/larb0 clk*/
	if (iommu_dev != NULL) {
		for (larb_idx = 0; larb_idx < DISP_LARB_COUNT; larb_idx++)
			pm_runtime_put_sync(
				&iommu_dev->larb_pdev[larb_idx]->dev);
	}
#endif
