...
/********************************************************************
 * camera_isp.c - MT8168 Linux ISP Device Driver
 *
 * DESCRIPTION:
...
#include <m4u.h>
//#define EP_NO_CLKMGR
#define M4U_CALLBACK 0    /* m4u cb function not supported */
...
static struct dentry *mtkisp_dbgfs;
unsigned int Enable_ISP_LOG;
/*******************************************************************
 *
 ********************************************************************/
/* For other projects.*/
...
/* ///////////////////////////////////////////////////////////////// */
#define CAMSV_BASE_ADDR         0x15050000
#define CAMSV_REG_RANGE         (0x8000)
...
struct ISP_CLK_STRUCT {
	struct clk *CG_MMSYS_SMI_COMMON;
	struct clk *CG_MMSYS_SMI_COMM0;
	struct clk *CG_MMSYS_SMI_COMM1;
	struct clk *CG_MMSYS_SMI_IMG;
	struct clk *CG_CAMSYS_LARB2;
	struct clk *CG_CAMSYS_CAM;
	struct clk *CG_CAMSYS_CAMTG;
	struct clk *CG_CAMSYS_CAM_SENIF;
	struct clk *CG_CAMSYS_CAMSV0;
	struct clk *CG_CAMSYS_CAMSV1;
};
...
#define ISP_CAMSV_ADDR (gISPSYS_Reg[ISP_CAMSV_BASE_ADDR])
#else
...
#define ISP_REG_ADDR_IMGO_SV_CROP (ISP_CAMSV_ADDR + 0x224)
...
#define ISP_REG_ADDR_IMGO_SV_D_CROP (ISP_CAMSV_ADDR + 0x244)
...
#define ISP_REG_ADDR_CAMSV_TG_VF_CON            (ISP_CAMSV_ADDR +     0x414)
#define ISP_REG_ADDR_CAMSV_TG2_VF_CON           (ISP_CAMSV_ADDR +     0xC14)
#define CAMSV_TG_REG_ADDR_GRAB_W                (ISP_CAMSV_ADDR +     0x418)
#define CAMSV_TG2_REG_ADDR_GRAB_W               (ISP_CAMSV_ADDR +     0xC18)
#define CAMSV_TG_REG_ADDR_GRAB_H                (ISP_CAMSV_ADDR +     0x41C)
#define CAMSV_TG2_REG_ADDR_GRAB_H               (ISP_CAMSV_ADDR +     0xC1C)
...
#define _MAX_SUPPORT_P2_BURSTQ_NUM_ 2
...
struct ISP_EDBUF_STRUCT {
	unsigned long processID; /* caller process ID */
...
#define IRQ_LOG_KEEPER(irq, ppb, logT, fmt, ...) do {\
		...
		ptr     = pDes = (char *)&(gSvLog[irq]._str[ppb][logT]\
		...
		if (avaLen > 1) {\
			...
			if ('\0' !=     gSvLog[irq]._str[ppb][logT][str_leng -\
				1]) { \
				log_inf("(%d)(%d)log str over flow", irq,\
					logT);\
			} \
			while (*ptr++ != '\0') {\
				(*ptr2)++;\
...
static unsigned int ISP_DumpDmaDeepDbg(void)
{
	if (g_bDmaERR_p1) {
		...
		log_err("IMGI:0x%x,BPCI:0x%x,LSCI=0x%x,UFDI=0x%x,LCEI=0x%x,imgo=0x%x,",
			g_DmaErr_p1[0], g_DmaErr_p1[1], g_DmaErr_p1[2],
			g_DmaErr_p1[3], g_DmaErr_p1[4], g_DmaErr_p1[5]);
		log_err("rrzo:0x%x,lcso:0x%x,esfko:0x%x,aao:0x%x,ufeo:0x%x",
...
#if     0
	if (g_bDmaERR_p2) {
		log_err("vipi:0x%x,VIPI:0x%x,VIP2I=0x%x,VIP3I=0x%x,MFBO=0x%x,",
		...
		log_err("IMG3BO=0x%x,IMG3CO:0x%x,IMG2O:0x%x,IMG3O:0x%x,FEO:0x%x",
...
bool ISP_chkModuleSetting(void)
{
	...
	bmx_width = ((cam_bmx_crop >> 16) & 0x1fff) -
		(cam_bmx_crop & 0x1fff) + 1;
...
/*	CAM_FLK_OFST.FLK_OFST_Y.value <= sggmux_v_size.value */
		if ((((FLK_NUM_X * FLK_SIZE_X) + FLK_OFST_X) > sggmux_h_size) ||
			...
			sggmux_v_size)) {
			/*Error */
			pr_info("HwRWCtrl:: Flicker Error: bmx_enable(%d), sgg_sel(%d), grab_width(%d),",
				bmx_enable, sgg_sel, grab_width);
			pr_info(" grab_height(%d), bmx_width(%d), bmx_height(%d)!!",
				grab_height, bmx_width, bmx_height);
			pr_info("HwRWCtrl:: Flicker Error: (CAM_FLK_NUM.FLK_NUM_X.value(%d) *",
				FLK_NUM_X);
			pr_info(" CAM_FLK_SIZE.FLK_SIZE_X.value(%d)) + CAM_FLK_OFST.FLK_OFST_X.value(%d)",
				FLK_SIZE_X, FLK_OFST_X);
			pr_info(" <= sggmux_h_size.value(%d)!!",
				sggmux_h_size);
			pr_info("HwRWCtrl:: Flicker Error: (CAM_FLK_NUM.FLK_NUM_Y.value(%d) *",
				FLK_NUM_Y);
			pr_info(" CAM_FLK_SIZE.FLK_SIZE_Y.value(%d)) + CAM_FLK_OFST.FLK_OFST_Y.value(%d)",
				FLK_SIZE_Y, FLK_OFST_Y);
			pr_info(" <= sggmux_v_size.value(%d)!!",
				sggmux_v_size);
		}

/*4. flko_xs == ((CAM_FLK_NUM.FLK_NUM_X.value * CAM_FLK_NUM.FLK_NUM_Y.value * */
			/*CAM_FLK_SIZE.FLK_SIZE_Y.value * 2) - 1) */
		if (esfko_xsize != ((FLK_NUM_X * FLK_NUM_Y * FLK_SIZE_Y * 2)
			- 1)) {
			/*Error */
			pr_info("HwRWCtrl:: Flicker Error: flko_xs(%d) must be equal ((CAM_FLK_NUM.FLK_NUM_X.value(%d) *",
				esfko_xsize, FLK_NUM_X);
			pr_info(" CAM_FLK_NUM.FLK_NUM_Y.value(%d) * CAM_FLK_SIZE.FLK_SIZE_Y.value(%d) * 2) - 1)!!",
				FLK_NUM_Y, FLK_SIZE_Y);
		}
		/*Check AF setting */

	/* under twin case, sgg_sel won't be 0 , so ,*/
	/* don't need to take into consideration at twin case */
	...
	/* win_num_x = CAM_READ_BITS(this->m_pDrv->getPhyObj(),*/
	/* CAM_AF_BLK_1,AF_BLK_XNUM); */
	/* win_num_y = CAM_READ_BITS(this->m_pDrv->getPhyObj(),*/
	/* CAM_AF_BLK_1,AF_BLK_YNUM); */
...
/*Check AE setting */
...
#endif
		{
			...
			if ((cam_aao_ysize + 1) != 1)
				pr_info("Error HwRWCtrl::AAO_YSIZE(%d) must be equal 1 !!",
					cam_aao_ysize);
			if ((AAO_XSIZE + 1) !=
			    (AWB_W_HNUM * AWB_W_VNUM * 5 +
			    (histogramen_num << 8)))
				pr_info("Error HwRWCtrl::AAO_XSIZE(%d) = AWB_W_HNUM(%d)*AWB_W_VNUM(%d)*5 +",
					AAO_XSIZE, AWB_W_HNUM, AWB_W_VNUM);
				pr_info(" (how many histogram enable(%d)(AE_HST0/1/2/3_EN))*2*128 !!",
					histogramen_num);
		}
		...
		if (AAO_InWidth < (AWB_W_HNUM * AWB_W_HPIT + AWB_W_HORG)) {
			/*Error */
			pr_info("Error HwRWCtrl:: bmx_enable(%d), bmx_width(%d),",
				bmx_enable, bmx_width);
			pr_info(" bmx_height(%d), grab_width(%d), grab_height(%d)!!",
				bmx_height, grab_width, grab_height);
			pr_info("Error HwRWCtrl:: input frame width(%d) >= AWB_W_HNUM(%d)",
				 AAO_InWidth, AWB_W_HNUM);
			pr_info(" * AWB_W_HPIT(%d) + AWB_W_HORG(%d) !!",
		...
		if (AAO_InHeight < (AWB_W_VNUM * AWB_W_VPIT + AWB_W_VORG)) {
			...
			pr_info("Error HwRWCtrl:: input frame height(%d) >=",
				AAO_InHeight);
			pr_info(" AWB_W_VNUM(%d)	* AWB_W_VPIT(%d) + AWB_W_VORG(%d) !!",
		...
		if ((EIS_NUM_VWIN > 8) || (EIS_NUM_HWIN > 4) ||
			...
			pr_info("EIS Error, 2. The max vertical window size is 8, EIS_NUM_VWIN(%d)!!",
				EIS_NUM_VWIN);
			pr_info("EIS Error, 3. EIS_MF_OFFSET.EIS_RP_VOFST or EIS_RP_HOFST  > 16!!,");
			pr_info(" EIS_RP_VOFST(%d), EIS_RP_HOFST(%d)",
		...
		if ((EIS_WIN_HSIZE < (((EIS_NUM_HRP + 1) << 4) + 1)) ||
			...
			/*Error */
			pr_info("EIS Error, 8. EIS_MB_INTERVAL.EIS_WIN_HSIZE >= (EIS_PREP_ME_CTRL1.EIS_NUM_HRP+1)*");
			pr_info("16+1!!, EIS_WIN_HSIZE:%d, EIS_NUM_HRP:%d",
				EIS_WIN_HSIZE, EIS_NUM_HRP);
			pr_info("EIS Error, 9. EIS_MB_INTERVAL.EIS_WIN_VSIZE >= (EIS_PREP_ME_CTRL1.EIS_NUM_VRP+1)*");
			pr_info("16+1!!, EIS_WIN_VSIZE:%d, EIS_NUM_VRP:%d",
 ...
 */
		if ((((EIS_RP_HOFST + ((EIS_WIN_HSIZE - 1) * (EIS_NUM_HWIN - 1))
			...
			|| (((EIS_RP_VOFST + ((EIS_WIN_VSIZE - 1) *
			(EIS_NUM_VWIN - 1)) + (EIS_NUM_VRP << 4)) *
			EIS_OP_VERT) >= EIS_IMG_HEIGHT)) {
			/*Error */
			pr_info("EIS Error, 10. (EIS_MB_OFFSET.EIS_RP_HOFST(%d) + ((EIS_MB_INTERVAL.EIS_WIN_HSIZE(%d)-1)*",
				EIS_RP_HOFST, EIS_WIN_HSIZE);
			pr_info("(EIS_PREP_ME_CTRL1.EIS_NUM_HWIN(%d)-1))+EIS_PREP_ME_CTRL1.EIS_NUM_HRP(%d)*16)*",
				EIS_NUM_HWIN, EIS_NUM_HRP);
			pr_info("EIS_PREP_ME_CTRL1.EIS_OP_HORI(%d) < EIS_IMAGE_CTRL.WIDTH(%d)!!",
				EIS_OP_HORI, EIS_IMG_WIDTH);
			pr_info("EIS Error, 10. (EIS_MB_OFFSET.EIS_RP_VOFST(%d) + ((EIS_MB_INTERVAL.EIS_WIN_VSIZE(%d)-1)*",
				EIS_RP_VOFST, EIS_WIN_VSIZE);
			pr_info("(EIS_PREP_ME_CTRL1.EIS_NUM_VWIN(%d)-1))+EIS_PREP_ME_CTRL1.EIS_NUM_VRP(%d)*16)*",
				EIS_NUM_VWIN, EIS_NUM_VRP);
			pr_info("EIS_PREP_ME_CTRL1.EIS_OP_VERT(%d) < EIS_IMG_HEIGHT.WIDTH(%d)!!",
				EIS_OP_VERT, EIS_IMG_HEIGHT);
		}
		...
		case 2:
			if (bmx_enable == MTRUE) {
				if (((EIS_SUBG_EN == 1) && (EIS_IMG_WIDTH !=
					(rrz_out_width + rrz_d_out_width) / 2))
					|| ((EIS_SUBG_EN == 0) &&
					(EIS_IMG_WIDTH != (rrz_out_width +
					rrz_d_out_width))) ||
		...
		if (bError == MTRUE) {
			pr_info("EIS Error, 4. EIS_IMAGE_CTRL.WIDTH = EIS input image width but if");
			pr_info("(two_pix mode) EIS_IMAGE_CTRL.WIDTH = input image width/2!!\n");
			...
			pr_info("EIS_IMG_WIDTH:%d, EIS_IMG_HEIGHT:%d, rrz_out_width:%d, rrz_out_height:%d, hbin2_en:%d",
				EIS_IMG_WIDTH, EIS_IMG_HEIGHT,
				rrz_out_width, rrz_out_height, hbin2_en);
	...
	if (cam_tg2_vf_con & 0x01) {
		...
		grab_width =
			((cam_tg2_sen_grab_pxl >> 16) & 0x7fff) -
			(cam_tg2_sen_grab_pxl & 0x7fff);
		grab_height =
			((cam_tg2_sen_grab_lin >> 16) & 0x1fff) -
...
/*2. The min vertical window size is 4. (AF_WIN_HT/AF_FLO_HT_x) */
		...
		if ((AF_WIN_WD < 8) || (AF_FLO_WD_1 < 8) || (AF_FLO_WD_2 < 8) ||
			(AF_FLO_WD_3 < 8) || (AF_WIN_HT < 4) ||
			(AF_FLO_HT_1 < 4) || (AF_FLO_HT_2 < 4) ||
			(AF_FLO_HT_3 < 4)) {
			pr_info("HwRWCtrl:: AF_D Error: The min horizontal window size is 8.");
			pr_info("(AF_WIN_WD(%d), AF_FLO_WD_1(%d), AF_FLO_WD_2(%d), AF_FLO_WD_3(%d))!!",
				AF_WIN_WD, AF_FLO_WD_1, AF_FLO_WD_2,
				AF_FLO_WD_3);
			pr_info("HwRWCtrl:: AF_D Error: The min vertical window size is 4.");
			pr_info("(AF_WIN_HT(%d), AF_FLO_HT_1(%d), AF_FLO_HT_2(%d), AF_FLO_HT_3(%d))!!",
...
/*	(AF_WINX_x/AF_FLO_WINX_x/AF_WIN_WD/AF_FLO_WD_x) */
		if (((AF_WINX_0 % 2) != 0) || ((AF_WINX_1 % 2) != 0) ||
			...
			pr_info("HwRWCtrl:: AF_D Error: AF_WINX_0(%d)/AF_WINX_1(%d)/AF_WINX_2(%d)/",
				AF_WINX_0, AF_WINX_1, AF_WINX_2);
			pr_info("AF_WINX_3(%d)/AF_WINX_4(%d)/AF_WINX_5(%d)!!",
				AF_WINX_3, AF_WINX_4, AF_WINX_5);
			pr_info("HwRWCtrl:: AF_D Error: AF_WINY_0(%d)/AF_WINY_1(%d)/AF_WINY_2(%d)/",
				AF_WINY_0, AF_WINY_1, AF_WINY_2);
			pr_info("AF_WINY_3(%d)/AF_WINY_4(%d)/AF_WINY_5(%d)!!",
...
/*11. Horizontal size of floating window must be multiples of 2 */
		if (((AF_FLO_WINX_1 % 2) != 0) || ((AF_FLO_WINX_2 % 2) != 0)
		    ...
		    || ((AF_FLO_HT_3 % 2) != 0)) {
			pr_info("HwRWCtrl:: AF_D Error: Horizontal start position of floating window");
			pr_info(" must be multiples of 2!!");
...
/*6. Vertical size of each normal window is limited to 511*2 (AF_WIN_HT) */
		if ((AF_WIN_WD > 1020) || (AF_WIN_HT > 1022)) {
			pr_info("HwRWCtrl:: AF_D Error: Horizontal size of each normal window is");
			pr_info(" limited to 510*2 (AF_WIN_WD(%d))!!",
				AF_WIN_WD);
			pr_info("HwRWCtrl:: AF_D Error: Vertical size of each normal window is");
			pr_info(" limited to 511*2 (AF_WIN_HT(%d))!!",
				AF_WIN_HT);
		}
		...
		if ((AF_FLO_WD_1 > 4094) || (AF_FLO_WD_2 > 4094) ||
			(AF_FLO_WD_3 > 4094) ||
			(AF_FLO_HT_1 > 4095) || (AF_FLO_HT_2 > 4095) ||
			(AF_FLO_HT_3 > 4095))
...
/*16. "CAM_AF_WINX_x.AF_WINX_x + CAM_AF_SIZE.AF_WIN_WD <= input frame width"
 * for a valid window
 *17. "CAM_AF_WINY_x.AF_WINY_x + CAM_AF_SIZE.AF_WIN_HT <= input frame height"
 * for a valid window
 *14. If AF window is aligned to image left/right boundary, minimum horizontal
 * size should be
 ...
 */
		...
		if ((AF_WINX_0 < 16) || (AF_LeftOverWidth < 16)) {
			if (AF_WIN_WD < (24 * (1 << AF_DECI))) {
				pr_info("HwRWCtrl:: AF_D Error: minimum horizontal size should be larger than");
				pr_info("24*(1<<AF_DECI_1) for non-zero 24-tap FIR statistics result!!");
				pr_info("HwRWCtrl:: AF_D Error: AF_WINX_0(%d), AF_DECI(%d),",
					AF_WINX_0, AF_DECI);
				pr_info(" AF_LeftOverWidth(%d), AF_IMAGE_WD(%d), AF_WIN_WD(%d)!!",
					AF_LeftOverWidth, AF_IMAGE_WD,
					AF_WIN_WD);
			}
		}
...
#endif
		{
			...
			if ((cam_aao_ysize + 1) != 1)
				log_err("Error HwRWCtrl::AAO_D_YSIZE(%d) must be equal 1 !!",
					cam_aao_ysize);
			if ((AAO_XSIZE + 1) !=
				(AWB_W_HNUM * AWB_W_VNUM * 5 +
				(histogramen_num << 8)))
				pr_info("Error HwRWCtrl::AAO_D_XSIZE(%d) = AWB_W_HNUM(%d)*AWB_W_VNUM(%d)*5 +",
					AAO_XSIZE, AWB_W_HNUM, AWB_W_VNUM);
				pr_info(" (how many histogram enable(%d)(AE_HST0/1/2/3_EN))*2*128 !!",
					histogramen_num);
		}
		...
		if (AAO_InWidth < (AWB_W_HNUM * AWB_W_HPIT + AWB_W_HORG)) {
			/*Error */
			pr_info("Error HwRWCtrl:: AWB_D bmx_enable(%d), bmx_width(%d),",
				bmx_enable, bmx_width);
			pr_info("	bmx_height(%d), grab_width(%d), grab_height(%d)!!",
				bmx_height, grab_width, grab_height);
			pr_info("Error HwRWCtrl:: AWB_D input frame width(%d) >= ",
				AAO_InWidth);
			pr_info("AWB_W_HNUM(%d) * AWB_W_HPIT(%d) + AWB_W_HORG(%d) !!",
		...
		if (AAO_InHeight < (AWB_W_VNUM * AWB_W_VPIT + AWB_W_VORG)) {
			...
			pr_info("	bmx_height(%d), grab_width(%d), grab_height(%d)!!",
				bmx_height, grab_width, grab_height);
			pr_info("Error HwRWCtrl:: AWB_D input frame height(%d) >=",
				AAO_InHeight);
			pr_info("AWB_W_VNUM(%d) * AWB_W_VPIT(%d) + AWB_W_VORG(%d) !!",
				AWB_W_VNUM, AWB_W_VPIT, AWB_W_VORG);
		}
...
#if 0
static void _ISP_RegTest(unsigned long addr)
{
	unsigned int value, value1;

	value = ISP_RD32(addr);
	ISP_WR32(addr, 0xFFFFFFFF);

	value1 = ISP_RD32(addr);
	ISP_WR32(addr, value);

	log_inf("\taddr=0x%08lX , \t0x%08X \t0x%08X",
		addr&0xFFFF, value, value1);

}

static void ISP_RegTest(void)
{
	ISP_WR32(IMGSYS_REG_CG_CLR, 0xFFFF);
	/*_ISP_RegTest(ISP_IMGSYS_BASE);*/

	_ISP_RegTest(ISP_ADDR+0x0);
	_ISP_RegTest(ISP_ADDR+0x410);
	_ISP_RegTest(ISP_ADDR+0x4F0);
	_ISP_RegTest(ISP_ADDR+0x500);
	_ISP_RegTest(ISP_ADDR+0x530);
	_ISP_RegTest(ISP_ADDR+0x550);
	_ISP_RegTest(ISP_ADDR+0x570);
	_ISP_RegTest(ISP_ADDR+0x580);
	_ISP_RegTest(ISP_ADDR+0x590);
	_ISP_RegTest(ISP_ADDR+0x5B0);
	_ISP_RegTest(ISP_ADDR+0x650);
	_ISP_RegTest(ISP_ADDR+0x6A0);
	_ISP_RegTest(ISP_ADDR+0x6B0);
	_ISP_RegTest(ISP_ADDR+0x770);
	_ISP_RegTest(ISP_ADDR+0x774);
	_ISP_RegTest(ISP_ADDR+0x7A0);
	_ISP_RegTest(ISP_ADDR+0x800);
	_ISP_RegTest(ISP_ADDR+0x880);
	_ISP_RegTest(ISP_ADDR+0x8A0);
	_ISP_RegTest(ISP_ADDR+0x910);
	_ISP_RegTest(ISP_ADDR+0x920);
	_ISP_RegTest(ISP_ADDR+0x948);
	_ISP_RegTest(ISP_ADDR+0xA00);
	_ISP_RegTest(ISP_ADDR+0xA1C);
	_ISP_RegTest(ISP_ADDR+0xAA4);
	_ISP_RegTest(ISP_ADDR+0xB00);
	_ISP_RegTest(ISP_ADDR+0xBA0);
	_ISP_RegTest(ISP_ADDR+0xC00);
	_ISP_RegTest(ISP_ADDR+0xD10);
	_ISP_RegTest(ISP_ADDR+0xDC0);
	_ISP_RegTest(ISP_ADDR+0xE00);
	_ISP_RegTest(ISP_ADDR+0xF40);
	_ISP_RegTest(ISP_ADDR+0xF60);
	_ISP_RegTest(ISP_ADDR+0xFC0);
	_ISP_RegTest(ISP_ADDR+0x1000);
	_ISP_RegTest(ISP_ADDR+0x1800);
	_ISP_RegTest(ISP_ADDR+0x1FF0);
	_ISP_RegTest(ISP_ADDR+0x3200);
	_ISP_RegTest(ISP_ADDR+0x3600);
	_ISP_RegTest(ISP_ADDR+0x2410);
	_ISP_RegTest(ISP_ADDR+0x2500);
	_ISP_RegTest(ISP_ADDR+0x2530);
	_ISP_RegTest(ISP_ADDR+0x2550);
	_ISP_RegTest(ISP_ADDR+0x2800);
	_ISP_RegTest(ISP_ADDR+0x2A00);
	_ISP_RegTest(ISP_ADDR+0x2C00);
	_ISP_RegTest(ISP_ADDR+0x2E00);

	_ISP_RegTest(ISP_CAMSV_ADDR);
	_ISP_RegTest(ISP_CAMSV_ADDR+0x200);
	_ISP_RegTest(ISP_CAMSV_ADDR+0x410);
	_ISP_RegTest(ISP_CAMSV_ADDR+0x50C);
...
static signed int ISP_DumpReg(void)
{
	...
	if (((pstRTBuf->ring_buf[_imgo_].active) &&
		((p1_fbc[0].Bits.FB_NUM - p1_fbc[0].Bits.FBC_CNT) == 0)) ||
		((pstRTBuf->ring_buf[_rrzo_].active) &&
		((p1_fbc[1].Bits.FB_NUM - p1_fbc[1].Bits.FBC_CNT) == 0)) ||
		((pstRTBuf->ring_buf[_imgo_d_].active) &&
		((p1_fbc[2].Bits.FB_NUM - p1_fbc[2].Bits.FBC_CNT) == 0)) ||
		((pstRTBuf->ring_buf[_rrzo_d_].active) &&
		((p1_fbc[3].Bits.FB_NUM - p1_fbc[3].Bits.FBC_CNT) == 0))) {

		log_err("[0x%08X %08X],[0x%08X %08X],[0x%08X %08X],[0x%08X %08X][0x%08X %08X],[0x%08X %08X],[0x%08X %08X]\n",
		(unsigned int)(ISP_TPIPE_ADDR + 0x4),
		(unsigned int)ISP_RD32(ISP_ADDR + 0x4),
		(unsigned int)(ISP_TPIPE_ADDR + 0x8),
		(unsigned int)ISP_RD32(ISP_ADDR + 0x8),
		(unsigned int)(ISP_TPIPE_ADDR + 0x10),
		(unsigned int)ISP_RD32(ISP_ADDR + 0x10),
		(unsigned int)(ISP_TPIPE_ADDR + 0x14),
		(unsigned int)ISP_RD32(ISP_ADDR + 0x14),
		(unsigned int)(ISP_TPIPE_ADDR + 0x70),
		(unsigned int)ISP_RD32(ISP_ADDR + 0x70),
		(unsigned int)(ISP_TPIPE_ADDR + 0x74),
		(unsigned int)ISP_RD32(ISP_ADDR + 0x74),
		(unsigned int)(ISP_TPIPE_ADDR + 0x78),
		(unsigned int)ISP_RD32(ISP_ADDR + 0x78));
		log_err("[0x%08X %08X],[0x%08X %08X],[0x%08X %08X],[0x%08X %08X][0x%08X %08X],[0x%08X %08X],[0x%08X %08X]\n",
		(unsigned int)(ISP_TPIPE_ADDR + 0x418),
		(unsigned int)ISP_RD32(ISP_ADDR + 0x418),
		(unsigned int)(ISP_TPIPE_ADDR + 0x41C),
		(unsigned int)ISP_RD32(ISP_ADDR + 0x41C),
		(unsigned int)(ISP_TPIPE_ADDR + 0x448),
		(unsigned int)ISP_RD32(ISP_ADDR + 0x448),
		(unsigned int)(ISP_TPIPE_ADDR + 0x44C),
		(unsigned int)ISP_RD32(ISP_ADDR + 0x44C),
		...
		log_err("[0x%08X %08X],[0x%08X %08X],[0x%08X %08X],[0x%08X %08X][0x%08X %08X],[0x%08X %08X],[0x%08X %08X]\n",
		(unsigned int)(ISP_TPIPE_ADDR + 0x244C),
		(unsigned int)ISP_RD32(ISP_ADDR + 0x244C),
		(unsigned int)(ISP_TPIPE_ADDR + 0x4128),
		(unsigned int)ISP_RD32(ISP_ADDR + 0x4128),
		(unsigned int)(ISP_TPIPE_ADDR + 0x4134),
		(unsigned int)ISP_RD32(ISP_ADDR + 0x4134),
		(unsigned int)(ISP_TPIPE_ADDR + 0x43B4),
		(unsigned int)ISP_RD32(ISP_ADDR + 0x43B4),
		(unsigned int)(ISP_TPIPE_ADDR + 0x4528),
		(unsigned int)ISP_RD32(ISP_ADDR + 0x4528),
		(unsigned int)(ISP_TPIPE_ADDR + 0x4534),
		(unsigned int)ISP_RD32(ISP_ADDR + 0x4534),
		(unsigned int)(ISP_TPIPE_ADDR + 0x47B4),
		(unsigned int)ISP_RD32(ISP_ADDR + 0x47B4));
		goto EXIT_DumpReg;
	}
...
#if     0
	g_bDmaERR_p1 = g_bDmaERR_p1_d = g_bDmaERR_p2 = g_bDmaERR_deepDump
		= MTRUE;
	ISP_DumpDmaDeepDbg();
#endif
	/* TG1 */
	...
	ISP_WR32(ISP_IMGSYS_BASE + 0x4160, 0);
	...
	log_err("0x%08X	%08X (0x150075f4=031e)",
		(unsigned int)(ISP_IMGSYS_BASE + 0x4164),
		(unsigned int)ISP_RD32(ISP_IMGSYS_BASE + 0x4164));
	ISP_WR32(ISP_IMGSYS_BASE + 0x75f4, 0x80101);
	log_err("0x%08X	%08X (0x150075f4=0x80101)",
		(unsigned int)(ISP_IMGSYS_BASE + 0x4164),
		(unsigned int)ISP_RD32(ISP_IMGSYS_BASE + 0x4164));
	ISP_WR32(ISP_IMGSYS_BASE + 0x75f4, 0x00101);
	log_err("0x%08X	%08X (0x150075f4=0x00101)",
		(unsigned int)(ISP_IMGSYS_BASE + 0x4164),
		(unsigned int)ISP_RD32(ISP_IMGSYS_BASE + 0x4164));
	ISP_WR32(ISP_IMGSYS_BASE + 0x75f4, 0x000201);
	log_err("0x%08X	%08X (0x150075f4=0x000201)",
		(unsigned int)(ISP_IMGSYS_BASE + 0x4164),
		(unsigned int)ISP_RD32(ISP_IMGSYS_BASE + 0x4164));
	ISP_WR32(ISP_IMGSYS_BASE + 0x75f4, 0x010201);
	log_err("0x%08X	%08X (0x150075f4=0x010201)",
		(unsigned int)(ISP_IMGSYS_BASE + 0x4164),
		(unsigned int)ISP_RD32(ISP_IMGSYS_BASE + 0x4164));
	regValue = (unsigned int)ISP_RD32(ISP_IMGSYS_BASE + 0x4164);
	if (regValue&0x1)
		log_err("[DMA]IMGI fifo_overflow");
	if (regValue&0x2)
		log_err("[DMA]IMGI fifo_underflow");
	if (regValue&0x4)
		log_err("[DMA]IMGI fifo_empty");
	if (regValue&0x8)
		log_err("[DMA]IMGI fifo_full");
	if (regValue&0x10)
		log_err("[DMA]IMGI fifo_aempty");
	if (regValue&0x20)
		log_err("[DMA]IMGI fifo_afull");
	log_err("[DMA]IMGI debug_sel=0x01030B wa(FIFO_ptr)=0x%08x",
		(regValue&0x000FFC00)>>10);
	log_err("[DMA]IMGI debug_sel=0x01030B ra(FIFO_ptr)=0x%08x",
		(regValue&0xFFF00000)>>20);

	log_err("IMG2O_DEBUG Related");
	ISP_WR32(ISP_IMGSYS_BASE + 0x75f4, 0x1C1e);
	log_err("0x%08X	%08X (0x150075f4=0x1C1e)",
		(unsigned int)(ISP_IMGSYS_BASE + 0x4164),
		(unsigned int)ISP_RD32(ISP_IMGSYS_BASE + 0x4164));
	ISP_WR32(ISP_IMGSYS_BASE + 0x75f4, 0x1D1e);
	log_err("0x%08X	%08X (0x150075f4=0x1D1e)",
		(unsigned int)(ISP_IMGSYS_BASE + 0x4164),
		(unsigned int)ISP_RD32(ISP_IMGSYS_BASE + 0x4164));
	ISP_WR32(ISP_IMGSYS_BASE + 0x75f4, 0x1E1e);
	log_err("0x%08X	%08X (0x150075f4=0x1E1e)",
		(unsigned int)(ISP_IMGSYS_BASE + 0x4164),
		(unsigned int)ISP_RD32(ISP_IMGSYS_BASE + 0x4164));
	ISP_WR32(ISP_IMGSYS_BASE + 0x75f4, 0x1F1e);
	log_err("0x%08X	%08X (0x150075f4=0x1F1e)",
		(unsigned int)(ISP_IMGSYS_BASE + 0x4164),
		(unsigned int)ISP_RD32(ISP_IMGSYS_BASE + 0x4164));
	/* yuv */
...
{
	int ret;
	/* must keep this clk open order: CG_SCP_SYS_MM0->*/
	/* CG_SCP_SYS_ISP -> ISP clk */

	ret = pm_runtime_get_sync(cam_isp_devs->dev);
	if (ret < 0)
		log_err("cannot pm_runtime_get_sync(ISP), error(%d)\n",
			ret);

	ret = clk_prepare_enable(isp_clk.CG_MMSYS_SMI_COMMON);
	if (ret)
		log_err("cannot get CG_MMSYS_SMI_COMMON clock\n");

	ret = clk_prepare_enable(isp_clk.CG_MMSYS_SMI_COMM0);
	if (ret)
		log_err("cannot get CG_MMSYS_SMI_COMM0 clock\n");

	ret = clk_prepare_enable(isp_clk.CG_MMSYS_SMI_COMM1);
	if (ret)
		log_err("cannot get CG_MMSYS_SMI_COMM1 clock\n");

	ret = clk_prepare_enable(isp_clk.CG_MMSYS_SMI_IMG);
	if (ret)
		log_err("cannot get CG_MMSYS_SMI_IMG clock\n");

	ret = clk_prepare_enable(isp_clk.CG_CAMSYS_LARB2);
	if (ret)
		log_err("cannot get CG_CAMSYS_LARB2 clock\n");

	ret = clk_prepare_enable(isp_clk.CG_CAMSYS_CAM);
	if (ret)
		log_err("cannot get CG_CAMSYS_CAM clock\n");

	ret = clk_prepare_enable(isp_clk.CG_CAMSYS_CAMTG);
	if (ret)
		log_err("cannot get CG_CAMSYS_CAMTG clock\n");

	ret = clk_prepare_enable(isp_clk.CG_CAMSYS_CAM_SENIF);
	if (ret)
		log_err("cannot get CG_CAMSYS_CAM_SENIF clock\n");

	ret = clk_prepare_enable(isp_clk.CG_CAMSYS_CAMSV0);
	if (ret)
		log_err("cannot get CG_CAMSYS_CAMSV0 clock\n");

	ret = clk_prepare_enable(isp_clk.CG_CAMSYS_CAMSV1);
	if (ret)
		log_err("cannot get CG_CAMSYS_CAMSV1 clock\n");

	log_dbg("ISP power/clock on");
...
static inline void Disable_Unprepare_ccf_clock(void)
{
	...
	/* ISP clk -> CG_SCP_SYS_ISP -> CG_SCP_SYS_MM0 */

	clk_disable_unprepare(isp_clk.CG_MMSYS_SMI_COMMON);
	clk_disable_unprepare(isp_clk.CG_MMSYS_SMI_COMM0);
	clk_disable_unprepare(isp_clk.CG_MMSYS_SMI_COMM1);
	clk_disable_unprepare(isp_clk.CG_MMSYS_SMI_IMG);
	clk_disable_unprepare(isp_clk.CG_CAMSYS_LARB2);
	clk_disable_unprepare(isp_clk.CG_CAMSYS_CAM);
	clk_disable_unprepare(isp_clk.CG_CAMSYS_CAMTG);
	clk_disable_unprepare(isp_clk.CG_CAMSYS_CAM_SENIF);
	clk_disable_unprepare(isp_clk.CG_CAMSYS_CAMSV0);
	clk_disable_unprepare(isp_clk.CG_CAMSYS_CAMSV1);

	pm_runtime_put_sync(cam_isp_devs->dev);

	/* disable through smi API : CG_IMG_LARB2_SMI,*/
	/* CG_MM_SMI_COMMON*/

	log_dbg("ISP power/clock off");
}
...
static void ISP_EnableClock(bool En)
{
	...
	if (En) {
		...
		/* *MMSYS_CG_CLR0 = 0x00000003;*CLK_CFG_7 = *CLK_CFG_7 |*/
		/* 0x02000000;*CAM_CTL_CLK_EN = 0x00000009; */
...
static inline void ISP_Reset(signed int rst_path)
{
	...
	if (rst_path == ISP_REG_SW_CTL_RST_CAM_P1) {
		...
		setReg = (Reg & (~ISP_REG_SW_CTL_SW_RST_P2_MASK)) | (0x00 &
			ISP_REG_SW_CTL_SW_RST_P2_MASK);
		ISP_WR32(ISP_REG_ADDR_CAM_SW_CTL, setReg);

#else
		ISP_WR32(ISP_REG_ADDR_CAM_SW_CTL,
			ISP_REG_SW_CTL_SW_RST_P2_TRIG);
		ISP_WR32(ISP_REG_ADDR_CAM_SW_CTL, 0);
		do {
			Reg = ISP_RD32(ISP_REG_ADDR_CAM_SW_CTL);
...
static signed int ISP_ReadReg(struct ISP_REG_IO_STRUCT *pRegIo)
{
	...
	if ((pRegIo->Count > (PAGE_SIZE/sizeof(unsigned int))) ||
		(pRegIo->Count == 0)
		|| (pRegIo->pData == NULL)) {
		log_err("ERROR %s pRegIo->pData is NULL or pRegIo->Count error:%d\n",
	...
	if (copy_from_user(pDataArray, (void *)pRegIo->pData,
		...
		log_err("ERROR %s copy_from_user failed\n", __func__);
		Ret = -EFAULT;
		goto EXIT;
	}
	pData = pDataArray;
	for (i = 0; i < pRegIo->Count; i++) {
		...
		if ((ISP_ADDR_CAMINF + pData->Addr >= ISP_ADDR_CAMINF)
			&& (ISP_ADDR_CAMINF + pData->Addr < (ISP_ADDR_CAMINF +
			ISP_REG_RANGE))) {
	...
	if (copy_to_user((void *)pRegIo->pData, pDataArray,
		(pRegIo->Count)*sizeof(struct ISP_REG_STRUCT)) != 0) {
		log_err("ERROR %s copy_to_user failed\n", __func__);
		Ret = -EFAULT;
		goto EXIT;
	}
	/*      */
...
/* 'ISP_BufWrite_Init' defined but not used [-Wunused-function] */
#if 0
...
{
	unsigned int i;
	unsigned int size = ISP_BUF_SIZE_WRITE * sizeof(unsigned char);
	/*      */
	if (IspInfo.DebugMask & ISP_DBG_BUF_WRITE)
		log_dbg("- E.");
			      ...
			      struct ISP_REG_STRUCT *pData)
{
	...
	for (i = 0; i < ISP_BUF_WRITE_AMOUNT; i++) {
		if (IspInfo.BufInfo.Write[i].Status == ISP_BUF_STATUS_EMPTY) {
			...
			/*      */
			if (copy_from_user
				((unsigned char *)(IspInfo.BufInfo.Write[i]
				.pData),
				(unsigned char *)pData, Size) != 0) {
				log_err("copy_from_user	failed");
				return false;
...
static long ISP_REF_CNT_CTRL_FUNC(unsigned long Param)
{
	...
	if (copy_from_user(&ref_cnt_ctrl, (void __user *)Param,
		sizeof(struct ISP_REF_CNT_CTRL_STRUCT)) ==
	    0) {
...
static int vsync_cnt[_IRQ_MAX] = {0, 0};


/* record lost p1_done or not, 1 for lost p1_done. 0 for normal ,*/
/* 2 for last working buffer. */
	...
	struct ISP_RT_BUF_INFO_STRUCT *prt_buf_info)
{
	...
	if ((rt_dma >= 0) && (rt_dma < _rt_dma_max_)) {
		...
		log_err("[rtbc][ENQUE]:real time buffer number error:rt_dma(%d)",
			rt_dma);
		Ret = -EFAULT;
		return Ret;
	}
	...
	if (pstRTBuf->ring_buf[rt_dma].empty_count == 1) {
		if (_imgo_ == rt_dma) {
			...
			ISP_WR32(ISP_REG_ADDR_IMGO_D_BASE_ADDR,
				 pstRTBuf->ring_buf[rt_dma].data[index]
				 .base_pAddr);
		...
		/* if(IspInfo.DebugMask & ISP_DBG_BUF_CTRL)     { */
#if (ISP_RAW_D_SUPPORT == 1)
		log_dbg("[rtbc][ENQUE]:dma(%d),base_pAddr(0x%x)/imgo(0x%x)/rrzo(0x%x)/",
		 rt_dma,
		 pstRTBuf->ring_buf[rt_dma].data[index].base_pAddr,
		...
		("imgo_d(0x%x)/rrzo_d(0x%x)/empty_count(%d)",
		 ...
		 pstRTBuf->ring_buf[rt_dma].empty_count);
#else
		log_dbg("[rtbc][ENQUE]:dma(%d),base_pAddr(0x%x)/imgo(0x%x)/rrzo(0x%x)/camsv_imgo(0x%x)/empty_count(%d)",
		 rt_dma,
		 pstRTBuf->ring_buf[rt_dma].data[index].base_pAddr,
		 ISP_RD32(ISP_REG_ADDR_IMGO_BASE_ADDR),
		 ISP_RD32(ISP_REG_ADDR_RRZO_BASE_ADDR),
			...
			unsigned int VF_2, unsigned int VF_3, unsigned int VF_4)
{
	...
	for (z = 0; z < _rt_dma_max_; z++) {
		ret = sprintf(str2, "%d_", pstRTBuf->ring_buf[z].active);
		if (ret < 0)
			log_err("ret sprintf fail");
	...
	if (VF_1) {
		...
		str[0] = '\0';
		for (z = 0; z < ISP_RT_BUF_SIZE; z++) {
			ret = sprintf(str2, "%d_", dma_en_recorder[dma][z]);
			if (ret < 0)
				log_err("ret sprintf fail");
			strncat(str, str2, strLeng - strlen(str));
		}
		...
		str[0] = '\0';
		for (z = 0; z < ISP_RT_BUF_SIZE; z++) {
			ret = sprintf(str2, "%d_", mFwRcnt.INC[_IRQ][z]);
			if (ret < 0)
				log_err("ret sprintf fail");
			strncat(str, str2, strLeng - strlen(str));
		}
	...
	if (VF_2) {
		...
		str[0] = '\0';
		for (z = 0; z < ISP_RT_BUF_SIZE; z++) {
			ret = sprintf(str2, "%d_", mFwRcnt.INC[_IRQ_D][z]);
			if (ret < 0)
				log_err("ret sprintf fail");
			strncat(str, str2, strLeng - strlen(str));
		}
		...
		dma = _camsv_imgo_;
		{
			...
			for (z = 0; z < ISP_RT_BUF_SIZE; z++) {
				...
				if (ret < 0)
					log_err("ret sprintf fail");
				strncat(str, str2, strLeng - strlen(str));
				log_inf("magic queue=0x%x", g_magQue[_camsv_1]
					[z]);
			}
			log_inf("%s", str);
			log_inf("================================\n");
	...
	if (pdeque_buf->count) {
		...
		for (i = 0; i < pdeque_buf->count; i++) {
			pdeque_buf->data[i].memID =
				pstRTBuf->ring_buf[rt_dma].data[index + i]
				.memID;
...
/* update vAddr for deciding which buffer had been changed to dummy frame. */
	...
	/*	ring_buf[rt_dma].data[i].base_vAddr);*/
				...
				if (i == ISP_RT_BUF_SIZE) {
					...
					log_err("[rtbc][replace3]can't find Addr(0x%x)\n",
					rt_buf_info.base_pAddr);
					spin_unlock_irqrestore(
					&(IspInfo.SpinLockIrq[irqT_Lock]),
					flags);
			...
			if (pstRTBuf->ring_buf[rt_dma].active == MTRUE) {
				if (rt_buf_ctrl.ex_data_ptr != 0) {
					if (((p1_fbc[rt_dma].Bits.FB_NUM -
						...
						- p1_fbc[rt_dma].Bits.FBC_CNT)
						== 0)) {
						mFwRcnt.bLoadBaseAddr[irqT]
							= MTRUE;
					}
				}
				dma_en_recorder[rt_dma][mFwRcnt.DMA_IDX[rt_dma]]
					= MTRUE;
				mFwRcnt.DMA_IDX[rt_dma]
					= (++mFwRcnt.DMA_IDX[rt_dma] >=
					ISP_RT_BUF_SIZE) ?
	...
	/**/
		...
		if (pstRTBuf->ring_buf[rt_dma].active == MTRUE) {
			...
			mFwRcnt.DMA_IDX[rt_dma] =
				...
				for (z = 0; z < ISP_RT_BUF_SIZE; z++) {
					if (dma_en_recorder[rt_dma][mFwRcnt
						.rdIdx[irqT]]) {
 ...
 */
#if (ISP_RAW_D_SUPPORT == 1)
	log_dbg("[rtbc][ENQUE]:dma(%d),PA(0x%x),O(0x%x),ZO(0x%x),O_D(0x%x),ZO_D(0x%x)\n",
		rt_dma,
		rt_buf_info.base_pAddr,
		ISP_RD32(ISP_REG_ADDR_IMGO_BASE_ADDR),
		ISP_RD32(ISP_REG_ADDR_RRZO_BASE_ADDR),
...
#else
		log_dbg("[rtbc][ENQUE]:dma(%d),PA(0x%x),O(0x%x),ZO(0x%x),camsv(0x%x/0x%x)fps(%d/%d/%d/%d)us,rtctrl_%d\n",
		rt_dma,
		rt_buf_info
		.base_pAddr,
		ISP_RD32(ISP_REG_ADDR_IMGO_BASE_ADDR),
		...
		case ISP_RT_BUF_CTRL_DEQUE:
			...
			reg_val = ISP_RD32(ISP_REG_ADDR_TG_VF_CON);
			reg_val2 = ISP_RD32(ISP_REG_ADDR_TG2_VF_CON);
			camsv_reg_cal[0] = ISP_RD32(
				ISP_REG_ADDR_CAMSV_TG_VF_CON);
			camsv_reg_cal[1] = ISP_RD32(
				ISP_REG_ADDR_CAMSV_TG2_VF_CON);
	/* VF start already     */
	/* spin_lock_irqsave(&(IspInfo.SpinLockIrq[irqT_Lock]), flags); */
	...
	/* p1_fbc[rt_dma].Bits.WCNT - 1;       //WCNT = [1,2,..]     */
			...
			/* unsigned int out; */
					...
					deque_buf.data[i].image.m_num_0 =
						pstRTBuf->ring_buf[rt_dma].data
						[iBuf + i].image.m_num_0;
					deque_buf.data[i].image.frm_cnt =
						pstRTBuf->ring_buf[rt_dma].data
						[iBuf + i].image.frm_cnt;
	...
	/*log_inf("[rtbc][deque][m_num]:d(%d),fc(0x%x),*/
	 ...
	 */
				...
				if (prv_tstamp_s[rt_dma] != 0) {
					...
					prv_tstamp_s[rt_dma] = deque_buf.data
						[i].timeStampS;
					prv_tstamp_us[rt_dma] = deque_buf.data
						[i].timeStampUs;
	...
	/* spin_unlock_irqrestore(&(IspInfo.SpinLockRTBC),g_Flash_SpinLock); */
		...
		case ISP_RT_BUF_CTRL_CUR_STATUS:
			...
			camsv_reg_cal[0] = ISP_RD32(
				ISP_REG_ADDR_CAMSV_TG_VF_CON) & 0x1;
			camsv_reg_cal[1] = ISP_RD32(
				ISP_REG_ADDR_CAMSV_TG2_VF_CON) & 0x1;
	...
	/*      */
			...
			if (ISP_RTBC_BUF_FILLED ==
			    pstRTBuf->ring_buf[rt_dma].data[pstRTBuf->ring_buf
				[rt_dma].read_idx].bFilled) {
				bWaitBufRdy = 0;
			}
			...
			if (IspInfo.DebugMask & ISP_DBG_BUF_CTRL) {
				...
				IRQ_LOG_KEEPER(irqT, 0, _LOG_DBG,
					...
					pstRTBuf->ring_buf[rt_dma].total_count,
					bWaitBufRdy);
				for (z = 0; z < pstRTBuf->ring_buf[rt_dma]
					.total_count; z++)
...
#else
/* if ((_imgo_ == rt_dma)||(_rrzo_ == rt_dma)||(_imgo_d_ == rt_dma) */
/*	||(_rrzo_d_ == rt_dma)) */
/* active */
...

			{
				...
				if ((pstRTBuf->ring_buf[_imgo_].active ==
					MFALSE) && (pstRTBuf->ring_buf[_rrzo_]
					.active == MFALSE)) {
					out[0] = _IRQ;
				}
				if ((pstRTBuf->ring_buf[_imgo_d_].active ==
					MFALSE) && (pstRTBuf->ring_buf
		...
		for (ii = 0; ii < 4; ii++) {
			if (out[ii] != _IRQ_MAX) {
				...
				memset((void *)mFwRcnt.INC[out[ii]],
					0,
					sizeof(unsigned int) *
					ISP_RT_BUF_SIZE);
...
/* No drop */
	 ...
	 */
			for (i = 0;
				i < pstRTBuf->ring_buf[_working_dma]
				.total_count;
				i++) {
				/*      */
				if (curr_pa ==
...
if (IspInfo.DebugMask & ISP_DBG_INT_2) {
	IRQ_LOG_KEEPER(irqT, m_CurrentPPB, _LOG_INF,
			...
			pstRTBuf->ring_buf[ch_rrzo]
				.data[rrzo_idx]
				.rrzInfo.srcX =
				ISP_RD32(ISP_INNER_REG_ADDR_RRZ_D_HORI_INT_OFST)
				& 0x1FFF;
			pstRTBuf->ring_buf[ch_rrzo]
				.data[rrzo_idx]
				.rrzInfo.srcY =
				ISP_RD32(ISP_INNER_REG_ADDR_RRZ_D_VERT_INT_OFST)
					& 0x1FFF;
...
/* theoretically, it shout not be happened( wcnt     is inc. at p1_done)     */
		...
		if (dma == _camsv_imgo_) {
			...
			pstRTBuf->ring_buf[dma].data[camsv_imgo_idx]
				.image.xsize =
			...
			pstRTBuf->ring_buf[dma].data[camsv_imgo_idx]
				...
				((_tmp >> 16) & 0x1FFF) -
				(_tmp & 0x1FFF);
			pstRTBuf->ring_buf[dma].data[camsv_imgo_idx].image
				.stride =
				(ISP_RD32(ISP_REG_ADDR_IMGO_SV_STRIDE)
				& 0x3FFF);
			pstRTBuf->ring_buf[dma].data[camsv_imgo_idx].image
				.bus_size =
				((ISP_RD32(ISP_REG_ADDR_IMGO_SV_STRIDE) >> 16)
				& 0x03);
			pstRTBuf->ring_buf[dma].data[camsv_imgo_idx]
				.image.fmt =
			...
			pstRTBuf->ring_buf[dma].data[camsv_imgo_idx]
				.dmaoCrop.x =
				ISP_RD32(ISP_REG_ADDR_IMGO_SV_CROP)
				& 0x3fff;
			pstRTBuf->ring_buf[dma].data[camsv_imgo_idx]
				.dmaoCrop.y =
				(ISP_RD32(ISP_REG_ADDR_IMGO_SV_CROP) >> 16)
				& 0x1fff;
			pstRTBuf->ring_buf[dma].data[camsv_imgo_idx]
				.dmaoCrop.w =
	...
	/* For sync buffer with configFrame magic num in HAL1,
	 * first 3 buffer: mag# 0.
	 */
					pstRTBuf->ring_buf[dma].data
						[camsv_imgo_idx]
						.image.m_num_0 = 0;
				} else {
	/* For sync buffer with DmaCfg magic num in HAL3,
	 * first 3 buffer: mag# 1.
	 */
					pstRTBuf->ring_buf[dma].data
						[camsv_imgo_idx]
						.image.m_num_0 = 1;
				...
				if (enqueImd[_camsv_1] == MFALSE) {
					/* Choose current or last g_magQue
					 * because
					 * configFrame do not comes per-frame.
					 */
					if (g_magQue[_camsv_1][g_magQIdx
						[_camsv_1][1]] >=
	...
	/* HAL3 immed enque, magic number will be updated per-frame. */
					pstRTBuf->ring_buf[dma].data
						...
						g_magQue[_camsv_1]
						[g_magQIdx[_camsv_1][1]];
					log_dbg("camsv SOF:Ridx(%d), mag(0x%x)",
						g_magQIdx[_camsv_1][1],
						g_magQue[_camsv_1]
						[g_magQIdx[_camsv_1][1]]);
					g_magQIdx[_camsv_1][1]++;
			...
			pstRTBuf->ring_buf[dma].data[camsv_imgo_idx]
				.dmaoCrop.y =
				(ISP_RD32(ISP_REG_ADDR_IMGO_SV_D_CROP) >> 16)
				& 0x1fff;
			pstRTBuf->ring_buf[dma].data[camsv_imgo_idx]
				.dmaoCrop.w =
				(ISP_RD32(ISP_REG_ADDR_IMGO_SV_D_XSIZE) &
...
/* For sync buffer with DmaCfg magic num in HAL3, first 3 buffer: mag# 1. */
	...
	/* configFrame do not comes per-frame. */
					if (g_magQue[_camsv_2][g_magQIdx
						[_camsv_2][1]] >=
...
/* HAL3 immed enque, magic number will be updated per-frame. */
					pstRTBuf->ring_buf[dma].data
						[camsv_imgo_idx].image.m_num_0 =
						g_magQue[_camsv_2][g_magQIdx
						[_camsv_2][1]];
					log_dbg("camsv SOF:Ridx(%d), mag(0x%x)",
						g_magQIdx[_camsv_2][1],
						g_magQue[_camsv_2][g_magQIdx
				...
				unsigned long usec)
{
	...
	if ((_working_dma >= 0) && (_working_dma < _rt_dma_max_)) {
		;
	} else {
		log_err("_working_dma number error");
		return 0;
	}
...
#if     0
				loopCount++;
				if (loopCount > pstRTBuf->ring_buf[i_dma]
					.total_count) {
		...
		for (i = 0;
			...
			if (P2_EDBUF_MList_FirstBufIdx <=
				...
				for (i = P2_EDBUF_MList_FirstBufIdx;
					 ...
					 * of buffer1 have beend deque done
...
/*get in the first     for     loop */
	...
	if (idx == -1)
		log_err("Could not find	match buffer tag(%d) pid/cid/p2dupCQidx(%llu/%llu/%d)",
...
static signed int ISP_ED_BufQue_CTRL_FUNC(struct ISP_ED_BUFQUE_STRUCT param)
{
		...
		if (((P2_EDBUF_MList_LastBufIdx + 1) %
			...
			log_err("F/L(%d,%d),(%d_%d,%d), RF/C/L(%d,%d,%d),(%d,%d,%d)",
				...
				P2_EDBUF_RingList[P2_EDBUF_RList_LastBufIdx]
					.bufSts);
			log_err("p2 ring buffer list is full, drop frame.");
		}
		if (P2_EDBUF_MList_LastBufIdx != -1) {
#if 0
	...
	case ISP_ED_BUFQUE_CTRL_WAIT_DEQUE:
		...
		spin_unlock(&(SpinLockEDBufQueList));
		if (idx == -1) {
			log_err("Can't find match buffer(pid/cid %llu/%llu)to deque!",
		...
		if (restTime == 0) {
			log_err("Wait Deque fail, idx(%d) pID(%llu),cID(%llu)",
	...
	case ISP_ED_BUFQUE_CTRL_WAIT_FRAME:
		...
		if (idx == -1) {
			spin_unlock(&(SpinLockEDBufQueList));
			log_err("could not find	match buffer pID/cID(%llu/%llu)",
				param.processID, param.callerID);
		...
		if (P2_EDBUF_MgrList[idx].dequedNum ==
			...
			spin_unlock(&(SpinLockEDBufQueList));
			ret = 0;
			log_dbg("Frame is alreay dequeued, return user, pd(%lu/%lu),idx(%d)",
		...
		if (restTime == 0) {
			log_err("Dequeue Buffer	fail, rT(%d),idx(%d) pID(%llu),cID(%llu),p2SupportBNum(%d)\n",
	...
	case ISP_ED_BUFQUE_CTRL_CLAER_ALL:
		...
		break;
	default:
		log_err("Error: do not	support	this ctrl cmd(%d)", param.ctrl);
		break;
	}
	return ret;
}
...
static signed int ISP_GET_MARKtoQEURY_TIME(struct ISP_WAIT_IRQ_STRUCT *irqinfo)
{
	...
	if (irqinfo->UserInfo.Status & IspInfo.IrqInfo
		...
		if (irqinfo->TimeInfo.passedbySigcnt > 0) {
			...
			if ((int)(irqinfo->TimeInfo.tLastSig2GetSig_usec) < 0) {
				...
				if ((int)(irqinfo->TimeInfo.tLastSig2GetSig_sec)
					< 0)
					irqinfo->TimeInfo.tLastSig2GetSig_sec =
					0;
	...
	spin_unlock_irqrestore(&(IspInfo.SpinLockIrq[eIrq]), flags);
	log_dbg(" [%s] user/type/idx(%d/%d/%d),mark sec/usec (%d/%d),",
		__func__,
		...
		IspInfo.IrqInfo.MarkedTime_usec[irqinfo->UserInfo.UserKey]
		[irqinfo->UserInfo.Type][idx]);
	log_dbg("irq sec/usec (%d/%d),query sec/usec(%d/%d),sig(%d)\n",
...
static signed int ISP_FLUSH_IRQ_V3(struct ISP_WAIT_IRQ_STRUCT irqinfo)
{
	...
	/* 2. force     to wake up the user
	 * that are waiting for that signal
	 */
	wake_up_interruptible_all(&IspInfo.WaitQueueHead);
...
static signed int ISP_WaitIrq_v3(struct ISP_WAIT_IRQ_STRUCT *WaitIrq)
{
	...
	}
	{
		 ...
		 * & WaitIrq->Status);
		 */
					IspInfo.IrqInfo.Status[WaitIrq->UserInfo
						.UserKey]
						[WaitIrq->UserInfo.Type] &=
						(~WaitIrq->UserInfo.Status);
				}
...
/*   IspInfo_FrmB.IrqInfo.Status[WaitIrq->UserInfo.Type]); */
				IspInfo.IrqInfo.Status[WaitIrq->UserInfo
					.UserKey][WaitIrq->UserInfo.Type] = 0;
				spin_unlock_irqrestore(&(IspInfo
					.SpinLockIrq[eIrq]), flags);
			}
		}
	...
	if (Timeout == 0) {
		spin_lock_irqsave(&(IspInfo.SpinLockIrq[eIrq]), flags);
		log_err("v3 ERRRR WaitIrq Timeout(%d) Clear(%d),Type(%d)",
		 WaitIrq->Timeout, WaitIrq->Clear, WaitIrq->UserInfo.Type);
		log_err("IrqStatus(0x%08X),WaitStatus(0x%08X),Timeout(%d),userKey(%d)\n",
		 IspInfo.IrqInfo.Status[WaitIrq->UserInfo.UserKey]
		 [WaitIrq->UserInfo.Type],
		...
		/* TODO:  AF */
		if (WaitIrq->bDumpReg
		    || (strcmp((const char *)IrqUserKey_UserInfo
		    [WaitIrq->UserInfo.UserKey].userName,
		    "HwIRQ3A") == 0)) {
			ISP_DumpReg();
		}
	...
	if (WaitIrq->UserInfo.Type < ISP_IRQ_TYPE_INT_STATUSX &&
		...
		if (WaitIrq->UserInfo.Type == ISP_IRQ_TYPE_INT_P1_ST) {
			...
			/* eis meta     */
			WaitIrq->EisMeta.tLastSOF2P1done_sec =
				IspInfo.IrqInfo.Eismeta[WaitIrq->UserInfo.Type]
					[gEismetaRIdx]
					.tLastSOF2P1done_sec;
			WaitIrq->EisMeta.tLastSOF2P1done_usec =
				IspInfo.IrqInfo.Eismeta[WaitIrq->UserInfo.Type]
...
EXIT:
	/* 4. clear     mark flag /     reset marked time /   reset time
	 * related infor and passedby signal count
	 */
	spin_lock_irqsave(&(IspInfo.SpinLockIrq[eIrq]), flags);
	if (WaitIrq->UserInfo.Status &
	IspInfo.IrqInfo
	...
	if (IrqStatus_CAMSV & ISP_IRQ_CAMSV_STATUS_TG_SOF1_ST) {
		...
		if ((fbc.Bits.FB_NUM - fbc.Bits.FBC_CNT) == 0) {
			...
			if (lost_camsv_done_cnt == 2) {
				...
				for (k = 0;
					k < pstRTBuf->ring_buf[rt_dma]
					.total_count;
					k++) {

					if (pstRTBuf->ring_buf[rt_dma].data[k]
						.bFilled
						== ISP_RTBC_BUF_FILLED) {

						lost_camsv_done_cnt = 0;
						IRQ_LOG_KEEPER(_CAMSV_IRQ,
							m_CurrentPPB,
							_LOG_INF,
				...
				if (k < pstRTBuf->ring_buf[rt_dma]
					.total_count) {

					for (j = 0; j < ISP_IRQ_USER_MAX; j++) {
						IspInfo.IrqInfo.Status[j]
						  [ISP_IRQ_TYPE_INT_CAMSV] |=
					  ISP_IRQ_CAMSV_STATUS_PASS1_DON_ST;
					}
				}
		...
		if ((enqueImd[_camsv_1] == MFALSE) ||
			((enqueImd[_camsv_1] == MTRUE) &&
			(g_firstCnt[_camsv_1] < fbc.Bits.FB_NUM))) {
			/**/
			if (pstRTBuf->ring_buf[_camsv_imgo_].active) {
				if (pstRTBuf->ring_buf[_camsv_imgo_]
					.empty_count > 0) {
					IRQ_LOG_KEEPER(_CAMSV_IRQ, m_CurrentPPB,
						_LOG_INF,
						CAMSV_TAG "wr2Phy 0x%x,",
						pstRTBuf->ring_buf[_camsv_imgo_]
						.data[buf_idx].base_pAddr);
					ISP_WR32(ISP_REG_ADDR_IMGO_SV_BASE_ADDR,
						pstRTBuf->ring_buf[_camsv_imgo_]
						.data[buf_idx].base_pAddr);
				}
		...
		if (gSof_camsvdone[0] == 1)
			ISP_CAMSV_SOF_Buf_Get(_camsv_imgo_,
				fbc, curr_pa, sec, usec, MTRUE);
		else
			ISP_CAMSV_SOF_Buf_Get(_camsv_imgo_,
				fbc, curr_pa, sec, usec, MFALSE);
	}
...
static __tcmfunc irqreturn_t ISP_Irq_CAMSV2(signed int Irq, void *DeviceId)
{
	...
	if (IrqStatus_CAMSV2 & ISP_IRQ_CAMSV2_STATUS_TG_SOF1_ST) {
		...
		if ((enqueImd[_camsv_2] == MFALSE) ||
			((enqueImd[_camsv_2] == MTRUE) && (g_firstCnt[_camsv_2]
			< fbc.Bits.FB_NUM))) {
			if (pstRTBuf->ring_buf[_camsv2_imgo_].active &&
			...
			if (pstRTBuf->ring_buf[_camsv2_imgo_].active &&
				(fbc.Bits.FB_NUM - fbc.Bits.FBC_CNT) == 0) {
				IRQ_LOG_KEEPER(_CAMSV_D_IRQ, m_CurrentPPB,
					_LOG_INF,
					CAMSV2_TAG "drop wr2Phy_0x%x: ",
					pstRTBuf->ring_buf[_camsv2_imgo_]
					.data[pstRTBuf->ring_buf[_camsv2_imgo_]
					.start].base_pAddr);
				ISP_WR32(ISP_REG_ADDR_IMGO_SV_D_BASE_ADDR,
					pstRTBuf->ring_buf[_camsv2_imgo_]
					.data[pstRTBuf->ring_buf[_camsv2_imgo_]
		...
		if (IspInfo.DebugMask & ISP_DBG_INT) {
			...
			if (pstRTBuf->ring_buf[_camsv2_imgo_].active) {
				if ((_fbc_chk.Bits.WCNT - fbc.Bits.WCNT) != 0)
					IRQ_LOG_KEEPER(
					_CAMSV_D_IRQ, m_CurrentPPB, _LOG_INF,
		...
		if (gSof_camsvdone[1] == 1)
			ISP_CAMSV_SOF_Buf_Get(_camsv2_imgo_, fbc, curr_pa,
				sec, usec, MTRUE);
		else
			ISP_CAMSV_SOF_Buf_Get(_camsv2_imgo_, fbc, curr_pa,
				sec, usec, MFALSE);
 ...
 */
	...
	if (IrqStatus[ISP_IRQ_TYPE_INT_P1_ST] &
		...
		IspInfo.IrqInfo
			...
			(unsigned int)(usec);
		IspInfo.IrqInfo
			.LastestSigTime_sec[ISP_IRQ_TYPE_INT_P1_ST][10] =
			(unsigned int)(sec);
		gEismetaInSOF = 0;
...
/*      */
	...
	if (IrqStatus[ISP_IRQ_TYPE_INT_P1_ST] & ISP_IRQ_P1_STATUS_SOF1_INT_ST) {
		...
		if ((p1_fbc[_dmaport].Bits.FB_NUM -
			p1_fbc[_dmaport].Bits.FBC_CNT) == 0) {
			sof_pass1done[0] = 1;
#ifdef _rtbc_buf_que_2_0_
...
/* "lost p1Done ErrHandle\n"); */
	 ...
	 */
		...
		if ((gEismetaWIdx >= 0) && (gEismetaWIdx < EISMETA_RINGSIZE)) {
			IspInfo.IrqInfo
			    .Eismeta[ISP_IRQ_TYPE_INT_P1_ST][gEismetaWIdx]
				.tLastSOF2P1done_sec =
				(unsigned int)(sec);
			IspInfo.IrqInfo
			    .Eismeta[ISP_IRQ_TYPE_INT_P1_ST][gEismetaWIdx]
...
static long ISP_ioctl(struct file *pFile, unsigned int Cmd, unsigned long Param)
{
...
#if     1
			else { /* isp driver related operation in v1 is
				*  redirected
	...
	case ISP_DUMP_ISR_LOG:
		if (copy_from_user(DebugFlag, (void *)Param,
			...
			IRQ_LOG_PRINTER_PR_ERR(DebugFlag[0],
				currentPPB, _LOG_ERR);
		} else {
			log_err("copy_from_user	failed");
			Ret = -EFAULT;
	...
	case ISP_GET_VSYNC_CNT:
		if (copy_from_user(&DebugFlag[0], (void *)Param,
			...
			switch (DebugFlag[0]) {
			case _IRQ:
				DebugFlag[1] = vsync_cnt[_IRQ];
				break;
			case _IRQ_D:
				DebugFlag[1] = vsync_cnt[_IRQ_D];
				break;
			default:
	...
	case ISP_RESET_VSYNC_CNT:
		vsync_cnt[_IRQ] = vsync_cnt[_IRQ_D] = 0;
		break;

	case ISP_UPDATE_MAGNUM:
		if (copy_from_user(magInfo,
			(void *)Param, sizeof(magInfo)) == 0) {
			union CQ_RTBC_FBC fbc;
			unsigned int camsv_reg_cal = 0;

			if ((magInfo[1] != _camsv_tg_)
				&& (magInfo[1] != _camsv2_tg_)) {
				log_err("invalid channel(0x%x)\n", magInfo[1]);
				Ret = -EFAULT;
				break;
			}
			...
			if (magInfo[1] == _camsv_tg_) {
				...
				camsv_reg_cal =
					ISP_RD32(ISP_REG_ADDR_CAMSV_TG_VF_CON);
				if ((g_firstCnt[_camsv_1] >= fbc.Bits.FB_NUM)
				   && (camsv_reg_cal & 0x1)) {
					if (g_magQIdx[_camsv_1][0] >=
					   ISP_RT_BUF_SIZE)
						g_magQIdx[_camsv_1][0] = 0;
					g_magQue[_camsv_1][g_magQIdx
						[_camsv_1][0]] = magInfo[0];
			 ...
			 */
				...
				camsv_reg_cal =
					ISP_RD32(ISP_REG_ADDR_CAMSV_TG2_VF_CON);
				if ((g_firstCnt[_camsv_2] >= fbc.Bits.FB_NUM)
					&& (camsv_reg_cal & 0x1)) {
					if (g_magQIdx[_camsv_2][0]
						>= ISP_RT_BUF_SIZE)
						g_magQIdx[_camsv_2][0] = 0;
					g_magQue[_camsv_2][g_magQIdx
						[_camsv_2][0]] = magInfo[0];
...
/* Enable sodi (Multi-Core Deep Idle). */
	...
	log_inf("isp release G_u4EnableClockCount: %d", G_u4EnableClockCount);
	/*  */
...
static signed int ISP_mmap(struct file *pFile, struct vm_area_struct *pVma)
{
	...
	case SENINF_BASE_ADDR:
		...
		break;
	case CAMSV_BASE_ADDR:
		if (length > CAMSV_REG_RANGE) {
			log_err("mmap range	error : length(0x%lx),CAMSV_REG_RANGE(0x%x)!",
				length, CAMSV_REG_RANGE);
			return -EAGAIN;
		}
		break;
	case PLL_BASE_ADDR:
...
#if (!defined(CONFIG_MTK_CLKMGR)) && (!defined(EP_NO_CLKMGR))
	/* Power Domain  */
	WARN_ON(IS_ERR(cam_isp_devs->dev));
	pm_runtime_enable(cam_isp_devs->dev);

	/*CCF: Grab clock pointer (struct clk*) */
	isp_clk.CG_MMSYS_SMI_COMMON = devm_clk_get(&pDev->dev,
						"MMSYS_SMI_COMMON");
	isp_clk.CG_MMSYS_SMI_COMM0 = devm_clk_get(&pDev->dev,
						"MMSYS_SMI_COMM0");
	isp_clk.CG_MMSYS_SMI_COMM1 = devm_clk_get(&pDev->dev,
						"MMSYS_SMI_COMM1");
	isp_clk.CG_MMSYS_SMI_IMG = devm_clk_get(&pDev->dev, "MMSYS_SMI_IMG");
	isp_clk.CG_CAMSYS_LARB2 = devm_clk_get(&pDev->dev, "CAMSYS_LARB2");
	isp_clk.CG_CAMSYS_CAM = devm_clk_get(&pDev->dev, "CAMSYS_CAM");
	isp_clk.CG_CAMSYS_CAMTG = devm_clk_get(&pDev->dev, "CAMSYS_CAMTG");
	isp_clk.CG_CAMSYS_CAM_SENIF = devm_clk_get(&pDev->dev, "CAMSYS_SENIF");
	isp_clk.CG_CAMSYS_CAMSV0 = devm_clk_get(&pDev->dev, "CAMSYS_CAMSV0");
	isp_clk.CG_CAMSYS_CAMSV1 = devm_clk_get(&pDev->dev, "CAMSYS_CAMSV1");

	if (IS_ERR(isp_clk.CG_MMSYS_SMI_COMMON)) {
		log_err("cannot get CG_MMSYS_SMI_COMMON clock\n");
		return PTR_ERR(isp_clk.CG_MMSYS_SMI_COMMON);
	}
	if (IS_ERR(isp_clk.CG_MMSYS_SMI_COMM0)) {
		log_err("cannot get CG_MMSYS_SMI_COMM0 clock\n");
		return PTR_ERR(isp_clk.CG_MMSYS_SMI_COMM0);
	}
	if (IS_ERR(isp_clk.CG_MMSYS_SMI_COMM1)) {
		log_err("cannot get CG_MMSYS_SMI_COMM1 clock\n");
		return PTR_ERR(isp_clk.CG_MMSYS_SMI_COMM1);
	}
	if (IS_ERR(isp_clk.CG_MMSYS_SMI_IMG)) {
		log_err("cannot get CG_MMSYS_SMI_IMG clock\n");
		return PTR_ERR(isp_clk.CG_MMSYS_SMI_IMG);
	}
	if (IS_ERR(isp_clk.CG_CAMSYS_LARB2)) {
		log_err("cannot get CG_CAMSYS_LARB2 clock\n");
		return PTR_ERR(isp_clk.CG_CAMSYS_LARB2);
	}
	if (IS_ERR(isp_clk.CG_CAMSYS_CAM)) {
		log_err("cannot get CG_CAMSYS_CAM clock\n");
		return PTR_ERR(isp_clk.CG_CAMSYS_CAM);
	}
	if (IS_ERR(isp_clk.CG_CAMSYS_CAMTG)) {
		log_err("cannot get CG_CAMSYS_CAMTG clock\n");
		return PTR_ERR(isp_clk.CG_CAMSYS_CAMTG);
	}
	if (IS_ERR(isp_clk.CG_CAMSYS_CAM_SENIF)) {
		log_err("cannot get CG_CAMSYS_CAM_SENIF clock\n");
		return PTR_ERR(isp_clk.CG_CAMSYS_CAM_SENIF);
	}
	if (IS_ERR(isp_clk.CG_CAMSYS_CAMSV0)) {
		log_err("cannot get CG_CAMSYS_CAMSV0 clock\n");
		return PTR_ERR(isp_clk.CG_CAMSYS_CAMSV0);
	}
	if (IS_ERR(isp_clk.CG_CAMSYS_CAMSV1)) {
		log_err("cannot get CG_CAMSYS_CAMSV1 clock\n");
		return PTR_ERR(isp_clk.CG_CAMSYS_CAMSV1);
	}
#endif
...
#endif
	clk_wake_lock = wakeup_source_register(&pDev->dev, "isp_clk_wakelock");
...
static signed int ISP_remove(struct platform_device *pDev)
{
	...
	/* Power Domain  */
	WARN_ON(IS_ERR(cam_isp_devs->dev));
	pm_runtime_disable(cam_isp_devs->dev);
#endif
...
/* log_dbg("[0x%04x,0x%08x][0x%04x,0x%08x] */
/* [0x%04x,0x%08x][0x%04x,0x%08x][0x%04x,0x%08x] */
/* [0x%04x,0x%08x] [0x%04x,0x%08x][0x%04x,0x%08x]*/
/* [0x%04x,0x%08x][0x%04x,0x%08x]\n", */
/* add[0],val[0],add[1],val[1],add[2],val[2],*/
/* add[3],val[3],add[4],val[4],add[5],val[5],add[6],val[6],*/
/* add[7],val[7],add[8],val[8],add[9],val[9]); */
		log_dbg("[0x%p,0x%08x][0x%p,0x%08x][0x%p,0x%08x][0x%p,0x%08x][0x%p,0x%08x]",
			add[0], val[0], add[1], val[1], add[2],	val[2],
	...
	int port, unsigned int mva, void *data)
{
	...
	case M4U_PORT_CAM_LCSO:
		...
		log_dbg("[TF_%d]0x%08X %08X", port,
			...
			(unsigned int)ISP_RD32(ISP_ADDR + 0x00c0));
		log_dbg("[TF_%d]0x%08X %08X", port,
			(unsigned int)(ISP_TPIPE_ADDR + 0x3340),
			(unsigned int)ISP_RD32(ISP_ADDR + 0x3340));
		log_dbg("[TF_%d]0x%08X %08X", port,
			(unsigned int)(ISP_TPIPE_ADDR + 0x3344),
			(unsigned int)ISP_RD32(ISP_ADDR + 0x3344));
		log_dbg("[TF_%d]0x%08X %08X", port,
			(unsigned int)(ISP_TPIPE_ADDR + 0x3348),
			(unsigned int)ISP_RD32(ISP_ADDR + 0x3348));
		log_dbg("[TF_%d]0x%08X %08X", port,
			(unsigned int)(ISP_TPIPE_ADDR + 0x334c),
			(unsigned int)ISP_RD32(ISP_ADDR + 0x334c));
		log_dbg("[TF_%d]0x%08X %08X", port,
			(unsigned int)(ISP_TPIPE_ADDR + 0x3350),
			(unsigned int)ISP_RD32(ISP_ADDR + 0x3350));
		log_dbg("[TF_%d]0x%08X %08X", port,
			(unsigned int)(ISP_TPIPE_ADDR + 0x3354),
			(unsigned int)ISP_RD32(ISP_ADDR + 0x3354));
		log_dbg("[TF_%d]0x%08X %08X", port,
			(unsigned int)(ISP_TPIPE_ADDR + 0x3358),
...
/* FIX-ME: linux-3.10 procfs API changed */
	...
	m4u_register_fault_callback(M4U_PORT_CAM_ESFKO,
				    ISP_M4U_TranslationFault_callback, NULL);
	m4u_register_fault_callback(M4U_PORT_CAM_SV0,
				    ISP_M4U_TranslationFault_callback, NULL);
	m4u_register_fault_callback(M4U_PORT_CAM_LSCI_0,
				    ISP_M4U_TranslationFault_callback, NULL);
	m4u_register_fault_callback(M4U_PORT_CAM_LSCI_1,
				    ISP_M4U_TranslationFault_callback, NULL);
	m4u_register_fault_callback(M4U_PORT_CAM_BPCI_0,
				    ISP_M4U_TranslationFault_callback, NULL);
	m4u_register_fault_callback(M4U_PORT_CAM_BPCI_1,
				    ISP_M4U_TranslationFault_callback, NULL);
...
static void process_dbg_opt(const char *opt)
{
	...
	log_inf("ISP debug cmd %s\n", opt);
	if (strncmp(opt, "log:", 4) == 0) {
		/*ex: echo log:0 > /d/mtkisp */
		ret = sscanf(opt, "log:%d\n", &isp_log);
		if (ret != 1) {
			log_inf("%d error to parse cmd %s\n", __LINE__, opt);
		} else {
			Enable_ISP_LOG = isp_log;
			log_inf("ISP debug log %d\n", Enable_ISP_LOG);
		}
	}
}
...
{
	mtkisp_dbgfs = debugfs_create_file("mtkisp",
		S_IFREG | 0444, NULL, (void *)0, &debug_fops);
}

void ISP_DBG_Deinit(void)
{
	debugfs_remove(mtkisp_dbgfs);
}
