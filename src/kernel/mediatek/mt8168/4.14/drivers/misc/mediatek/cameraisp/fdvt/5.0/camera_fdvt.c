...
#define IRQ_LOG_KEEPER(irq, ppb, logT, fmt, ...) do {\
	...
	if (avaLen > 1) {\
		count = snprintf((char *)(pDes), avaLen, fmt, ##__VA_ARGS__);\
		if (count < 0) {\
			log_err("log str snprintf error(%d)", irq);\
		} \
		...
		log_err("(%d)(%d)log str is not avalible", irq, logT);\
	} \
} while (0)
#else
...
static bool ConfigFDVTRequest(signed int ReqIdx)
{
	...
	if (ReqIdx < 0 || ReqIdx >= _SUPPORT_MAX_FDVT_REQUEST_RING_SIZE_) {
		log_err("[%s] ReqIdx fault!!\n", __func__);
		return MFALSE;
	}
...
static signed int __init FDVT_Init(void)
{
	...
	proc_entry = proc_create("fdvt_dump", 0440,
				 isp_fdvt_dir, &fdvt_dump_proc_fops);

	proc_entry = proc_create("fdvt_reg", 0640,
