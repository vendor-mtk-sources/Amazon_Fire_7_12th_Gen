			    ...
			    gfp_t gfp)
{
	...
	/*
	 *  must get spin lock to avoid race conditon of queuedata = NULL
	 *  in mmc_cleanup_queue() when hot plug SD card. once we get the
	 *  mmc_host *host, then is safe, as host is always exist.
	 */
