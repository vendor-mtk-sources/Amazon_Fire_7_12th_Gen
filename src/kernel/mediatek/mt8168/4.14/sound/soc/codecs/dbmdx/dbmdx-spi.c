...
#define DBMDX_SPI_READ_REG_COMMAND_LEN	4
#define DBMDX_SPI_WRITE_32_COMMAND_LEN	12
#define DBMDX_SPI_WRITE_32_RETURN_LEN	9
/*
 * Write 32 bit - Indirect Write
 */
ssize_t send_spi_cmd_write_va_32(struct dbmdx_private *p, u16 register_number,
								 u32 value)
{
	struct dbmdx_spi_private *spi_p =
				(struct dbmdx_spi_private *)p->chip->pdata;
	u8 send[DBMDX_SPI_WRITE_32_COMMAND_LEN + 1];
	int ret = 0;

	/* Write Format:<RRR>W<VVVVVVVV> */
	ret = snprintf(send, (DBMDX_SPI_WRITE_32_COMMAND_LEN + 1), "%03xW%08x",
				register_number, value);

	dev_dbg(spi_p->dev, "%s: Send %s\n", __func__, send);

	if (ret >= 0)
		ret = send_spi_data(p, send, DBMDX_SPI_WRITE_32_COMMAND_LEN);

	usleep_range(DBMDX_USLEEP_SPI_VA_CMD_AFTER_SEND_2 + p->debug_delay_usec,
			DBMDX_USLEEP_SPI_VA_CMD_AFTER_SEND_2 +
					p->debug_delay_usec + 100);

	if (p->va_debug_mode & DBMDX_DEBUG_MODE_FW_LOG)
		usleep_range(DBMDX_USLEEP_DBG_MODE_CMD_TX,
				DBMDX_USLEEP_DBG_MODE_CMD_TX + 1000);
	return ret;
}

/*
 * READ 32 bit - Indirect READ
 */
ssize_t send_spi_cmd_read_va_32(struct dbmdx_private *p, u16 register_number,
	u32 *response)
{
	struct dbmdx_spi_private *spi_p =
				(struct dbmdx_spi_private *)p->chip->pdata;
	u8 recv[DBMDX_SPI_WRITE_32_RETURN_LEN + 1];
	u8 send[DBMDX_SPI_READ_REG_COMMAND_LEN + 1];
	int ret;

	memset(recv, 0, (DBMDX_SPI_WRITE_32_RETURN_LEN + 1));

	/* Write Format: <RRR>register#,<R>READ32 */
	ret = snprintf(send, (DBMDX_SPI_READ_REG_COMMAND_LEN + 1),
				"%03xR", register_number);

	dev_dbg(spi_p->dev, "%s: Send %s\n", __func__, send);
	ret = send_spi_data(p, send, DBMDX_SPI_READ_REG_COMMAND_LEN);
	if (ret != DBMDX_SPI_READ_REG_COMMAND_LEN)
		goto out;

	usleep_range(DBMDX_USLEEP_SPI_VA_CMD_AFTER_SEND + p->debug_delay_usec,
		DBMDX_USLEEP_SPI_VA_CMD_AFTER_SEND + p->debug_delay_usec + 100);

	if (p->va_debug_mode & DBMDX_DEBUG_MODE_FW_LOG)
		usleep_range(DBMDX_USLEEP_DBG_MODE_CMD_RX,
			DBMDX_USLEEP_DBG_MODE_CMD_RX + 1000);
	ret = 0;

	/* The sleep command cannot be acked before the device
	 * goes to sleep
	 */
	ret = read_spi_data(p, recv, DBMDX_SPI_WRITE_32_RETURN_LEN);
	if (ret < 0) {
		dev_err(spi_p->dev, "%s:spi_read failed =%d\n",
			__func__, ret);
		return ret;
	}
	recv[DBMDX_SPI_WRITE_32_RETURN_LEN] = 0;
	ret = kstrtou32((const char *)&recv[1], 16, response);
	if (ret < 0) {
		dev_err(spi_p->dev, "%s failed -%d\n", __func__, ret);
		dev_err(spi_p->dev, "%s: %x:%x:%x:%x: %x:%x:%x:%x:\n",
			__func__, recv[1], recv[2],
			recv[3], recv[4], recv[5],
			recv[6], recv[7], recv[8]);
		return ret;
	}

	dev_dbg(spi_p->dev, "%s: Received 0x%08x\n", __func__, *response);
...
ssize_t send_spi_cmd_va_32(struct dbmdx_private *p, u32 command,
				   u32 value, u32 *response)
{
	u16 register_number;
	ssize_t ret = 0;

	register_number = (u16)((command >> 16) & 0x0fff);

	if (response)
		ret = send_spi_cmd_read_va_32(p, register_number, response);
	else
		ret = send_spi_cmd_write_va_32(p, register_number, value);

	return ret;
}

#define DBMDX_SPI_CMD_VA_SEND_LEN		8
ssize_t send_spi_cmd_va(struct dbmdx_private *p, u32 command,
				   u16 *response)
{
	struct dbmdx_spi_private *spi_p =
				(struct dbmdx_spi_private *)p->chip->pdata;
	u8 send[DBMDX_SPI_CMD_VA_SEND_LEN + 1];
	u8 recv[6] = {0, 0, 0, 0, 0, 0};
	int ret;

	if (response) {
		ret = snprintf(send, (DBMDX_SPI_READ_REG_COMMAND_LEN + 1),
					"%03xr", (command >> 16) & 0x0fff);
		dev_dbg(spi_p->dev, "%s: Send %s\n", __func__, send);
		...
		usleep_range(DBMDX_USLEEP_SPI_VA_CMD_AFTER_SEND +
						p->debug_delay_usec,
			DBMDX_USLEEP_SPI_VA_CMD_AFTER_SEND +
						p->debug_delay_usec + 100);
		...
		ret = snprintf(send, (DBMDX_SPI_CMD_VA_SEND_LEN+1),
					"%03xw%04x",
					(command >> 16) & 0x0fff,
					(command  & 0x0ffff));
		dev_dbg(spi_p->dev, "%s: Send %s\n", __func__, send);
		...
		ret = send_spi_data(p, send, DBMDX_SPI_CMD_VA_SEND_LEN);
		if (ret != DBMDX_SPI_CMD_VA_SEND_LEN)
			goto out;
		ret = 0;
	}
	...
	while (to_copy > 0) {
		if (to_copy < max_size) {
			...
			if (to_copy % (size_t)bytes_per_word != 0) {
				...
				memset(send+to_copy, 0, pad_size);
			}
...
#ifdef	WAKEUP_SOURCE_REGISTER
		__pm_stay_awake(spi_p->ps_nosuspend_wl);
#else
		__pm_stay_awake(&spi_p->ps_nosuspend_wl);
#endif /* WAKEUP_SOURCE_REGISTER */
#endif
	...
	size_t *data_offset)
{
	...
	ret = send_spi_cmd_va(p,
			DBMDX_REGN_READ_AUDIO_BUFFER | samples, NULL);
...
int spi_common_probe(struct spi_device *client)
{
	...
	p->chip.send_cmd_va_32 = send_spi_cmd_va_32;
