...
int dbmd4_spi_read_dsp_log(u8 *src, u8 *dest, int size)
{
	struct dbmdx_private *p = dbmdx_data;
	u8 read_mem_cmd[6], mem_data[7];
	ssize_t send_bytes;
	u32 addr = (u32)src;
	u16 nr_dwords = size / 4;
	int ret = 0;
	...
	do {
		read_mem_cmd[2] = (u8)(addr&0x000000ff);
		read_mem_cmd[3] = (u8)((addr&0x0000ff00)>>8);
		read_mem_cmd[4] = (u8)((addr&0x00ff0000)>>16);
		read_mem_cmd[5] = (u8)((addr&0xff000000)>>24);
		send_bytes = send_spi_data(p, read_mem_cmd, sizeof(read_mem_cmd));
		if (send_bytes != sizeof(read_mem_cmd)) {
			dev_err(p->dev,
				"%s: -----------> read DSP memory error\n",
				__func__);
			break;
		}
		ret = read_spi_data(p, (void *)mem_data, 7);
		if (ret < 0) {
			dev_err(p->dev, "%s: read_spi_data error\n", __func__);
			goto out;
		}
		memcpy(dest, &mem_data[3], 4);
		dest += 4;
		nr_dwords--;
		addr += 2;
	} while (nr_dwords > 0);
...
static int dbmd4_spi_prepare_amodel_loading(struct dbmdx_private *p)
{
	...
	dev_dbg(spi_p->dev, "%s\n", __func__);
#ifndef AMODEL_LOAD_USES_NORMAL_SPEED
...
static int dbmd4_spi_probe(struct spi_device *client)
{
	...
	p->chip.read_dsp_log = dbmd4_spi_read_dsp_log;
...
late_initcall(dbmd_4_8_modinit);
