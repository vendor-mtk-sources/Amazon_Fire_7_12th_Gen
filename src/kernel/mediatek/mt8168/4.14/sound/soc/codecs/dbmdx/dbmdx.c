...
#endif /* CONFIG_AMAZON_DSP_FRAMEWORK */
...
#define DBMDX_VDD_VCORE_MIN_UV    1100000
#define DBMDX_VDD_VCORE_MAX_UV    1100000
...
static int dbmdx_set_backlog_len(struct dbmdx_private *p, u32 history)
{
	...
	model_selected_mask = (u16)((vt_eng_index << 13) & 0xE000);
	...
	if (all_engines_selected) {
		for (i = 0; i < DBMDX_MAX_SUPPORTED_VT_ENGINES; i++) {
			if ((p->vt_engines[i].history))	//check valid memory ptr before writing
...
static int dbmdx_set_trigger_gpio(struct dbmdx_private *p)
{
	int ret = 0;
	...
	ret = dbmdx_send_cmd(p,
				(DBMDX_REGN_FW_DEBUG_REGISTER_2 |
				DBMDX_REGV_FORCE_DETECTION_INTERRUPT), NULL);
	if (ret < 0) {
		dev_err(p->dev, "%s: failed to send cmd\n", __func__);
		ret = -EIO;
	}
					...
					ssize_t *amodel_size)
{
	...
	if (pos == 0 || pos > MAX_AMODEL_SIZE || amodel_buf == NULL) {
		ret = -EINVAL;
		goto out;
	}
					...
					ssize_t *amodel_chunks_size)
{
	...
	if (file_data == NULL) {
		dev_err(p->dev, "%s: Invalid file data\n", __func__);
		ret = -EINVAL;
		goto out;
	}

	if (amodel_buf == NULL) {
		dev_err(p->dev, "%s: Invalid amodel buffer\n", __func__);
		ret = -EINVAL;
		goto out;
	}
...
static int dbmdx_acoustic_model_build(struct dbmdx_private *p,
					...
					ssize_t *amodel_chunks_size)
{
	if (amodel_files_data == NULL || amodel_files_data[0] == NULL)
		return -EINVAL;
...
#ifdef CONFIG_AMAZON_DSP_FRAMEWORK
/* Load dspg bin to dsp */
bool dbmdx_fw_bin_cpy(void *dest, void *bin_data, size_t bin_size)
{
	...
	if (!bin_data) {
		dev_err(p->dev, "%s: bin address invalid\n", __func__);
		return false;
	}

	if (bin_size <= 4) {
		dev_err(p->dev, "%s: bin size (%zu) invalid\n",
			__func__, bin_size);
		goto out_err;
	}

	fw_file_checksum = &bin_data[bin_size - 4];
	...
	dbmdx_get_firmware_version(bin_data, bin_size, fw_version, 200);
	...
	/* boot */
	ret = p->chip->boot(bin_data, bin_size, p, fw_file_checksum, 4, 1);
	if (ret)
				...
				u16 *post_pll_div)
{
	...
	if (chip_clock_conf->wanted_ahb_clk == 0)
		...
		wanted_ahb_clk = chip_clock_conf->wanted_ahb_clk;
	if (chip_clock_conf->wanted_apb_clk == 0)
		wanted_apb_clk = wanted_ahb_clk;
	else
		wanted_apb_clk = chip_clock_conf->wanted_apb_clk;
				...
				int num_dep_divisors)
{
	...
	dev_dbg(p->dev,	"%s: %s Clock Operation:\t%s\n", __func__,
		chip == DBMDX_CHIP_VA ? "VA" : "VA_VE",
	...
	if (chip_clock_conf->use_pll_post_div)
		new_config = (new_config & 0xFFE0) | (u16)(post_pll_div-1);
	else
		new_config &= 0xFFE0;

	dev_dbg(p->dev,
		"%s: cur_config=%x, new_config=%x,  use_pll_post_div=%d, post_pll_div=%d\n",
		__func__, cur_config, new_config,
		chip_clock_conf->use_pll_post_div, post_pll_div);
	...
	if ((pll_step > 14 &&
			(clock_op != DBMDX_CLOCK_OP_SWITCH_TO_TDM_CLOCK)) ||
		(clock_op == DBMDX_CLOCK_OP_SWITCH_TO_MASTER_CLOCK)) {
#ifndef FW_SUPPORTS_DSP_CLOCK_32BIT_CMD
		ret = dbmdx_send_cmd(p,	(DBMDX_REGN_DSP_CLOCK_CONFIG |
					...
					ahb_div), NULL);
#else
		ret = dbmdx_send_cmd_32(p, (u32)DBMDX_REGN_DSP_CLOCK_CONFIG,
					((u32)((u32)switch_clk << 12) |
					(u32)((u32)clk_src << 12) |
			(u32)((u32)DBMDX_REGV_PLL_OSC_SEL_USE_PLL << 12) |
					(u32)((u32)pll_step << 12) |
					tl3_div << 8	|
					apb_div	<< 5	|
					ahb_div), NULL);
...
#if defined(FW_SUPPORTS_DSP_CLOCK_32BIT_CMD) && defined(SET_PLL_OSC_SEL_USE_OSC)
		ret = dbmdx_send_cmd_32(p, (u32)DBMDX_REGN_DSP_CLOCK_CONFIG,
					((u32)((u32)switch_clk << 12) |
					(u32)((u32)clk_src << 12) |
					(u32)((u32)DBMDX_REGV_PLL_OSC_SEL_USE_OSC << 12) |
					(u32)((u32)pll_step << 12)	|
					tl3_div << 8	|
					apb_div	<< 5	|
					ahb_div), NULL);
...
static int configure_usecase_clocks(struct dbmdx_private *p,
					...
					bool i2s_master_mode_post_start_cfg)
{
	enum dbmdx_clock_op clock_op;
	enum TDM_CLK_FREQ_CONF freq;
	int num_of_bits;
	...
	if (p->va_chip_enabled) {
		if (i2s_master_mode_post_start_cfg)
			clock_op = usecase->clock_op_va_i2s_master;
		else
		...
		if (p->va_debug_mode != DBMDX_DEBUG_MODE_OFF) {
			if (chip_clock_conf->wanted_tl3_clk_debug)
				wanted_tl3_clk =
					chip_clock_conf->wanted_tl3_clk_debug;
			else
				wanted_tl3_clk = p->pdata->default_va_clock;
		} else if (chip_clock_conf->wanted_tl3_clk == 0) {
			wanted_tl3_clk = p->pdata->default_va_clock;
		} else {
		...
		if (!(p->mics_connected_to_va_ve_chip)) {
			for (i = 0; i < 4; i++) {
				if (usecase->mic_freq[i]) {
					dep_divisors[cur_divisor_index] =
						usecase->mic_freq[i];
					cur_divisor_index++;
				}
			}
		...
		if (clock_op == DBMDX_CLOCK_OP_SWITCH_TO_TDM_CLOCK) {
			...
			if (!(p->pdata->change_clock_src_enabled &
				...
				DBMDX_CLOCK_OP_CONFIG_IF_MASTER_CLOCK_HIGHER) ||
			(clock_op ==
			DBMDX_CLOCK_OP_CONFIG_IF_MASTER_CLOCK_LOWER)) &&
			(p->cur_va_clock == DBMDX_CLOCK_FROM_MASTER)) {

			ret = dbmdx_send_cmd(p,
					DBMDX_REGN_DSP_CLOCK_CONFIG, &cur_val);
			if (ret) {
				dev_err(p->dev,
					"%s: VA Error Reading DSP CLK\n",
					__func__);
				ret = -EIO;
				goto out;
			}
			...
			if (clock_op ==
				DBMDX_CLOCK_OP_CONFIG_IF_MASTER_CLOCK_HIGHER) {
				if (cur_val >
					chip_clock_conf->threshold_value) {
					freq = TDM_CLK_FREQ_0;
					num_of_bits = 0;
					perform_config = true;
				}
							...
							bool allow_sleep)
{
	...
	p->va_ve_flags.ignore_vt_data = 0;
	...
	ret = configure_usecase_clocks(p, p->idle_usecase, false);
	if (ret < 0) {
	...
	p->va_flags.wd_inuse = 1;
	dev_info(p->dev, "%s Set WD INT\n", __func__);
...
#if !defined(DBMDX_MIC_TYPE_IS_DIGITAL) && defined(DBMDX_SAR_ADC_SELECT_SEC)
	msleep(DBMDX_MSLEEP_SAR_MIC_SEC_CONFIG);
#endif
	return 0;
}
					...
					u32 seq_cmd)
{
	...
	case DBMDX_UC_SEQ_CMD_CHANGE_CLK_SRC:
		if (usecase->change_clock_src) {
			ret = configure_usecase_clocks(p, usecase, false);
			if (ret < 0) {
				dev_err(p->dev,
...
int dbmdx_set_usecase_mode(struct dbmdx_private *p, int mode)
{
	...
	if (usecase_mode == DBMDX_IDLE) {
		...
		if (mode & DBMDX_BUFFERING_WITH_BACKLOG_MASK) {
			buffering_with_backlog = true;
			dev_info(p->dev, "%s Buffering with backlog\n",
					__func__);
		}
		...
		if (!buffering_with_backlog) {
			...
			dev_err(p->dev,
				"%s BUFFERING with backlog, p->va_cur_backlog_length = %d\n",
				__func__, (int)p->va_cur_backlog_length);
	...
	if (p->va_ve_flags.usecase_mode == 1) {
		dev_info(p->dev, "%s: mode change to  %d, close spi power lock\n",
...
#ifdef CONFIG_AMAZON_DSP_FRAMEWORK
		if (adfLoad_checkMagic(p->va_ve_fw->data)) {
			pr_info("%s %d detect ADF FW header, parse and load it\n", __func__, __LINE__);
			/* if dsp was crashed previously,
			* then we need to dump the dsp log before loading fw
			* as there only have one core so dsp id is zero
			*/
			if (p->va_flags.wd_reset) {
				adfDebug_printLog(0, ADF_LOG_DUMP_ALL);
				p->va_flags.wd_reset = 0;
			}
			ret = adfLoad_init((void *)p->va_ve_fw->data,
							NULL,
							p->va_ve_fw->size,
							dbmdx_fw_bin_cpy,
							0);
		} else {
			pr_info("%s detect DSPG FW header, parse and load it\n", __func__);
...
#ifdef CONFIG_AMAZON_DSP_FRAMEWORK
		if (adfLoad_checkMagic(p->va_fw->data)) {
			pr_info("%s %d detect ADF FW header, parse and load it\n", __func__, __LINE__);
			/* if dsp was crashed previously,
			* then we need to dump the dsp log before loading fw
			* as there only have one core so dsp id is zero
			*/
			...
			ret = adfLoad_init((void *)p->va_fw->data,
							NULL,
							p->va_fw->size,
							dbmdx_fw_bin_cpy,
							0);
				  ...
				  const char *buf, size_t size)
{
	...
	if (!strncmp(buf, "clk_output", min_t(int, size, 10)))
		...
		ret = 0;
	} else if (!strncmp(buf, "trigger_gpio", min_t(int, size, 12))) {
		ret = dbmdx_set_trigger_gpio(p);
	} else if (!strncmp(buf, "help", min_t(int, size, 4))) {
		dev_info(p->dev,
			"%s: Commands: clk_output | uart_dbg | clk_uart_output | disable_dbg | pm_suspend | pm_resume | disable_sleep | enable_sleep | trigger_gpio | help\n",
			__func__);
		ret = 0;
	}
					...
					char *buf)
{
	...
	ret = snprintf(buf, PAGE_SIZE,
		"Supported Commands: [ clk_output | uart_dbg | clk_uart_output | disable_dbg | pm_suspend | pm_resume | disable_sleep | enable_sleep | mic_disable_on | mic_disable_off | trigger_gpio | help ]\n");
				  ...
				  const char *buf, size_t size)
{
	...
	if (!strncmp(buf, "mic_override_enable", min_t(int, size, 19))) {
		...
		p->va_ve_mic_mode = 0;
		ret = 0;
	} else if (!strncmp(buf, "gain_override_enable",
					min_t(int, size, 20))) {
		p->va_ve_gain_mode = 1;
		ret = 0;
	} else if (!strncmp(buf, "gain_override_disable",
					min_t(int, size, 21))) {
		p->va_ve_gain_mode = 0;
		ret = 0;
					...
					char *buf)
{
	...
	off += snprintf(buf + off, PAGE_SIZE - off,
				"\tGain Override Enabled:\t%s\n",
				p->va_ve_gain_mode ? "ON" : "OFF");
					...
					struct usecase_config *uc)
{
	...
	int i;
	struct vt_engine *vte;
	int highest_loaded_vte_index = -1;
	int lowest_vte_index_to_load = DBMDX_MAX_SUPPORTED_VT_ENGINES;
	bool is_loaded_to_mem = false;
	bool is_loaded_to_fw = false;
	...
	/* Check if models have to be temporary unloaded
	 * We try to avoid state where model with higher vte index
	 * has been already loaded to FW
	 */
	for (i = 0; i < DBMDX_MAX_SUPPORTED_VT_ENGINES; i++) {
		...
		if (vte->enabled &&
			((vte->internal_state == MODEL_STATE_LOADED) ||
				(vte->internal_state == MODEL_STATE_STARTED)))
			highest_loaded_vte_index = i;

		if ((lowest_vte_index_to_load ==
				DBMDX_MAX_SUPPORTED_VT_ENGINES) &&
			vte->enabled) {

			ret = check_if_model_loaded(p, vte,
			...
			if (!is_loaded_to_fw)
				lowest_vte_index_to_load = i;
		}

	}

	dev_info(p->dev, "%s: Lowest VTE to Load: %d, Highest Loaded %d\n",
		__func__, lowest_vte_index_to_load, highest_loaded_vte_index);

	if (lowest_vte_index_to_load <  highest_loaded_vte_index) {

		ret = uc_load_models_unload_all(p, uc);

		if (ret < 0) {
			dev_err(p->dev,	"%s: Error during unloading models\n",
				__func__);
			return -EIO;
		}
						...
						struct usecase_config *uc)
{
	...
	for (i = 0; i < DBMDX_MAX_SUPPORTED_VT_ENGINES; i++) {
		...
		if (uc->model_cp == 0)
			cp_val = DBMDX_REGV_IN_CP_NUM_0;
		else if (uc->model_cp == 1)
			cp_val = DBMDX_REGV_IN_CP_NUM_1;
		else if (uc->model_cp == 2)
			cp_val = DBMDX_REGV_IN_CP_NUM_2;
		else if (uc->model_cp == 3)
			cp_val = DBMDX_REGV_IN_CP_NUM_3;
		else if (uc->model_cp == 4)
			cp_val = DBMDX_REGV_IN_CP_NUM_4;
		else if (uc->model_cp == 5)
			cp_val = DBMDX_REGV_IN_CP_NUM_5;
		else if (uc->model_cp == 6)
			cp_val = DBMDX_REGV_IN_CP_NUM_6;
		else if (uc->model_cp == 7)
			cp_val = DBMDX_REGV_IN_CP_NUM_7;
		else
			cp_val = DBMDX_REGV_IN_CP_NUM_0;
			   ...
			   enum dbmdx_load_amodel_mode model_type)
{
	...
	int chunk_ind;
	u16 mem_loc;
	u16 model_ftype = DBMDX_REGV_LOAD_PRIMARY_VT_MODEL;
	u16 blk_start;
	u16 vt_num;
	...
	while (retry--) {
		...
		ret = dbmdx_send_cmd(
				...
				DBMDX_FILE_LOAD_INIT_OR_KILL_REL |
				blk_start |
				mem_loc |
				model_ftype,
				NULL);
...
static int dbmdx_model_loaded_correctly(struct dbmdx_private *p, u16 vt_index)
{
	...
	/* when load model wait some time before dsp ready avoid spi read fail */
	msleep(DBMDX_MSLEEP_MODEL_LOAD);
	ret = dbmdx_va_alive(p);
	if (ret) {
				...
				enum dbmdx_model_detection_type det_type)
{
	...
	if (vte->vte_index == 1) {
		vt_num_stop = DBMDX_REGV_SELECT_VT_EN_1;
		vt_num = DBMDX_REGV_LOAD_ENGINE_TYPE_VT1;
	} else if (vte->vte_index == 2) {
		vt_num_stop = DBMDX_REGV_SELECT_VT_EN_2;
		vt_num = DBMDX_REGV_LOAD_ENGINE_TYPE_VT2;
	} else {
		vt_num_stop = DBMDX_REGV_SELECT_VT_EN_3;
		vt_num = DBMDX_REGV_LOAD_ENGINE_TYPE_VT3;
	}
	...
	ret = dbmdx_send_cmd(p,
				...
				NULL);
	if (ret < 0) {
		dev_err(p->dev,	"%s: Error disabling detection for %s\n",
			__func__,
			dbmdx_vte_names[vte->eng_type]);
		ret = -EIO;
		goto out;
...
static int check_if_spi_transfer_addr_valid(struct dbmdx_private *p, u32 addr_start, u32 addr_end)
{
	if (addr_start < DBMD4_DTCM_MEM_START || addr_start > DBMD4_AHB_DATA_MEM_END)
		return 0;
	if (addr_start < DBMD4_AHB_DATA_MEM_START &&  addr_start > DBMD4_DTCM_MEM_END)
		return 0;
	if (addr_end < DBMD4_DTCM_MEM_START || addr_end > DBMD4_AHB_DATA_MEM_END)
		return 0;
	if (addr_end < DBMD4_AHB_DATA_MEM_START && addr_end > DBMD4_DTCM_MEM_END)
		return 0;
	return 1;
}
...
int dbmdx_debug_read(u8 *src, u8 *dest, int size)
{
	...
	u32 addr = (u32)src;
	u16 nr_words = 0;
	u16 samples_chunk_size, nr_samples;
	int bytes_to_read = 0;
	int ret = -EINVAL;

	dev_info(p->dev, "%s: SPI data block read addr 0x%08x, %d Bytes\n", __func__, addr, size);

	if (size == 0) {
		dev_err(p->dev, "%s: Invalid length\n", __func__);
		return -EINVAL;
	}
	if (size % 2) {
		dev_err(p->dev, "%s: DSP TL3 doesn't support BYTE, minimum unit is WORD(2 BYTES)\n", __func__);
		return -EINVAL;
	}
	nr_words = size / 2;
	if (!check_if_spi_transfer_addr_valid(p, addr, addr + nr_words)) {
		dev_err(p->dev, "%s: spi transfer block word address is not in range (0x%x - 0x%x) (0x%x - 0x%x)\n", __func__,
			DBMD4_DTCM_MEM_START,	DBMD4_DTCM_MEM_END,
			DBMD4_AHB_DATA_MEM_START,	DBMD4_AHB_DATA_MEM_END);
		return -EINVAL;
	}

	if (p->va_flags.wd_reset) {
		//NOTE: Assume p->lock(p) is called before reaching here
		ret = p->chip->read_dsp_log(src, dest, size);
		if (ret < 0) {
			dev_err(p->dev, "%s: DSP log read failed!\n",
				__func__);
		}
		return  ret < 0 ? ret : size;
	}

	samples_chunk_size = (p->chip->get_read_chunk_size(p) - 2) / 2 ;		// in words, minus spi offset

	do {
		if (nr_words > samples_chunk_size)
		...
		ret = dbmdx_send_cmd(p, DBMDX_REGN_IO_PORT_ADDR_LO | (addr & 0x0000FFFF), NULL);
		if (ret < 0) {
			dev_err(p->dev, "%s: write register 0x05 error\n", __func__);
			goto out_unlock;
		}
		ret = dbmdx_send_cmd(p, DBMDX_REGN_IO_PORT_ADDR_HI | ((addr>>16) & 0x0000FFFF), NULL);
		if (ret < 0) {
			dev_err(p->dev, "%s: write register 0x06 error\n", __func__);
			goto out_unlock;
		}
		ret = dbmdx_send_cmd(p, DBMDX_REGN_SPI_TRANSFER_BLOCK | (nr_samples & 0x7fff), NULL);
		if (ret < 0) {
			dev_err(p->dev, "%s: write register 0x39 error\n", __func__);
			goto out_unlock;
		}

		bytes_to_read = nr_samples*2 + 2;		// 2bytes header.
		ret = p->chip->read(p, p->read_audio_buf, bytes_to_read);
		if (ret != bytes_to_read) {
			dev_err(p->dev, "%s: read buff failed, %u bytes to read, res(%d)\n", __func__, bytes_to_read, ret);
			goto out_unlock;
		}
...
static ssize_t dbmdx_va_memblock_read_store(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t size)
{
	struct dbmdx_private *p = dev_get_drvdata(dev);
	...
	uintptr_t addr = 0;
	u32 len = 0;
	bool addr_set = false, len_set = false;
	u8 *dest_buf;
	u8 *data_p;
	u32 i = 0;
	int ret = -EINVAL;
	...
	while ((str_p = strsep(&args, " \t")) != NULL) {
		...
		if (strncmp(str_p, "addr=", 5) == 0) {
			ret = kstrtoul((str_p+5), 0, &val);
			if (ret) {
				dev_err(p->dev, "%s: bad addr\n", __func__);
				ret = -EINVAL;
				goto print_usage;
			}
			addr = (u32)val;
		...
		if (strncmp(str_p, "len=", 4) == 0) {
			ret = kstrtoul((str_p+4), 0, &val);
			if (ret) {
				dev_err(p->dev, "%s: bad read size\n", __func__);
				ret = -EINVAL;
				goto print_usage;
			}
			len = (u32)val;
	...
	if (!addr_set) {
		dev_err(p->dev, "%s: addr is not set\n", __func__);
		ret = -EINVAL;
		goto print_usage;
	}
	if (!len_set) {
		dev_err(p->dev, "%s: len is not set\n", __func__);
		ret = -EINVAL;
		goto print_usage;
	}

	dest_buf  = vmalloc(len);
	if (!dest_buf) {
		dev_err(p->dev, "%s: Fail to allocate memory %d bytes\n", __func__, len);
		ret = -ENOMEM;
		goto print_usage;
	}

	ret = dbmdx_debug_read((u8 *)addr, dest_buf, len);
	if (ret < 0) {
		dev_err(p->dev, "%s: memblock read failed!\n", __func__);
		goto rel_mem;
	}

	data_p = dest_buf;
	for (i = 0; i < (len / 8) * 8; i += 8) {
		dev_info(p->dev, "%08x - %02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x\n", (unsigned int)addr+i/2,
			((u8 *)data_p)[i + 0], ((u8 *)data_p)[i + 1],
			((u8 *)data_p)[i + 2], ((u8 *)data_p)[i + 3],
			((u8 *)data_p)[i + 4], ((u8 *)data_p)[i + 5],
			((u8 *)data_p)[i + 6], ((u8 *)data_p)[i + 7]);
	}
	i = (len / 8) * 8;

	switch (len - i) {
	case 2:
		dev_info(p->dev, "%08x - %02x:%02x", (unsigned int)addr+i/2,
			((u8 *)data_p)[i + 0], ((u8 *)data_p)[i + 1]);
		break;

	case 4:
		dev_info(p->dev, "%08x - %02x:%02x:%02x:%02x", (unsigned int)addr+i/2,
			((u8 *)data_p)[i + 0], ((u8 *)data_p)[i + 1],
			((u8 *)data_p)[i + 2], ((u8 *)data_p)[i + 3]);
		break;

	case 6:
		dev_info(p->dev, "%08x - %02x:%02x:%02x:%02x:%02x:%02x", (unsigned int)addr+i/2,
			((u8 *)data_p)[i + 0], ((u8 *)data_p)[i + 1],
			((u8 *)data_p)[i + 2], ((u8 *)data_p)[i + 3],
			((u8 *)data_p)[i + 4], ((u8 *)data_p)[i + 5]);
		break;
	...
	dev_info(p->dev, "%s: memblock was read successfully\n",
				__func__);
rel_mem:
	vfree(dest_buf);
	return ret < 0 ? ret : size;
...
print_usage:
	dev_info(p->dev,
		"%s: Usage: addr=memaddr len=bytes\n", __func__);
	return ret;
}
...
int dbmdx_debug_write(u8 *dest, u8 *src, int size)
{
	...
	u16 nr_words = 0;
	u16 samples_chunk_size, nr_samples;
	u16 bytes_to_write = 0;
	int ret = -EINVAL;

	dev_info(p->dev, "%s: SPI data block write addr 0x%08x, %d Bytes\n", __func__, addr, size);
	...
	if (!check_if_spi_transfer_addr_valid(p, addr, addr + nr_words)) {
		dev_err(p->dev, "%s: spi transfer block word address is not in range (0x%x - 0x%x) (0x%x - 0x%x)\n",
			__func__,
			DBMD4_DTCM_MEM_START,
			DBMD4_DTCM_MEM_END,
			DBMD4_AHB_DATA_MEM_START,
			DBMD4_AHB_DATA_MEM_END);
		return -EINVAL;
	}

	samples_chunk_size = (p->chip->get_write_chunk_size(p)) / 2 ;		// in words
	...
	do {
		...
		ret = dbmdx_send_cmd(p,
				DBMDX_REGN_IO_PORT_ADDR_LO | (addr & 0x0000FFFF), NULL);
		if (ret < 0) {
			dev_err(p->dev, "%s: write register 0x05 error\n", __func__);
			goto out_unlock;
		}
		...
		ret = dbmdx_send_cmd(p,
				DBMDX_REGN_IO_PORT_ADDR_HI | ((addr>>16) & 0x0000FFFF), NULL);
		if (ret < 0) {
			dev_err(p->dev, "%s: write register 0x06 error\n", __func__);
			goto out_unlock;
		}
		...
		ret = dbmdx_send_cmd(p,
				DBMDX_REGN_SPI_TRANSFER_BLOCK | (nr_samples & 0x7fff) | 0x8000, NULL);
		if (ret < 0) {
			dev_err(p->dev, "%s: write register 0x39 error\n", __func__);
			goto out_unlock;
		}
		...
		/* Send Data */
		bytes_to_write = nr_samples*2;
		ret = p->chip->write(p, src, bytes_to_write);
...
u8 *adfModel_PreLoad(struct firmware *wakewordModel)
{
	...
	struct dbmdx_private *p = dbmdx_data;
#endif
	u8 *modelAddress = (u8 *)0x500d400;	//dummy address
	unsigned int cmd;
	int ret = -EINVAL;

	dev_info(p->dev, "%s: Start\n", __func__);

	ret = dbmdx_usecase_manager(p, DBMDX_USECASE_MGR_CMD_SET_IDLE);
	if (ret < 0) {
		dev_err(p->dev, "%s: Usecase manager cannot set to IDLE\n",
			__func__);
	}
	//STOP DETECTION & UNLOAD MODEL
	cmd = (unsigned int)(((DBMDX_USECASE_MGR_CMD_LOAD_AMODEL << DBMDX_USECASE_MGR_CMD_BITS) & DBMDX_USECASE_MGR_CMD_MASK)
		|((DBMDX_USECASE_MGR_LM_OP_STOP_DETECTION << DBMDX_USECASE_MGR_LM_MODEL_OPERATION_BITS) & DBMDX_USECASE_MGR_LM_MODEL_OPERATION_MASK)
		|((DBMDX_VT_ENGINE_TYPE_ALEXA << DBMDX_USECASE_MGR_LM_MODEL_TYPE_BITS) & DBMDX_USECASE_MGR_LM_MODEL_TYPE_MASK));
	ret = dbmdx_usecase_manager(p, cmd);
	if (ret < 0) {
	...
	cmd = (unsigned int)(((DBMDX_USECASE_MGR_CMD_LOAD_AMODEL << DBMDX_USECASE_MGR_CMD_BITS) & DBMDX_USECASE_MGR_CMD_MASK)
		|((DBMDX_USECASE_MGR_LM_OP_UNLOAD_MODEL << DBMDX_USECASE_MGR_LM_MODEL_OPERATION_BITS) & DBMDX_USECASE_MGR_LM_MODEL_OPERATION_MASK)
		|((DBMDX_VT_ENGINE_TYPE_ALEXA << DBMDX_USECASE_MGR_LM_MODEL_TYPE_BITS) & DBMDX_USECASE_MGR_LM_MODEL_TYPE_MASK));
	ret = dbmdx_usecase_manager(p, cmd);
	if (ret < 0) {
		dev_err(p->dev, "%s: Usecase manager reported error\n",
...
bool adfModel_Load(u8 *modelAddress, struct firmware *wakewordModel)
{
#if defined(SOC_CONTROLS_FOR_DBMDX_CODEC_ONLY)
#ifdef SND_SOC_COMPONENT
	struct snd_soc_component *component = snd_kcontrol_chip(kcontrol);
	...
	ret = dbmdx_get_vte_from_model_type(p, DBMDX_VT_ENGINE_TYPE_ALEXA, &vte);
	if (ret < 0) {
		dev_err(p->dev,	"%s: Model type ALEXA is not supported\n",
			__func__);
		ret = -EINVAL;
		goto out;
	}
	...
	files_data[0] = wakewordModel->data;
	amodel_files_size[0] = wakewordModel->size;

	model->amodel_loaded = false;
	ret = dbmdx_acoustic_model_build(p,
					1,
					files_data,
					amodel_files_size,
	...
	cmd = (unsigned int)(((DBMDX_USECASE_MGR_CMD_LOAD_AMODEL << DBMDX_USECASE_MGR_CMD_BITS) & DBMDX_USECASE_MGR_CMD_MASK)
		|((DBMDX_USECASE_MGR_LM_OP_START_DETECTION << DBMDX_USECASE_MGR_LM_MODEL_OPERATION_BITS) & DBMDX_USECASE_MGR_LM_MODEL_OPERATION_MASK)
		|((DBMDX_VT_ENGINE_TYPE_ALEXA << DBMDX_USECASE_MGR_LM_MODEL_TYPE_BITS) & DBMDX_USECASE_MGR_LM_MODEL_TYPE_MASK));
	...
	if (ind >= num_usecases || !cur_usecase) {
		dev_err(p->dev,
			"%s: Cannot find usecase id\n", __func__);
		ret = -EINVAL;
		goto out;
	}

	if (!(cur_usecase->complex_clb_1)) {
		dev_err(p->dev, "%s: CLB_1 is not set\n", __func__);
		ret = -EIO;
		goto out;
	}
	ret = cur_usecase->complex_clb_1(p, cur_usecase);		//load WW model function
	if (ret < 0) {
		dev_err(p->dev, "%s: Error during CLB_1\n", __func__);
		ret = -EIO;
	}
...
bool adfModel_PostLoad(void)
{
	return true;
}

int adfModel_GetFilename(char *chunk_name, unsigned int prefix)
{
#if defined(SOC_CONTROLS_FOR_DBMDX_CODEC_ONLY)
#ifdef SND_SOC_COMPONENT
	struct snd_soc_component *component = snd_kcontrol_chip(kcontrol);
	...
	snprintf(amodel_prefix_buf, 16, "%03d_", prefix);
	amodel_prefix = amodel_prefix_buf;
	ret = dbmdx_get_vte_from_model_type(p, DBMDX_VT_ENGINE_TYPE_ALEXA, &vte);
	if (ret < 0) {
		dev_err(p->dev, "%s: Model type ALEXA is not supported\n", __func__);
	...
	amodel_fnames = vte->amodel_fnames[DETECTION_TYPE_PHRASE][0];
	if (strlen(amodel_fnames) > (NAME_MAX - strlen(amodel_prefix))) {
		dev_err(p->dev,
			"%s: Name length out of bounds\n",
			__func__);
		ret = -EINVAL;
	...
	strncpy(chunk_name, amodel_prefix, NAME_MAX);
	strncat(chunk_name, amodel_fnames,
		NAME_MAX - strlen(amodel_prefix));
	dev_dbg(p->dev, "%s: loading %s\n", __func__, chunk_name);
...
#define MAX_VA_MEMBLOCK_FILENAME_LEN 256

static ssize_t dbmdx_test_dynamic_load(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t size)
{
	struct dbmdx_private *p = dev_get_drvdata(dev);
	struct firmware *wakewordModel;
	char filename[MAX_VA_MEMBLOCK_FILENAME_LEN+1];
	int filenameLen = 0;
	int ret = -EINVAL;
	u8 *modelAddress;
	bool loadResult;
	...
	if (buf[filenameLen-1] == '\n')
		filenameLen -= 1;
	if (filenameLen > MAX_VA_MEMBLOCK_FILENAME_LEN) {
		dev_err(p->dev,	"%s: Filename is too long\n", __func__);
		return -EAGAIN;
	}
	strncpy(filename, buf, filenameLen);
	filename[filenameLen] = '\0';
	ret = request_firmware(
			(const struct firmware **)&wakewordModel,
			filename,
			p->dbmdx_dev);
	if (ret != 0) {
		dev_err(p->dev,
	...
	dev_dbg(p->dev, "%s: %s firmware successfully requested\n",
			__func__, filename);
	dev_dbg(p->dev, "%s FW size=%zu bytes\n",
			__func__, wakewordModel->size);

	if (!wakewordModel->size) {
		dev_warn(p->dev, "%s FW size is 0. Ignore...\n",
			__func__);
		ret = -EAGAIN;
		goto out;
	...
	modelAddress = adfModel_PreLoad(wakewordModel);
	loadResult = adfModel_Load(modelAddress, wakewordModel);
	if (loadResult) {
		if (adfModel_PostLoad())
			ret = size;
		else
			ret = -EINVAL;
	}
...
out:
	release_firmware(wakewordModel);
	return ret;
}


static ssize_t dbmdx_va_memblock_write_store(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t size)
{
	...
	u32 len = 0;
	bool addr_set = false, len_set = false;
	u8 src_test_buf[] = "0123456789ABCDEF0123456789ABCDEF0123456789ABCDEF0123456789ABCDEF0123456789ABCDEF";
	u8 *src = src_test_buf;
	u8 *data_p;
	int dataLen = 0;
	int i = 0;
	int ret = -EINVAL;
	struct firmware	*memblock_fw = NULL;
	char filename[MAX_VA_MEMBLOCK_FILENAME_LEN+1];
	char *pfilename;
	int filenameLen = 0;
	...
	while ((str_p = strsep(&args, " \t")) != NULL) {
		...
		if (strncmp(str_p, "file=", 5) == 0) {
			pfilename = str_p+5;
			filenameLen = strlen(pfilename);
			if (pfilename[filenameLen-1] == '\n')
				filenameLen -= 1;
			continue;
		}
	}
	...
	if (!len_set && src == src_test_buf && filenameLen == 0) {
		dev_err(p->dev, "%s: Either len, data or file should be set\n", __func__);
		ret = -EINVAL;
		goto print_usage;
	}
	if (src != src_test_buf) {	//ignore len if data is entered
		len = dataLen;
	}
	if (filenameLen) {
		if (filenameLen > MAX_VA_MEMBLOCK_FILENAME_LEN) {
		...
		strncpy(filename, pfilename, filenameLen);
		filename[filenameLen] = '\0';
		ret = request_firmware((const struct firmware **)&memblock_fw,
					filename, p->dev);
		if (ret != 0) {
			dev_err(p->dev, "%s: failed to request memblock file - %s\n",
					__func__, filename);
			ret = -EIO;
			goto out;
		}
		len = memblock_fw->size;
		src = (u8 *)memblock_fw->data;
	}

	ret = dbmdx_debug_write((u8 *)addr, src, len);
	if (ret < 0)
		goto rel_fw;
...
rel_fw:
	if (filenameLen)
		release_firmware(memblock_fw);

	if (ret < 0) {
		dev_err(p->dev, "%s: memblock write failed!\n", __func__);
		goto out;
	}

	dev_info(p->dev, "%s: memblock was written successfully\n",
				__func__);
	return size;

print_usage:
	dev_info(p->dev,
		"%s: Usage: addr=memaddr data=testPattern | file=filename.bin | len=bytes(test data) \n", __func__);
out:
	return ret;
}
...
static ssize_t dbmdx_va_auto_buffering_show(struct device *dev,
				    struct device_attribute *attr, char *buf)
{
	struct dbmdx_private *p = dev_get_drvdata(dev);
	int ret;
...
static ssize_t dbmdx_va_auto_buffering_store(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t size)
{
	struct dbmdx_private *p = dev_get_drvdata(dev);
	...
	if (val != 0 && val != 1) {
		dev_err(p->dev, "%s: invalid auto_buffering mode\n",
			__func__);
		return -EINVAL;
	}

	p->pdata->auto_buffering = val;
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	if (p->pdata->feature_va || p->pdata->feature_va_ve) {
		...
		off += snprintf(buf + off, PAGE_SIZE - off,
					"\tDSP Control:\t%s\n",
			p->va_dsp_control ? "ON" : "OFF");
		off += snprintf(buf + off, PAGE_SIZE - off,
					"\tDSP Idle:\t%s\n",
			p->va_dsp_idle ? "ON" : "OFF");
		off += snprintf(buf + off, PAGE_SIZE - off,
			"\tDSP Model:\t%d\n",
			p->va_dsp_model);
...
static DEVICE_ATTR(va_auto_buffering, S_IRUGO | S_IWUSR,
		   dbmdx_va_auto_buffering_show,
		   dbmdx_va_auto_buffering_store);
...
static DEVICE_ATTR(va_memblock_read, S_IWUSR,
		NULL, dbmdx_va_memblock_read_store);
static DEVICE_ATTR(va_memblock_write, S_IWUSR,
		NULL, dbmdx_va_memblock_write_store);
static DEVICE_ATTR(test_dynamic_load, S_IWUSR,
		NULL, dbmdx_test_dynamic_load);
...
static struct attribute *dbmdx_va_attributes[] = {
	...
	&dev_attr_va_auto_buffering.attr,
...
static struct attribute *dbmdx_va_ve_chip_va_attributes[] = {
	...
	&dev_attr_va_max_detection_buffer_size.attr,
	&dev_attr_va_amodel_options.attr,
	&dev_attr_va_memblock_read.attr,
	&dev_attr_va_memblock_write.attr,
	&dev_attr_test_dynamic_load.attr,
	NULL,
};
...
static int dbmdx_perform_recovery(struct dbmdx_private *p)
{
	...
	if (active_fw == DBMDX_FW_VA) {
		if (usecase_id) {
			p->unlock(p);
			p->usr_op_lock(p);
			dbmdx_usecase_manager(p,
				((usecase_id << DBMDX_USECASE_MGR_UID_BITS) & DBMDX_USECASE_MGR_UID_MASK)
	...
	struct snd_pcm_substream *substream)
{
	...
	p->current_pcm_rate = p->audio_pcm_rate;
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	int ret;
	unsigned int va_reg = DBMDX_VA_CMD_MASK | ((reg & 0xfff) << 16);
#if defined(SOC_CONTROLS_FOR_DBMDX_CODEC_ONLY)
#ifdef SND_SOC_COMPONENT
	struct snd_soc_component *component = snd_kcontrol_chip(kcontrol);
	struct dbmdx_private *p = snd_soc_component_get_drvdata(component);
	...
	ret = dbmdx_send_cmd(p, DBMDX_VA_CMD_MASK | ((reg & 0xfff) << 16),
				  &val);
	if (ret < 0)
		dev_err(p->dev, "%s: read 0x%x error\n", __func__, reg);
...
static int dbmdx_dsp_control_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
#if defined(SOC_CONTROLS_FOR_DBMDX_CODEC_ONLY)
#ifdef SND_SOC_COMPONENT
	...
	ucontrol->value.integer.value[0] = p->va_dsp_control;
...
/* 0 dsp disable (default)	*/
/* 1 load firmware			*/
static int dbmdx_dsp_control_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
#if defined(SOC_CONTROLS_FOR_DBMDX_CODEC_ONLY)
#ifdef SND_SOC_COMPONENT
...
static int dbmdx_dsp_idle_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
#if defined(SOC_CONTROLS_FOR_DBMDX_CODEC_ONLY)
#ifdef SND_SOC_COMPONENT
	...
	ucontrol->value.integer.value[0] = p->va_dsp_idle;
...
/* 0 dbmdx in detection		*/
/* 1 dbmdx in idle	(default)	*/
static int dbmdx_dsp_idle_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)

{
	...
	int ret = 0;
	unsigned int val = ucontrol->value.integer.value[0], cmd = 0;
	...
	/* if capture in progress skip set idle */
	if (dbmdx_pcm_get_capture_status(p->active_substream) == 1) {
		dev_err(p->dev, "%s: Capturing in progress, abort setting IDLE\n",
		__func__);
		return -EAGAIN;
	}
	...
	/* no need to set idle when val same as before */
	if (p->va_dsp_idle == val) {
		dev_info(p->dev, "%s: Idle already %d, no need to reset\n",
		__func__, p->va_dsp_idle);
		goto out;
	}
	...
	if (val == 0) {
		...
		ret = dbmdx_usecase_manager(p, cmd);
		if (ret < 0) {
			dev_err(p->dev, "%s: Usecase manager cannot start detection\n",
			__func__);
			goto out;
		}
		...
		cmd = DBMDX_USECASE_MGR_CMD_SET_IDLE;
	} else {
		ret = EINVAL;
		dev_err(p->dev, "%s: Invalid value!\n",
			__func__);
...
static int dbmdx_dsp_model_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
#if defined(SOC_CONTROLS_FOR_DBMDX_CODEC_ONLY)
#ifdef SND_SOC_COMPONENT
	...
	ucontrol->value.integer.value[0] = p->va_dsp_model;
...
/* 0 no model / unload model (default)	*/
/* 1~8 load model						*/
static int dbmdx_dsp_model_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)

{
	...
	if (val >= 0 && val <= 8) {
		p->va_dsp_model = val;
		modelAddress = adfModel_PreLoad(NULL);
		if (val != 0) { /* 1 ~ 8 */
			struct firmware *wakewordModel;
			bool loadResult;
			char chunk_name_buf[NAME_MAX + 1] = {0};
			char *chunk_name = chunk_name_buf;

			ret = adfModel_GetFilename(chunk_name, val-1);
			if (ret < 0) {
				goto out;
			}
			ret = request_firmware(
...
static int dbmdx_spi_interface_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
...
static int dbmdx_spi_interface_set(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
#if defined(SOC_CONTROLS_FOR_DBMDX_CODEC_ONLY)
#ifdef SND_SOC_COMPONENT
...
static const struct snd_kcontrol_new dbmdx_va_snd_controls[] = {
	...
	SOC_SINGLE("Verification Level",
		   ...
		   0, 0xffff, 0),
	SOC_SINGLE_EXT("Backlog size", (DBMDX_REGN_VT_AUDIO_HISTORY_CFG>>16)&0x0fff, 0, 0x0fff, 0,
		dbmdx_va_control_get, dbmdx_va_control_put),
...
#endif
	SOC_SINGLE_EXT("DSP Control", 0, 0, 0xff, 0,
		dbmdx_dsp_control_get, dbmdx_dsp_control_set),
	SOC_SINGLE_EXT("DSP Model", 0, 0, 0xff, 0,
		dbmdx_dsp_model_get, dbmdx_dsp_model_set),
	SOC_SINGLE_EXT("DSP Idle", 0, 0, 0xff, 0,
		dbmdx_dsp_idle_get, dbmdx_dsp_idle_set),
	SOC_SINGLE_EXT("SPI Interface", 0, 0, 0xff, 0,
		dbmdx_spi_interface_get, dbmdx_spi_interface_set),
};
...
#ifdef SND_SOC_COMPONENT
static const struct snd_soc_component_driver soc_component_dev_dbmdx = {
#else
...
static int dbmdx_get_detection_data(struct dbmdx_private *p,
					s64 event_time_ms,
					char *uevent_buf,
					size_t uevent_buf_size,
					u16 *vte_event_id,
					bool *buffering_flag)
{
	u16 event_id = 23;
	...
	if (p->pdata->send_uevent_on_detection && uevent_buf) {
		...
		if (det_vt_engine->detection_type == DETECTION_TYPE_PHRASE) {
			...
			act_passphrase_length_in_samples =
					passphrase_length_in_samples *
					num_of_channels;
			act_passphrase_length_in_ms =
					passphrase_length_in_samples * 1000 /
					p->pdata->va_buffering_pcm_rate;
			...
			"VOICE_WAKEUP EVENT_TYPE=Detection ACTION=HOTWORD EVENT_ID=%d",
					event_id);
					...
					bool to_start_buffering)
{
	...
	if (dbmdx_set_power_mode(p, DBMDX_PM_ACTIVE)) {
		p->va_flags.wd_reset = 1;
		p->unlock(p);
		ret = dbmdx_perform_recovery(p);
		goto out;
	}
	...
	if (!p->va_ve_flags.ignore_vt_data) {
		if (p->pdata->send_uevent_on_detection)
			ret = dbmdx_get_detection_data(p,
					event_time_ms,
					uevent_buf, sizeof(uevent_buf),
					&event_id,
					&to_start_buffering);
		else
			ret = dbmdx_get_detection_data(p, 0, NULL, 0,
						&event_id, &to_start_buffering);

		if (ret < 0) {
			dev_err(p->dev,	"%s: failed to read detection data\n",
								__func__);
			p->va_flags.wd_reset = 1;
			p->unlock(p);
			ret = dbmdx_perform_recovery(p);
		...
		dev_info(p->dev, "%s: Ignoring VT data\n", __func__);
		// for production usecase
		to_start_buffering = true;
		ret = dbmdx_send_cmd(p,	DBMDX_REGN_DETECT_AND_SYS_EVENTS,
								&vt_index_val);
		if (ret < 0) {
			dev_err(p->dev,	"%s: failed to read Event ID\n", __func__);
...
static void dbmdx_uevent_work(struct work_struct *work)
{
	...
	ret = dbmdx_process_detection_irq(p, p->pdata->auto_buffering);
...
static void dbmdx_pcm_streaming_work_mod_0(struct work_struct *work)
{
	...
	int bytes_per_sample = p->bytes_per_sample;
	unsigned int bytes_to_read;
	u16 nr_samples, nr_reg_samples = 0;
	size_t avail_samples;
	unsigned int total = 0;
	int kfifo_space = 0;
	int retries = 0;
	...
	do {
		...
		if (nr_reg_samples >= 2*samples_chunk_size) {
			nr_samples = nr_reg_samples - samples_chunk_size;
		} else {
			if (p->mics_connected_to_va_ve_chip)
				ret = dbmdx_switch_to_va_ve_chip_interface(p,
								DBMDX_CMD_INTERFACE);
		...
		nr_reg_samples = nr_samples;

		p->unlock(p);
		//dev_dbg(p->dev, "%s nr_samples = 0x%x samples_chunk_size = 0x%x\n", 	__func__, (int)nr_samples, (int)samples_chunk_size);
		...
		if (nr_samples >= samples_chunk_size) {
			...
			if (ret < 0) {
				...
				p->dspStreamDuration += period_bytes / ONE_MS_DATA_LENGTH;
			}
...
static void dbmdx_pcm_streaming_work_mod_1(struct work_struct *work)
{
	...
	if (cur_config & 0x80) {
		...
		if (ret < 0) {
			dev_err(p->dev, "%s: failed to reset backlog position\n",	__func__);
			goto out_fail_unlock;
		}
	}
...
static irqreturn_t dbmdx_sv_interrupt_hard(int irq, void *dev)
{
	...
	if (p && (p->device_ready) && (p->va_flags.irq_inuse || p->va_flags.wd_inuse)) {
...
static irqreturn_t dbmdx_sv_interrupt_soft(int irq, void *dev)
{
	...
	dev_dbg(p->dev, "%s\n", __func__);
	if ((p->device_ready) && (p->va_flags.irq_inuse || p->va_flags.wd_inuse)) {
		...
		/* IRQ log for auto test scripts */
		dev_info(p->dev, "%s - DBMD4 DSP IRQ Issued Successfully!\n", __func__);
	}
	return IRQ_HANDLED;
}

#ifndef NOT_AUTOLOAD_FW
...
static int dbmdx_common_probe(struct dbmdx_private *p)
{
	...
	if (p->vregulator) {
		ret = regulator_set_voltage(p->vregulator,
				    DBMDX_VDD_VCORE_MIN_UV,
				    DBMDX_VDD_VCORE_MAX_UV);
		if (ret != 0) {
			dev_err(p->dev, "%s: Cannot set vdd_vcore voltage: %d\n",
				__func__, ret);
			goto err_clk_off;
		}
	...
	if (ret != 0) {
		dev_err(p->dev,
#ifdef SND_SOC_COMPONENT
			"%s: Failed to register component and its DAI: %d\n",
#else
	...
	if (ret && ret != -EINVAL) {
		...
		dev_info(p->dev,
			"%s: no project_sub_type, setting to %d\n",
				__func__, 0);
		pdata->project_sub_type = DBMDX_VA_VE_PROJECT_SUB_TYPE_VT_ONLY;
	} else {
		dev_info(p->dev,
...
#ifdef CONFIG_AMAZON_DSP_FRAMEWORK
/* convert dsp ring buffer struct to host ring buffer struct */
static void rBuf_bit16_2_bit8(void *data)
{
	adfRingbuf_t *rbuf = (adfRingbuf_t *) data;
	int offset = sizeof(adfRingbuf_t) / sizeof(uint16_t);
...
/* convert host ring buffer struct to dsp ring buffer struct */
static void rBuf_bit8_2_bit16(void *data)
{
	adfRingbuf_t *rbuf = (adfRingbuf_t *) data;
	int offset = sizeof(adfRingbuf_t) / sizeof(uint16_t);
...
/* convert dsp char array to host char array */
/* data len unit is uint16_t */
static void data_bit16_2_bit8(void *data, uint32_t data_len)
{
	int i = 0;
	uint16_t *rPtr = (uint16_t *)data;
	uint8_t *wPtr = (uint8_t *)data;
	...
	memset(wPtr, 0, data_len);
}

/* convert host char array to dsp char array */
/* data len unit is uint16_t */
static void data_bit8_2_bit16(void *data, uint32_t data_len)
{
	int i = 0;
	uint8_t *rPtr = ((uint8_t *)data) + data_len - 1;
	uint16_t *wPtr = ((uint16_t *)data) + data_len - 1;
...
static int adf_debug_read(uintptr_t src, uintptr_t dest, int size)
{
	int rc = 0;
	rc = dbmdx_debug_read((u8 *)src, (u8 *)dest, size);

	if (rc > sizeof(adfRingbuf_t)) {
		/* log & cli is ring buffer header + char array */
		rBuf_bit16_2_bit8((void *)dest);
		data_bit16_2_bit8(((uint8_t *)dest + sizeof(adfRingbuf_t)),
						(rc - sizeof(adfRingbuf_t)) / sizeof(uint16_t));
	}
	return rc;
}

static int adf_debug_write(uintptr_t dest, uintptr_t src, int size)
{
	if (size > sizeof(adfRingbuf_t)) {
		/* log & cli is ring buffer header + char array */
		rBuf_bit8_2_bit16((void *)src);
		data_bit8_2_bit16((((uint8_t *)src) + sizeof(adfRingbuf_t)),
						(size - sizeof(adfRingbuf_t)) / sizeof(uint16_t));
	}
	...
	if (startRec) {
		if (strstr(logLine, "[CTRL_CLI]") && strstr(logLine, "% ")) {
			...
			if (freqIndex < ARRAY_SIZE(freqTs)) {
				if (kstrtou32(tsPtr, 10, &freqTs[freqIndex]))
					freqTs[freqIndex] = 0;
				freqIndex++;
			}
		}
...
static void dbmdx_do_metrics_upload_work(struct work_struct *work)
{
	 ...
	 */
	if (dbmdx_data->device_ready) {
		mutex_lock(&metrisUploadLock);
		freqTs = (uint32_t *)adfDebug_query(0, cliCmds, ARRAY_SIZE(cliCmds), dbmdx_dsp_log_fliter);
		mutex_unlock(&metrisUploadLock);
		if (freqTs) {
			/*
			* upload freq time occupation to metrics (seconds)
			* freq_idl_ts stand for 80MHZ
			* freq_run_ts stand for 20MHZ
			* freq_vad_ts stand for 10MHZ
			* freq table in dsp log as below
			* [ 61706]<I>[CTRL_CLI] Dump the power/freq interval, total 60705
			* [ 61707]<I>[CTRL_CLI] IDX  FREQ(KHz) PERC  INTERVAL(ms)
			* [ 61708]<I>[CTRL_CLI] =================================
			* [ 61709]<I>[CTRL_CLI]   0     81920   17%  10753
