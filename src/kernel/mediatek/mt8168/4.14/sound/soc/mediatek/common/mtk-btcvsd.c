...
static int btcvsd_tx_clean_buffer(struct mtk_btcvsd_snd *bt)
{
	...
	if (connsys_addr_tx == 0xdeadfeed) {
		...
		dev_info(bt->dev, "%s(), connsys_addr_tx == 0xdeadfeed\n",
			 __func__);
				   ...
				   unsigned int control)
{
	...
	if (connsys_addr_rx == 0xdeadfeed) {
		/* bt return 0xdeadfeed if read register during bt sleep */
		dev_info(bt->dev, "%s(), connsys_addr_rx == 0xdeadfeed\n",
			 __func__);
		return -EIO;
	}
...
static irqreturn_t mtk_btcvsd_snd_irq_handler(int irq_id, void *dev)
{
	...
	if (bt->rx->state != BT_SCO_STATE_RUNNING &&
		...
		dev_info(bt->dev, "%s(), in idle state: rx->state: %d, tx->state: %d\n",
	...
	if (((control >> 31) & 1) == 0) {
		dev_info(bt->dev, "%s(), ((control >> 31) & 1) == 0, control 0x%x\n",
			 __func__, control);
		goto irq_handler_exit;
	}

	if (packet_type >= BT_SCO_CVSD_MAX) {
		dev_info(bt->dev, "%s(), invalid packet_type %u, exit\n",
...
irq_handler_exit:
	*bt->bt_reg_ctl |= BT_CVSD_TX_UNDERFLOW;
	*bt->bt_reg_ctl &= ~BT_CVSD_CLEAR;
	dev_info(bt->dev, "%s(), irq_handler_exit, bt_reg_ctl = 0x%lx\n",
			   ...
			   struct mtk_btcvsd_snd_stream *bt_stream)
{
	...
	while (max_timeout_trial && !bt_stream->wait_flag) {
		...
		if (t2 > timeout_limit) {
			dev_info(bt->dev, "%s(), stream %d, timeout %llu, limit %llu, ret %d, flag %d\n",
		...
		if (ret < 0) {
			 ...
			 * a signal
			 */
			dev_info(bt->dev, "%s(), stream %d, error, trial left %d\n",
				 __func__,
			...
			max_timeout_trial--;
			dev_info(bt->dev, "%s(), stream %d, error, timeout, condition is false, trial left %d\n",
				 __func__,
				 bt_stream->stream, max_timeout_trial);
			    ...
			    size_t count)
{
	...
	while (count) {
		...
		if (count % packet_size != 0 ||
		    avail % packet_size != 0) {
			dev_info(bt->dev, "%s(), count %zu or d %lu is not multiple of packet_size %dd\n",
		...
		if (copy_to_user(buf + cur_buf_ofs,
				 ...
				 read_size)) {
			dev_info(bt->dev, "%s(), copy_to_user fail\n",
				 __func__);
			return -EFAULT;
		}
			     ...
			     size_t count)
{
	...
	while (count) {
		...
		if (count % packet_size != 0 ||
		    avail % packet_size != 0) {
			dev_info(bt->dev, "%s(), count %zu or avail %d is not multiple of packet_size %d\n",
				 __func__, count, avail, packet_size);
			count -= count % packet_size;
			avail -= avail % packet_size;
		}
		...
		if (copy_from_user(bt->tx_packet_buf +
				   ...
				   buf + cur_buf_ofs,
				   write_size)) {
			dev_info(bt->dev, "%s(), copy_from_user fail\n",
				 __func__);
			return -EFAULT;
		}
				    ...
				    struct snd_pcm_hw_params *hw_params)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK &&
		...
		dev_info(bt->dev, "%s(), error, buffer size %d not valid\n",
			 __func__,
				   ...
				   unsigned int __user *data, unsigned int size)
{
	...
	if (copy_to_user(data, &time_buffer_info_rx,
		...
		dev_info(bt->dev, "%s(), copy_to_user fail", __func__);
		ret = -EFAULT;
	}
...
static int mtk_btcvsd_snd_probe(struct platform_device *pdev)
{
	...
	ret = devm_request_irq(dev, irq_id, mtk_btcvsd_snd_irq_handler,
			       ...
			       (void *)btcvsd);
	if (ret) {
		dev_info(dev, "could not request_irq for BTCVSD_ISR_Handle\n");
		return ret;
	}
	...
	if (!btcvsd->bt_pkv_base) {
		dev_info(dev, "iomap bt_pkv_base fail\n");
		return -EIO;
	}

	btcvsd->bt_sram_bank2_base = of_iomap(dev->of_node, 1);
	if (!btcvsd->bt_sram_bank2_base) {
		dev_info(dev, "iomap bt_sram_bank2_base fail\n");
		return -EIO;
	}
	...
	if (IS_ERR(btcvsd->infra)) {
		dev_info(dev, "cannot find infra controller: %ld\n",
	...
	ret = of_property_read_u32_array(dev->of_node, "mediatek,offset",
					 ...
					 ARRAY_SIZE(offset));
	if (ret) {
		dev_info(dev, "%s(), get offset fail, ret %d\n", __func__, ret);
		return ret;
	}
