...
{
#ifdef BYPASS_HPIMP
	pr_debug("%s(), calculate hp_impedance\n", __func__);
...
static const char * const Pmic_Loopback_UL_Mode[] = {
	"OFF", "AMIC", "HEADSET_MIC"};
static const char * const Pmic_Loopback_DL_Mode[] = {
	"OFF", "SPEAKER", "HEADSET_HP"};

static int Codec_Loopback_UL_Mode;
static int Codec_Loopback_DL_Mode;

enum codec_loopback_ul_type {
	AUDIO_ANALOG_UL_OFF = 0,
	AUDIO_ANALOG_LOOP_AMIC,
	AUDIO_ANALOG_LOOP_HEADSET_MIC,
};

enum codec_loopback_dl_type {
	AUDIO_ANALOG_DL_OFF = 0,
	AUDIO_ANALOG_LOOP_SPEAKER,
	AUDIO_ANALOG_LOOP_HEADSET_HP,
};
...
static const struct soc_enum Pmic_Test_Enum[] = {
	...
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(Pmic_LPBK_function),
			    Pmic_LPBK_function),
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(Pmic_Loopback_UL_Mode),
				Pmic_Loopback_UL_Mode),
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(Pmic_Loopback_DL_Mode),
				Pmic_Loopback_DL_Mode),
};
...
static void Enable_ADC_In(bool enable)
{
	...
	adc1 = AUDIO_ANALOG_DEVICE_IN_ADC1;
	adc2 = AUDIO_ANALOG_DEVICE_IN_ADC2;

	pr_debug("%s enable = %d\n", __func__, enable);

	if (enable) {
		if (mCodec_data->mAudio_Ana_DevicePower[adc1] == false) {
			if (mAudio_Analog_Mic1_mode ==
					AUDIO_ANALOGUL_MODE_ACC)
				TurnOnADcPowerACC(adc1,	true);
			else if (mAudio_Analog_Mic1_mode ==
					 AUDIO_ANALOGUL_MODE_DCC)
				TurnOnADcPowerDCC(adc1, true, 0);
			else if (mAudio_Analog_Mic1_mode ==
					 AUDIO_ANALOGUL_MODE_DMIC)
				TurnOnADcPowerDmic(adc1, true);
			else if (mAudio_Analog_Mic1_mode ==
					 AUDIO_ANALOGUL_MODE_DCCECMDIFF)
				TurnOnADcPowerDCC(adc1, true, 1);
			else if (mAudio_Analog_Mic1_mode ==
					 AUDIO_ANALOGUL_MODE_DCCECMSINGLE)
				TurnOnADcPowerDCC(adc1, true, 2);
			mCodec_data->mAudio_Ana_DevicePower[adc1] = true;
		}

		if (mCodec_data->mAudio_Ana_DevicePower[adc2] == false) {
			if (mAudio_Analog_Mic2_mode ==
					AUDIO_ANALOGUL_MODE_ACC)
				TurnOnADcPowerACC(adc2,	true);
			else if (mAudio_Analog_Mic2_mode ==
					 AUDIO_ANALOGUL_MODE_DCC)
				TurnOnADcPowerDCC(adc2, true, 0);
			else if (mAudio_Analog_Mic2_mode ==
					 AUDIO_ANALOGUL_MODE_DMIC)
				TurnOnADcPowerDmic(adc2, true);
			else if (mAudio_Analog_Mic2_mode ==
					 AUDIO_ANALOGUL_MODE_DCCECMDIFF)
				TurnOnADcPowerDCC(adc2,	true, 1);
			else if (mAudio_Analog_Mic2_mode ==
					 AUDIO_ANALOGUL_MODE_DCCECMSINGLE)
				TurnOnADcPowerDCC(adc2,	true, 2);
		}
	} else {
		if (mCodec_data->mAudio_Ana_DevicePower[adc1]) {
			mCodec_data->mAudio_Ana_DevicePower[adc1] = false;
			if (mAudio_Analog_Mic1_mode ==
					AUDIO_ANALOGUL_MODE_ACC)
				TurnOnADcPowerACC(adc1, false);
			else if (mAudio_Analog_Mic1_mode ==
					 AUDIO_ANALOGUL_MODE_DCC)
				TurnOnADcPowerDCC(adc1,	false, 0);
			else if (mAudio_Analog_Mic1_mode ==
					 AUDIO_ANALOGUL_MODE_DMIC)
				TurnOnADcPowerDmic(adc1, false);
			else if (mAudio_Analog_Mic1_mode ==
					 AUDIO_ANALOGUL_MODE_DCCECMDIFF)
				TurnOnADcPowerDCC(adc1,	false, 1);
			else if (mAudio_Analog_Mic1_mode ==
					 AUDIO_ANALOGUL_MODE_DCCECMSINGLE)
				TurnOnADcPowerDCC(adc1,	false, 2);
		}

		if (mCodec_data->mAudio_Ana_DevicePower[adc2]) {
			mCodec_data->mAudio_Ana_DevicePower[adc2] = false;
			if (mAudio_Analog_Mic2_mode == AUDIO_ANALOGUL_MODE_ACC)
				TurnOnADcPowerACC(adc2,
							false);
			else if (mAudio_Analog_Mic2_mode ==
					 AUDIO_ANALOGUL_MODE_DCC)
				TurnOnADcPowerDCC(adc2,	false, 0);
			else if (mAudio_Analog_Mic2_mode ==
					 AUDIO_ANALOGUL_MODE_DMIC)
				TurnOnADcPowerDmic(adc2, false);
			else if (mAudio_Analog_Mic2_mode ==
					 AUDIO_ANALOGUL_MODE_DCCECMDIFF)
				TurnOnADcPowerDCC(adc2, false, 1);
			else if (mAudio_Analog_Mic2_mode ==
					 AUDIO_ANALOGUL_MODE_DCCECMSINGLE)
				TurnOnADcPowerDCC(adc2,	false, 2);
		}
	}
}

static void Codec_Loopback_UL_Func_Off(void)
{
	pr_debug("%s\n", __func__);
...
static void Codec_Loopback_UL_Func_On(int mode)
{
	pr_debug("%s value = %d\n", __func__, mode);
	...
	switch (mode) {
	case AUDIO_ANALOG_LOOP_HEADSET_MIC:
		Set_Mic_Source(1); /* 1 for headset mic */
		break;
	case AUDIO_ANALOG_LOOP_AMIC:
...
static void Codec_Loopback_DL_Func_Off(void)
{
	int spkL, headsetL, headsetR;

	pr_debug("%s\n", __func__);
	spkL = AUDIO_ANALOG_DEVICE_OUT_SPEAKERL;
	headsetL = AUDIO_ANALOG_DEVICE_OUT_HEADSETL;
	headsetR = AUDIO_ANALOG_DEVICE_OUT_HEADSETR;

	if (mCodec_data->mAudio_Ana_DevicePower[spkL] == true) {
		mCodec_data->mAudio_Ana_DevicePower[spkL] = false;
		Speaker_Amp_Change(false);
	}

	if (mCodec_data->mAudio_Ana_DevicePower[headsetL] == true) {
		mCodec_data->mAudio_Ana_DevicePower[headsetL] = false;
		Audio_Amp_Change(AUDIO_ANALOG_CHANNELS_LEFT1, false);
	}
	if (mCodec_data->mAudio_Ana_DevicePower[headsetR] == true) {
		mCodec_data->mAudio_Ana_DevicePower[headsetR] = false;
		Audio_Amp_Change(AUDIO_ANALOG_CHANNELS_RIGHT1, false);
	}
}

static void Codec_Loopback_DL_Func_On(int mode)
{
	int spkL, headsetL, headsetR;
	...
	case AUDIO_ANALOG_LOOP_SPEAKER:
		...
		if (mCodec_data->mAudio_Ana_DevicePower[spkL] == false) {
			Speaker_Amp_Change(true);
			mCodec_data->mAudio_Ana_DevicePower[spkL] = true;
		}
#else
		/*Spk connect to HPL/HPR*/
		if (mCodec_data->mAudio_Ana_DevicePower[headsetL] == false) {
			Audio_Amp_Change(AUDIO_ANALOG_CHANNELS_LEFT1, true);
			mCodec_data->mAudio_Ana_DevicePower[headsetL] = true;
		}
		if (mCodec_data->mAudio_Ana_DevicePower[headsetR] == false) {
			Audio_Amp_Change(AUDIO_ANALOG_CHANNELS_RIGHT1, true);
			mCodec_data->mAudio_Ana_DevicePower[headsetR] = true;
		}
#endif
		break;
	case AUDIO_ANALOG_LOOP_HEADSET_HP:
...
static int Codec_Loopback_UL_Mode_Get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	pr_debug("%s()\n", __func__);
	ucontrol->value.integer.value[0] = Codec_Loopback_UL_Mode;
	return 0;
}

static int Codec_Loopback_UL_Mode_Set(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	uint32_t prev_lpbk_type, next_lpbk_type;

	if (ucontrol->value.enumerated.item[0] >
			ARRAY_SIZE(Pmic_Loopback_UL_Mode)) {
		pr_debug("%s return -EINVAL\n", __func__);
		return -EINVAL;
	}

	prev_lpbk_type = Codec_Loopback_UL_Mode;
	next_lpbk_type = ucontrol->value.integer.value[0];

	if (next_lpbk_type == prev_lpbk_type) {
		pr_debug("%s same codec loopback mode %d %d\n",
			__func__, prev_lpbk_type, next_lpbk_type);
		return 0;
	}

	if (prev_lpbk_type != AUDIO_ANALOG_UL_OFF)
		Codec_Loopback_UL_Func_Off();
	if (next_lpbk_type != AUDIO_ANALOG_UL_OFF)
		Codec_Loopback_UL_Func_On(next_lpbk_type);

	Codec_Loopback_UL_Mode = ucontrol->value.integer.value[0];
	pr_debug("%s() Codec_Loopback_UL_Mode = %d\n",
		__func__, Codec_Loopback_UL_Mode);

	return 0;
}

static int Codec_Loopback_DL_Mode_Get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	pr_debug("%s()\n", __func__);
	ucontrol->value.integer.value[0] = Codec_Loopback_DL_Mode;
	return 0;
}

static int Codec_Loopback_DL_Mode_Set(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	uint32_t prev_lpbk_type, next_lpbk_type;

	if (ucontrol->value.enumerated.item[0] >
			ARRAY_SIZE(Pmic_Loopback_DL_Mode)) {
		pr_debug("%s return -EINVAL\n", __func__);
		return -EINVAL;
	}

	prev_lpbk_type = Codec_Loopback_DL_Mode;
	next_lpbk_type = ucontrol->value.integer.value[0];
	...
	if (prev_lpbk_type != AUDIO_ANALOG_DL_OFF)
		Codec_Loopback_DL_Func_Off();
	if (next_lpbk_type != AUDIO_ANALOG_DL_OFF)
		Codec_Loopback_DL_Func_On(next_lpbk_type);

	Codec_Loopback_DL_Mode = ucontrol->value.integer.value[0];
	pr_debug("%s() Codec_Loopback_DL_Mode = %d\n",
		__func__, Codec_Loopback_DL_Mode);
...
static const struct snd_kcontrol_new mt6357_pmic_Test_controls[] = {
	...
	SOC_SINGLE_EXT("Codec_Debug_Enable", SND_SOC_NOPM, 0, 0xffffffff, 0,
		       codec_debug_get,
		       codec_debug_set),
	SOC_ENUM_EXT("Codec_Loopback_UL_Setting", Pmic_Test_Enum[6],
			   Codec_Loopback_UL_Mode_Get,
			   Codec_Loopback_UL_Mode_Set),
	SOC_ENUM_EXT("Codec_Loopback_DL_Setting", Pmic_Test_Enum[7],
			   Codec_Loopback_DL_Mode_Get,
			   Codec_Loopback_DL_Mode_Set),
};
