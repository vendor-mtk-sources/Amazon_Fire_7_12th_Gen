	...
	int param_num, char *params[])
{
	...
	if (g_step_test_check.step_check_result == TEST_FAIL || index < 0)
		return 0;
...
void wmt_step_test_check_show_act(unsigned int len, ...)
{
	...
	if (g_step_test_check.step_check_result_string == NULL) {
		WMT_ERR_FUNC("STEP test failed: result is NULL");
		return;
	}
	va_start(args, len);
