...
static void connlog_buffer_init(int conn_type)
{
	...
	if (connlog_buffer_table[conn_type].cache_base == NULL) {
		pr_err("connlog mem allocate fail: type=%d, size=%zu\n",
				conn_type, cache_size_table[conn_type]);
	}
...
static int connlog_ring_buffer_init(void)
{
	...
	connlog_set_ring_buffer_base_addr();
	// cache_base or ring_cache was protected by size or log to file check.
	...
	if (gDev.log_data == NULL) {
		pr_err("didn't get mem for log_data.\n");
		return -2;
	}
	connlog_set_ring_ready();
...
static void connlog_ring_buffer_deinit(void)
{
	...
	for (i = 0; i < CONNLOG_TYPE_END; i++) {
		if (connlog_buffer_table[i].cache_base == NULL)
			continue;
		vfree(connlog_buffer_table[i].cache_base);
