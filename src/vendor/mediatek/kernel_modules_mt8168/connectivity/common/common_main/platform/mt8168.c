...
static VOID consys_clk_ctrl(UINT32 enable);
#if defined(CONFIG_MTK_COMBO_PEREGRINE)
static VOID consys_inc_bt_spi_driving(VOID);
static VOID consys_inc_wifi_spi_driving(VOID);
#endif
...
static struct clk *conn_clk[CONN_CLK_MAX];
...
WMT_CONSYS_IC_OPS consys_ic_ops = {
	...
	.consys_ic_clk_ctrl = consys_clk_ctrl,
#if defined(CONFIG_MTK_COMBO_PEREGRINE)
	.consys_ic_inc_bt_spi_driving = consys_inc_bt_spi_driving,
	.consys_ic_inc_wifi_spi_driving = consys_inc_wifi_spi_driving,
#endif
};
...
static INT32 consys_clk_get_from_dts(struct platform_device *pdev)
{
	...
	WMT_PLAT_PR_INFO("Get clock info\n");

	for (i = 0; i < CONN_CLK_MAX; i++) {
		conn_clk[i] = of_clk_get(pdev->dev.of_node, i);
		if (IS_ERR(conn_clk[i])) {
			ret = PTR_ERR(conn_clk[i]);

			if (ret == -EPROBE_DEFER)
				WMT_PLAT_PR_ERR("clk %d is not ready\n", i);
			else
				WMT_PLAT_PR_ERR("get clk %d fail, ret=%d\n",
				       i, (int)ret);
		}
	}
#if 0
	for (i = 0; i < CONN_CLK_MAX; i++) {
		...
		if (1) {
			WMT_PLAT_PR_INFO("clock enable\n");
			clk_prepare_enable(conn_clk[i]);
		} else {
			WMT_PLAT_PR_INFO("clock disable\n");
			clk_disable_unprepare(conn_clk[i]);
		}
	}
#endif
	return 0;
...
static VOID consys_set_if_pinmux(MTK_WCN_BOOL enable)
{
	...
	if (enable) {
		...
		CONSYS_REG_WRITE(consys_if_pinmux_reg_base + CONSYS_IF_DRV_PINMUX_OFFSET,
				(CONSYS_REG_READ(consys_if_pinmux_reg_base + CONSYS_IF_DRV_PINMUX_OFFSET) &
				CONSYS_IF_DRV_PINMUX_MASK) | CONSYS_IF_DRV_PINMUX_VALUE);
	} else {
		CONSYS_REG_WRITE(consys_if_pinmux_reg_base + CONSYS_IF_PINMUX_01_OFFSET,
				CONSYS_REG_READ(consys_if_pinmux_reg_base + CONSYS_IF_PINMUX_01_OFFSET) &
...
static VOID consys_inc_bt_spi_driving(VOID)
{
	...
	consys_if_driving_reg_base = ioremap_nocache(CONSYS_IF_DRIVING_REG_BASE, 0x1000);
	if (!consys_if_driving_reg_base) {
		WMT_PLAT_PR_INFO("consys_if_driving_reg_base(%x) ioremap fail\n", CONSYS_IF_DRIVING_REG_BASE);
		return;
	}

#if 0
	/* Modify BT SPI PIN driving to 4ma */
	CONSYS_REG_WRITE(consys_if_driving_reg_base + CONSYS_IF_DRIVING_BT_SPI_OFFSET,
			(CONSYS_REG_READ(consys_if_driving_reg_base + CONSYS_IF_DRIVING_BT_SPI_OFFSET) &
			CONSYS_IF_DRIVING_BT_SPI_MASK) | CONSYS_IF_DRIVING_BT_SPI_4MA_VALUE);
	WMT_PLAT_PR_INFO("consys driving is set to 4ma (0x%x)\n",
		CONSYS_REG_READ(consys_if_driving_reg_base + CONSYS_IF_DRIVING_BT_SPI_OFFSET));
#else
	/* Modify BT SPI PIN driving to 16ma */
	CONSYS_REG_WRITE(consys_if_driving_reg_base + CONSYS_IF_DRIVING_BT_SPI_OFFSET,
			(CONSYS_REG_READ(consys_if_driving_reg_base + CONSYS_IF_DRIVING_BT_SPI_OFFSET) &
			CONSYS_IF_DRIVING_BT_SPI_MASK) | CONSYS_IF_DRIVING_BT_SPI_16MA_VALUE);
	WMT_PLAT_PR_INFO("consys driving is set to 16ma (0x%x)\n",
		CONSYS_REG_READ(consys_if_driving_reg_base + CONSYS_IF_DRIVING_BT_SPI_OFFSET));
#endif

	if (consys_if_driving_reg_base)
		iounmap(consys_if_driving_reg_base);
}
...
static VOID consys_inc_wifi_spi_driving(VOID)
{
...
#if 0
	/* Modify BT SPI PIN driving to 4ma */
	CONSYS_REG_WRITE(consys_if_driving_reg_base + CONSYS_IF_DRIVING_WIFI_SPI_OFFSET,
			(CONSYS_REG_READ(consys_if_driving_reg_base + CONSYS_IF_DRIVING_WIFI_SPI_OFFSET) &
			CONSYS_IF_DRIVING_WIFI_SPI_MASK) | CONSYS_IF_DRIVING_WIFI_SPI_4MA_VALUE);
	WMT_PLAT_PR_INFO("consys driving is set to 4ma (0x%x)\n",
		CONSYS_REG_READ(consys_if_driving_reg_base + CONSYS_IF_DRIVING_WIFI_SPI_OFFSET));
#else
	/* Modify BT SPI PIN driving to 16ma */
	CONSYS_REG_WRITE(consys_if_driving_reg_base + CONSYS_IF_DRIVING_WIFI_SPI_OFFSET,
			(CONSYS_REG_READ(consys_if_driving_reg_base + CONSYS_IF_DRIVING_WIFI_SPI_OFFSET) &
			CONSYS_IF_DRIVING_WIFI_SPI_MASK) | CONSYS_IF_DRIVING_WIFI_SPI_16MA_VALUE);
	WMT_PLAT_PR_INFO("consys driving is set to 16ma (0x%x)\n",
		CONSYS_REG_READ(consys_if_driving_reg_base + CONSYS_IF_DRIVING_WIFI_SPI_OFFSET));
#endif
...
static VOID consys_hw_reset_bit_set(MTK_WCN_BOOL enable)
{
	...
	if (enable) {
...
#endif
		cnt = 0;
		while (CONSYS_REG_READ(conn_reg.mcu_base + 0x600) != 0x1D1E) {
			if (cnt > 10)
				break;
			WMT_PLAT_PR_INFO("poll 0x1D1E(%d): 0x18002600(0x%x), 0x1800216c(0x%x), 0x18007104(0x%x)\n",
					cnt,
		...
		cnt = 0;
		while (CONSYS_REG_READ(conn_reg.mcu_conn_hif_on_base + CONSYS_CPUPCR_OFFSET) != 0xB04) {
			if (cnt > 10)
				break;
			WMT_PLAT_PR_INFO("poll 0xB04(%d): 0x18002600(0x%x), 0x1800216c(0x%x), 0x18007104(0x%x)\n",
					cnt,
					CONSYS_REG_READ(conn_reg.mcu_base + 0x600),
					CONSYS_REG_READ(conn_reg.mcu_base + 0x16c),
					CONSYS_REG_READ(conn_reg.mcu_conn_hif_on_base + CONSYS_CPUPCR_OFFSET));
...
static INT32 polling_consys_chipid(VOID)
{
	...
	if (consys_reg_base) {
		value = CONSYS_REG_READ(consys_reg_base);
		value = (value & 0xff7fffff) | 0x800000;
		CONSYS_REG_WRITE(consys_reg_base, value);
		iounmap(consys_reg_base);
	}
	...
	if (enable) {
		...
		if (reg_VCN18) {
			if (regulator_disable(reg_VCN18))
				WMT_PLAT_PR_INFO("disable VCN_1V8 fail!\n");
			else
...
static UINT32 consys_store_reset_control(struct platform_device *pdev)
{
	...
	if (IS_ERR(rstc)) {
		iRet = PTR_ERR(rstc);
		WMT_PLAT_PR_ERR("CanNot get consys reset. ret=%d\n", iRet);
		return PTR_ERR(rstc);
	}
#endif
	return 0;
...
static VOID consys_clk_ctrl(UINT32 enable)
{
	...
	for (i = 0; i < CONN_CLK_MAX; i++) {
		...
		if (enable) {
			WMT_PLAT_PR_INFO("clock enable\n");
			ret = clk_prepare_enable(conn_clk[i]);
			if (ret)
				clk_put(conn_clk[i]);
		} else {
			WMT_PLAT_PR_INFO("clock disable\n");
