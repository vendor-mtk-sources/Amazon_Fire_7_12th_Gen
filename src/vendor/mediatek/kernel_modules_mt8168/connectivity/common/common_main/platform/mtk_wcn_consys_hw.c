...
INT32 mtk_wcn_consys_hw_reg_ctrl(UINT32 on, UINT32 co_clock_type)
{
	...
	if (on) {
		...
		if (wmt_consys_ic_ops->consys_ic_inc_bt_spi_driving)
			wmt_consys_ic_ops->consys_ic_inc_bt_spi_driving();
		if (wmt_consys_ic_ops->consys_ic_inc_wifi_spi_driving)
			wmt_consys_ic_ops->consys_ic_inc_wifi_spi_driving();
#endif
		...
		if (wmt_consys_ic_ops->consys_ic_clk_ctrl)
			wmt_consys_ic_ops->consys_ic_clk_ctrl(ENABLE);
		...
		if (wmt_consys_ic_ops->consys_ic_clk_ctrl)
			wmt_consys_ic_ops->consys_ic_clk_ctrl(DISABLE);
		if (co_clock_type) {
			if (wmt_consys_ic_ops->consys_ic_clock_buffer_ctrl)
