...
INT32 stp_dbg_nl_send(PINT8 aucMsg, UINT8 cmd, INT32 len)
{
	...
	INT32 ret = 0;
	INT32 killed_num = 0;
	static int error_log_print_flag;
	...
	for (i = 0; i < num_bind_process; i++) {
		...
		if (skb) {
			...
			if (msg_head == NULL) {
				nlmsg_free(skb);
				if ((error_log_print_flag & 0x1) == 0) {
					STP_DBG_PR_ERR("%s(): genlmsg_put fail...\n", __func__);
					error_log_print_flag |= 0x1;
				}
				return -1;
			} else {
				error_log_print_flag &= ~0x1;
			...
			if (rc != 0) {
				nlmsg_free(skb);
				if ((error_log_print_flag & 0x2) == 0) {
					STP_DBG_PR_ERR("%s(): nla_put_string fail...: %d\n", __func__, rc);
					error_log_print_flag |= 0x2;
				}
				return rc;
			} else {
				error_log_print_flag &= ~0x2;
			...
			if (rc != 0) {
				if ((error_log_print_flag & 0x4) == 0) {
					STP_DBG_PR_ERR("%s(): genlmsg_unicast fail...: %d\n", __func__, rc);
					error_log_print_flag |= 0x4;
				}
...
INT32 stp_dbg_dump_send_retry_handler(PINT8 tmp, INT32 len)
{
	...
	while (ret) {
		...
		STP_DBG_PR_INFO("**dump send fails, and retry again.**\n");
		osal_sleep_ms(3);
		ret = stp_dbg_nl_send(tmp, 2, len+5);
		if (!ret)
