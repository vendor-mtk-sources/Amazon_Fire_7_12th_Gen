...
#ifdef CONFIG_AMZN_POWEROFF_LOG
...
BLOCKING_NOTIFIER_HEAD(keycombo_notifier_list);
...
{
    return blocking_notifier_chain_register(&keycombo_notifier_list, nb);
}
EXPORT_SYMBOL(register_keycombo_notifier);

int unregister_keycombo_notifier(struct notifier_block *nb)
{
    return blocking_notifier_chain_unregister(&keycombo_notifier_list, nb);
}
EXPORT_SYMBOL(unregister_keycombo_notifier);
...
static void log_long_press_power_key(void)
{
	...
	if (rc < 0)
		pr_err("call /system_ext/bin/crashreport failed, rc = %d\n", rc);

	/*Sleep 6 seconds for saving logs*/
	msleep(6000);
}
#endif /* CONFIG_AMZN_POWEROFF_LOG */
...
static void do_panic(void *data)
{
	...
	blocking_notifier_call_chain(&keycombo_notifier_list, COMBO_FUNC_PANIC, NULL);
...
static void do_power_off(void *data)
{
	...
	pr_info("%s above %d seconds\n", keys_cfg->func->msg,
		keys_cfg->pcombo_data->key_down_delay / 1000);
	...
	blocking_notifier_call_chain(&keycombo_notifier_list, COMBO_FUNC_POWER_OFF, NULL);
